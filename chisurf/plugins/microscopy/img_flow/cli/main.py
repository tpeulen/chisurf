...
@click.option("--method", type=click.Choice(["stics", "pcf"]), default="stics",
              show_default=True,
              help="Track the peak's position (stics) or its arrival time (pcf).")
@click.option("--channel", "-c", default="0", show_default=True,
              help="Image channel, by index or by name.")
@click.option("--tile", type=int, default=24, show_default=True,
              help="Tile size in pixels — the spatial resolution of the map.")
@click.option("--step", type=int, default=0,
              help="Distance between tile origins (default: half a tile).")
@click.option("--lags", "n_lags", type=int, default=5, show_default=True,
              help="STICS: frame lags 0..N-1. The peak must stay inside the tile.")
@click.option("--distance", type=int, default=4, show_default=True,
              help="pCF: pair distance in pixels.")
@click.option("--pixel-dwell", "pixel_duration_us", type=float, default=20.0,
              show_default=True, help="Pixel dwell in microseconds.")
@click.option("--line-time", "line_duration_ms", type=float, default=0.0,
              help="Line time in ms (default: pixels per line x dwell).")
@click.option("--frame-time", "frame_duration_ms", type=float, default=0.0,
              help="Frame time in ms (default: lines x line time).")
@click.option("--pixel-size", "pixel_size_nm", type=float, default=100.0,
              show_default=True, help="Pixel size in nm; it scales every velocity.")
@click.option("--subtract", "subtract_average",
              type=click.Choice(["frame", "stack", ""]), default="frame",
              show_default=True,
              help="'stack' also removes the time-average, i.e. the immobile part.")
@click.option("--min-quality", type=float, default=0.5, show_default=True,
...
def map_command(filename, as_json, **kwargs):
    ...
    click.echo(f"mean vector: ({summary['mean_vx']:.4g}, {summary['mean_vy']:.4g}) um/s"
               f" at {summary['angle_deg']:.1f} deg")
    click.echo(f"coherence: {summary['coherence']:.2f}"
...
def demo_command(path, overwrite, n_frames, as_json):
    ...
    click.echo(f"true flow: {truth['profile']} along {truth['axis']}, "
               f"peak {truth['v_max_um_s']} um/s, D = "
               f"{truth['diffusion_coefficient_um2_s']} um^2/s")
    click.echo(f"timing: dwell {timing['pixel_duration_us']:.1f} us, frame "
               f"{timing['frame_duration_ms']:.1f} ms, pixel "
               f"{timing['pixel_size_nm']:.0f} nm")
    click.echo(f"the peak moves {truth['peak_shift_px_per_frame']:.2f} px per frame "
