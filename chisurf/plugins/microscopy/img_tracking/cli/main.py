...
@click.option("--pixel-size", type=float, default=1.0,
              help="Pixel size in um; 1 reports D in pixels squared per frame.")
@click.option("--frame-interval", type=float, default=1.0,
              help="Frame interval in seconds; 1 reports D per frame.")
@click.option("--method", type=click.Choice(["wavelet", "quantile"]), default="wavelet",
              help="Detector.")
...
@click.option("--min-separation", type=float, default=3.0,
              help="Closest two particles may be in one frame.")
@click.option("--max-distance", type=float, default=5.0,
...
@click.option("--min-track-length", type=int, default=10,
              help="Shortest track admitted to the transport fit.")
@click.option("--fit-alpha", is_flag=True,
...
@click.option("--simulate", is_flag=True,
              help="Track a simulated movie with a known D instead of a file.")
...
@click.option("--tracks-csv", type=click.Path(), default=None,
              help="Write every linked detection to this CSV file.")
@click.option("--output", "-o", type=click.Path(), default=None,
              help="Write the result as JSON to this file.")
def cli(image, channel, max_frames, pixel_size, frame_interval, method, threshold,
        min_area, min_separation, max_distance, max_gap, min_track_length, fit_alpha,
        bootstrap, simulate, sim_diffusion, sim_particles, sim_frames, sim_size,
        sim_seed, as_json, tracks_csv, output):
    ...
    if simulate:
        try:
            frames, _ = tk.simulate_particle_movie(
                n_frames=sim_frames, shape=(sim_size, sim_size),
                n_particles=sim_particles, diffusion_coefficient=sim_diffusion,
                seed=sim_seed,
            )
    ...
    result = _core.analyse(
        frames, pixel_size=pixel_size, frame_interval=frame_interval,
        method=method, threshold=threshold, min_area=min_area,
        min_separation=min_separation, max_distance=max_distance,
        max_frame_gap=max_gap, min_track_length=min_track_length,
        fix_alpha=None if fit_alpha else 1.0, n_bootstrap=bootstrap, info=info,
    )
