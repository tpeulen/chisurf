...
@click.option("--pixel-time", "-t", type=float, default=None,
              help="Your intended pixel dwell time in µs; marked against the sweep.")
@click.option("--pixel-size", type=float, default=50.0, show_default=True,
...
@click.option("--n-particles", type=float, default=50.0, show_default=True,
              help="Molecules in the illuminated region.")
@click.option("--brightness", type=float, default=100.0, show_default=True,
...
@click.option("--frames", type=int, default=100, show_default=True,
              help="Frames averaged. Precision improves as its square root.")
@click.option("--line-overhead", type=float, default=1.2, show_default=True,
...
@click.option("--points", type=int, default=9, show_default=True,
              help="Dwell times swept, logarithmically from 0.5 µs to 0.5 ms.")
@click.option("--n-lags", type=int, default=4, show_default=True,
              help="Largest lag fitted per axis. Cost grows as its fourth power.")
@click.option("--repeats", type=int, default=40, show_default=True,
...
@click.option("--out-csv", type=click.Path(), default=None,
              help="Write the swept prediction as CSV.")
@click.option("--json", "as_json", is_flag=True, help="Print the summary as JSON.")
def cli(diffusion_coefficient, pixel_time, pixel_size, w_r, w_z, n_particles,
        brightness, nx, ny, frames, line_overhead, membrane, points, n_lags,
        repeats, seed, out_csv, as_json):
    ...
    click.echo(
        f"\nbest around {sweep.best_dwell * 1e6:.3g} µs "
        f"at {sweep.best_error * 100:.1f} % error"
    )
    if sweep.current is not None:
        ...
        click.echo(
            f"your {pixel_time:g} µs: {here * 100:.1f} % error, {gain:.1f}x the best"
        )
