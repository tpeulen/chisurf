"""Lightweight rigid-body data structures.

The docking *engine* is ``IMP.bff``'s, in C++ (``IMP.bff.dock`` and the
functions around it). What is here is the plain data the OLGA-style
evaluators pass around -- :class:`RigidBody`, :class:`DistanceRestraint`,
:class:`SpringParameters` -- which is the application's shape for a problem,
not an algorithm, and so does not belong in a library of algorithms.

These briefly lived in ``IMP.bff.fret.engine`` and were dropped when that
package went flat and C++; dataclasses with no IMP in them had no place
there. They are back where they came from.
"""
