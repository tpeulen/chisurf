...
@click.option(
    "--eta", type=float, default=0.01, show_default=True, help="Solvent viscosity (poise)."
)
@click.option(
    "--rm", type=float, default=100000.0, show_default=True, help="Molecular weight (Da)."
)
@click.option(
    "--vbar", type=float, default=0.74, show_default=True, help="Partial specific volume (cm³/g)."
)
...
def run_cmd(
    structures, exe, work_dir, indmode, aer, nsig, temperature, eta, rm, vbar, rho, as_json
) -> None:
    ...
    rows = [
        {"file": r.struct_file, "diffusion_coefficient": r.diffusion_coefficient} for r in results
    ]
    if as_json:
