...
def test_create_structured_handler(db):
    """mmfdb.samples.create_structured creates sample via SampleDefinition."""
    sample_data = {
        "name": "test_create",
...
def test_entity_save_handler_with_sequence(db):
    """mmfdb.entities.save persists entity with sequence."""
    entity = {
...
def test_entity_delete_handler(db, sample_with_entities):
    """mmfdb.entities.delete soft-deletes entity."""
    _, sample_id = sample_with_entities
    with patch_db(db):
...
def test_probe_save_handler_all_fields(db):
    """mmfdb.probes.save persists all chemical fields."""
    probe = {
...
def test_probe_optical_properties_save_handler(db):
    """mmfdb.probes.optical_properties.save persists optical props."""
...
def test_fret_pair_crud_handlers(db, sample_with_entities):
    """mmfdb.fret_pairs.{list,save,delete} round-trip."""
    _, sample_id = sample_with_entities
...
def test_pdbx_suggest_handler():
    """mmfdb.pdbx.suggest_keys returns matching keys for prefix '_flr'."""
