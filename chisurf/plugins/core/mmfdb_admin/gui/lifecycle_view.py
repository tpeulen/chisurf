...
A thin Qt widget over the ``mmfdb.lifecycle.lifecycle.*`` RPC handlers (PRD-23: all logic is in
...
**standalone** — constructed with an ``MMFDBClient`` — so it can be slotted into the
mmfdb-admin dock layout (currently being rewritten, see ``OVERHAUL_PLAN.md``) without
entangling the in-flux tab framework, and so it can be smoke-tested in isolation.

>>> view = LifecycleView(client)            # client: MMFDBClient
