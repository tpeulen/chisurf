...
class _MMFDBBackgroundTask(QtCore.QObject):
    """Run one blocking MMFDB callable in a QThread."""
...
class PasswordChangeDialog(QtWidgets.QDialog):
    """Dialog for changing an MMFDB password with strength feedback."""
...
class MMFDBWidget(NavigationPanelTool):
    ...
    def __init__(self, parent: QtWidgets.QWidget | None = None, client: Any | None = None):
        ...
        super().__init__(
            title="mmfdb-admin — Multiparametric Fluorescence Database",
        ...
        self.setup_detail_widget = MMFDBDetailWidget("setup", parent=self)
        self.project_detail_widget = MMFDBDetailWidget("project", parent=self)
        self.raw_data_detail_widget = MMFDBDetailWidget("raw_data", parent=self)
        self.processing_run_detail_widget = MMFDBDetailWidget("processing_run", parent=self)
        self.processed_product_detail_widget = MMFDBDetailWidget("processed_product", parent=self)
        self.object_detail_widget = MMFDBDetailWidget("object", parent=self)
        self.analysis_detail_widget = MMFDBDetailWidget("analysis", parent=self)
        self.condition_detail_widget = MMFDBDetailWidget("condition", parent=self)
        ...
        if client_was_provided:
            ...
            self._set_status_message("Loading MMFDB overview...")
    ...
    def _verify_admin_access(self) -> None:
        ...
        admin user exists yet). When the MMFDB transport is unreachable the
        ...
        if not has_any_admin:
            return
        active_id = self._active_mmfdb_user_id()
    ...
    def _ensure_authenticated(self, username: str | None = None, password: str | None = None) -> None:
        ...
        username : str, optional
            User ID to authenticate as. Falls back to active MMFDB user if not provided.
        ...
        user_id = username or self._active_mmfdb_user_id()
        if not user_id:
            return
    ...
    def _create_standard_dock_tab(
        self,
        table: QtWidgets.QTableWidget,
        detail_widget: MMFDBDetailWidget,
    ...
    # ------------------------------------------------------------------ #
    # Usage-check helpers (count references across MMFDB)
    # ------------------------------------------------------------------ #
    ...
    def setup_toolbar(self) -> None:
        toolbar = self.addToolBar("mmfdb-admin")
        ...
        self.logout_action.setToolTip("Drop the current MMFDB session")
    ...
    def _update_username_prefill(self) -> None:
        ...
        if self._auth_login_user:
            ...
            default_user = self._active_mmfdb_user_id()
    ...
    def _open_connection_dialog(self) -> None:
        ...
        user = self.username_edit.text() or self._active_mmfdb_user_id()
        dialog = ConnectionAuthDialog(
            user=user,
    ...
    def _on_logout_clicked(self) -> None:
        """Log out of MMFDB and clear the session token."""
        try:
            self.client.logout()
        except Exception as exc:
        ...
        try:
            ...
            chisurf.logging.warning("_on_logout_clicked(self) -> None: %s", exc)
        from chisurf.plugins.core.mmfdb_admin.gui.session import clear_cached_session
    ...
    def _run_background_task(
        ...
        *,
        ...
        worker = _MMFDBBackgroundTask(fn)
    ...
    def _all_items_sources(self) -> list[dict[str, Any]]:
        """Definitions describing every MMFDB type surfaced in the unified dock.
    ...
    def populate_mock_data(self) -> None:
        """Populate the active MMFDB with bundled mock/demo data."""
        ...
        self._set_status_message(
            "Populating MMFDB demo data in background. The GUI remains usable."
        )
    ...
    def _auto_fill_condition(self) -> None:
        ...
        if not cid or self._loading:
            return
        try:
            sample = self.client._call("mmfdb.samples.get", {"sample_id": cid})
    ...
    def refresh(self) -> None:
        ...
        try:
            ...
            self.status_label.setText(f"MMFDB transport unavailable: {exc}")
            self._set_transport_connected(False)
            self._loading = False
            self._refresh_in_progress = False
            return
    ...
    def collect_user(self) -> dict[str, Any]:
        ...
        data["requester_id"] = self._active_mmfdb_user_id()
        return data
    ...
    def _active_mmfdb_user_id(self) -> str:
    ...
    def closeEvent(self, event) -> None:
        if self._background_tasks:
            self._set_status_message(
                "MMFDB task still running. Wait for it to finish before closing."
            )
            event.ignore()
            return
        self._save_dock_layout()
    ...
    def _dock_settings(self):
        settings = QtCore.QSettings(
            str(get_plugin_settings_path("mmfdb_admin")),
    ...
    def setups_tab(self) -> QtWidgets.QWidget:
        ...
        self._install_table_context_menu(
            ...
            item_kind="setup",
            id_col=0,
            delete_one_fn=lambda sid: self.client._call("mmfdb.setups.delete", {"setup_id": sid}),
        )
    ...
    def fill_setup_table(self) -> None:
        ...
        try:
            setups = self.client._call("mmfdb.setups.list").get("setups", [])
        except Exception:
            setups = []
        for item in setups:
    ...
    def validate_setup(self) -> None:
        ...
        if not setup_id:
            return
        try:
            res = self.client._call("mmfdb.setups.validate", {"setup_id": setup_id})
