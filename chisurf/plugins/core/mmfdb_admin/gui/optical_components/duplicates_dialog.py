...
class GroupDetailModel:
    def __init__(
        ...
        selected_probe_id: int | None = None,
    ):
        ...
        self.spec_types = sorted(
            list(
                {
                    sp["spectrum_type"]
                    for sp in self.spectra_data
                    if sp["wavelengths"] and sp["intensity_values"]
                }
            )
        )
    ...
    def plot_view_spec(self):
        from chisurf.core.dataspec import ModelView, PanelSection, PlotSection
    ...
    def meta_view_spec(self):
        ...
        for p in self.probes:
            ...
            probe_sections.append(
                ValueSection(label="Category", attr=f"category_{pid}", kind="str")
            )
...
class DuplicatesDialog(QtWidgets.QDialog):
    ...
    def _update_merge_btn(self) -> None:
        ...
        if checked_count > 0:
            ...
            self._merge_btn.setText(
                f"Merge {checked_count} Checked Group{'s' if checked_count > 1 else ''}"
            )
        else:
    ...
    def _on_tree_selection(
        self, current: QtWidgets.QTreeWidgetItem | None, previous: QtWidgets.QTreeWidgetItem | None
    ) -> None:
        if not current:
            return
    ...
    def _on_merge(self, norm_names_to_merge=None) -> None:
        ...
        try:
            ...
            if success_count > 0:
                dialogs.information(
                    self, "Merge Complete", f"Successfully merged {success_count} group(s)."
                )

                for merged_name in merged_norm_names:
                    self._duplicate_groups = [
                        g for g in self._duplicate_groups if g["norm_name"] != merged_name
                    ]
