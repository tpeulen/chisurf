# MMFDB Admin GUI Test Coverage

Last reviewed: 2026-07-06.

This file is the conservative truth table for MMFDB Admin GUI confidence. If a
...
| --- | --- | --- |
| MMFDB Admin navigation shell | `test_admin_navigation.py` verifies `MMFDBWidget` is a `NavigationPanelTool`, exposes nav/stack widgets, and lists expected panels. | Construction tested. |
...
| Surface | Current evidence | Status |
| --- | --- | --- |
| Sample Metadata | `test_sample_metadata_dock_loads_edits_and_saves_metadata` loads a seeded sample, edits a metadata row through the detail form, saves, and verifies MMFDB state. | Headless interaction tested. |
...
5. Keep every new ordinary form/table test on AutoForm/JSON view specs where
   ...
   layer before adding MMFDB-specific widget code.
...
The full `chisurf/plugins/core/mmfdb_admin/test` suite is green in that
