...
- `manifest.json` declares `mmfdb_admin` version `1.1.0`.
- GUI entrypoint: `mmfdb.admin.gui.tool:MMFDBWidget`.
- Service entrypoint: `mmfdb.admin.backend.services:register_services`.
...
- MMFDB source is vendored at `modules/mmfdb/src/mmfdb`; MMFDB Admin app code lives
  under `modules/mmfdb/src/mmfdb/admin`; `mmfdb` is a transitional
  facade that aliases loaded submodules to avoid duplicate class objects. MMFDB
  Admin, Database Connector, and Project Browser now import the vendored `mmfdb`
  package directly in their active backend/test paths.
- MMFDB dictionary files (`*.dic`) live under `modules/mmfdb/src/mmfdb/data/`; the
  old `chisurf/core/mmfdb/data` copy has been removed.
- MMFDB runtime paths, object-store root, and default user are resolved through
  `mmfdb.config`; ChiSurf publishes startup values via `MMFDB_*` environment
...
- The fluorophore reference `spectra.db` used for seed/reference imports is
  bundled in `modules/mmfdb/src/mmfdb/data/` and no longer resolved from the
  ChiSurf `_dev` plugin path by default.
- MMFDB project archiving encodes curve arrays locally and no longer imports
...
| Surface | Status |
| --- | --- |
| MMFDB admin GUI | Implemented. |
| ZMQ/in-process client wrapper | Implemented through `MMFDBClient`. |
| AutoForm entity detail forms | Implemented through `modules/mmfdb/src/mmfdb/admin/gui/autoform_entity_form.py`. |
...
| Main `mmfdb.*` handlers | Implemented across `modules/mmfdb/src/mmfdb/admin/backend/services.py` and helper modules. |
| Versioned `mmfdb.v1.*` core API adapter | Implemented and auth-gated before delegation. |
| Vendored MMFDB package import boundary | Implemented for `modules/mmfdb/src/mmfdb`, with `test/core/test_mmfdb_vendor_package.py` covering canonical `mmfdb` imports and the transitional `mmfdb` facade. |
| Legacy `sample_database.*` aliases | Removed from the supported MMFDB Admin contract. |
| Database connector services | Implemented and covered by temporary-MMFDB direct/in-process tests. |
...
| CLI | Implemented under `mmfdb.admin.cli`; ChiSurf keeps a compatibility wrapper. |

## Manifest Coverage

`manifest.json` declares every method registered by
`mmfdb.admin.backend.services:register_services`. This is enforced by
`test/core/test_mmfdb_vendor_package.py::test_mmfdb_admin_manifest_declares_registered_methods`.
...
- The retired `sample_database` plugin directory has been deleted. Active callers
  should use the ChiSurf `mmfdb_admin` shim and canonical `mmfdb.admin` /
  `mmfdb.*` / `mmfdb.v1.*` services.
- The old MMFDB Admin `backend/setup_services.py` compatibility module was removed;
  canonical setup handlers are registered as `mmfdb.setups.*` from
  `modules/mmfdb/src/mmfdb/admin/backend/services.py`.
- Many MMFDB Admin panels are still hand-written Qt. Ordinary form/table/detail
  surfaces should be migrated to AutoForm and JSON view specs.
- Many MMFDB Admin GUI panels are construction-tested only. Manual testing has
...
- The vendored `mmfdb` package still contains ChiSurf-specific integration imports
  in `mmfdb.chinet_adapter` for fit-state serialization and ChiNet parameter
...
4. Migrate manual MMFDB Admin UI panels to AutoForm. Use the shared
   `TableSection` primitive for read-only browse tables and add any missing
   primitives to `chisurf/gui/autoform` instead of local MMFDB-only widgets.
5. Keep the `core/database_connector` and `core/project_browser` plugin-local
   tests in the MMFDB smoke set; they now prove the non-legacy database path.
...
```bash
PYTHONPATH="modules/mmfdb/src:modules/chinet:modules/imp-tricks/src:." python3 -m pytest test/gui/test_autoform_table_section.py chisurf/plugins/core/mmfdb_admin/test/test_autoform_entity_form.py chisurf/plugins/core/mmfdb_admin/test/test_connection_dialog.py
```
...
PYTHONPATH="modules/mmfdb/src:modules/chinet:modules/imp-tricks/src:." python3 -m pytest \
  test/core/test_mmfdb_vendor_package.py \
  test/gui/test_autoform_table_section.py \
  chisurf/plugins/core/mmfdb_admin/test/test_admin_navigation.py \
  chisurf/plugins/core/mmfdb_admin/test/test_autoform_entity_form.py \
  chisurf/plugins/core/mmfdb_admin/test/test_connection_dialog.py \
  chisurf/plugins/core/mmfdb_admin/test/test_entity_dock_gui_interactions.py \
  chisurf/plugins/core/mmfdb_admin/test/test_session_sso.py \
...
MMFDB Admin plus adjacent database surfaces:

```bash
PYTHONPATH="modules/mmfdb/src:modules/chinet:modules/imp-tricks/src:." python3 -m pytest \
  test/core/test_mmfdb_vendor_package.py \
  chisurf/plugins/core/mmfdb_admin/test \
  chisurf/plugins/core/database_connector/test \
  chisurf/plugins/core/project_browser/test
```
...
PYTHONPATH="modules/mmfdb/src:modules/chinet:modules/imp-tricks/src:." python3 -m pytest \
  chisurf/plugins/core/mmfdb_admin/test \
  --ignore=chisurf/plugins/core/mmfdb_admin/test/test_admin_handlers.py
```
...
root = Path("chisurf/plugins/core/mmfdb_admin")
