...
def as_transmission_fraction(spec, label=None):
    ...
    if np.isfinite(peak) and peak > PERCENT_PEAK_THRESHOLD:
        logger.warning(
            ...
            label if label is not None else "<unnamed>",
            peak,
        )
        arr = arr / 100.0
...
def calculate_r0(
    donor_em: np.ndarray,
    donor_qy: float,
    acceptor_abs: np.ndarray,
    acceptor_ec_max: float,
    kappa2: float = 2 / 3,
    n: float = 1.33,
) -> float:
    ...
    area = np.trapezoid(donor_em, WAVELENGTHS)
    if area <= 0:
        return 0.0
    ...
    J = np.trapezoid(donor_em_norm * acceptor_abs * acceptor_ec_max * (WAVELENGTHS**4), WAVELENGTHS)
        ...
        for src_id, data in p_data.items():
            if isinstance(data, np.ndarray):
                if src_id not in merged:
                    merged[src_id] = np.zeros_like(WAVELENGTHS)
                merged[src_id] += data
    else:
    ...
    if node_type == "light_source":
        ...
        if probe_ids:
            ...
            with db:
                ...
                for probe_id in probe_ids:
                    ...
                    norm = np.trapezoid(cur_em_norm, WAVELENGTHS)
                    if norm > 0:
                    ...
                    for src_id, in_spec in spectral_in.items():
                        excitation_prob = np.trapezoid(in_spec * cur_abs_scaled, WAVELENGTHS)
                        ...
                        out_dict[emission_key(dye_name, src_id)] = excitation_prob * cur_em_scaled
        ...
        if probe_id:
            with db:
                ...
                out_dict = {}
                for src_id, in_spec in spectral_in.items():
                    signals[src_id] = float(np.trapezoid(in_spec * qe_y, WAVELENGTHS))
