...
@click.option("--i-aa", "column_i_aa", default=None,
              help="Column of the acceptor signal under acceptor excitation (auto-detected).")
@click.option("--tau", "column_tau", default=None,
...
@click.option("--linker-sigma", type=float, default=6.0,
              help="Width of the linker distance distribution (Angstrom).")
@click.option("--background", nargs=3, type=float, default=(0.0, 0.0, 0.0),
              help="Backgrounds Bg_DD Bg_DA Bg_AA in the units of the columns.")
@click.option("--gamma-source", type=click.Choice(["auto", "es", "lifetime", "combined"]),
              default="auto", help="Route that determines gamma.")
@click.option("--lightpath", default=None,
              help="Operation id of a saved light path used as the optics prior.")
...
@click.option("--max-populations", type=int, default=3,
              help="Largest number of FRET sub-populations searched.")
@click.option("--list-lightpaths", is_flag=True,
...
def cli(burst_table, column_i_dd, column_i_da, column_i_aa, column_tau, tau_d0, r0,
    ...
    result = _core.calibrate(
        _column("i_dd"), _column("i_da"), _column("i_aa"), _column("tau_f"),
        donor_lifetime=tau_d0, forster_radius=r0, linker_sigma=linker_sigma,
        background=background, gamma_source=gamma_source, n_bootstrap=bootstrap,
        lightpath=prior, use_priors=not no_priors, max_fret_populations=max_populations,
    )
