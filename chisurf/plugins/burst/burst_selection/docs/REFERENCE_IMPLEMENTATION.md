...
adapters, GUI code, and MMFDB archival side effects.

The important rule is ownership: the plugin owns its workflow and MMFDB
integration. `chisurf.core` provides generic MMFDB primitives only. Core must not
...
| --- | --- | --- |
| Models | `api/models.py` | Dataclass request, settings, result, and MMFDB context types. |
| Contract | `api/contract.py` | JSON-safe payload normalization, canonical RPC method names, contract descriptor. |
| Pure analysis | `api/selection.py`, `api/features.py`, `api/io.py` | Compute burst tables and write requested analysis files. No MMFDB, no Qt, no RPC transport. |
| MMFDB archival | `api/mmfdb.py` | Register successful analysis inputs and outputs with MMFDB using result-registry APIs. |
| Service adapter | `backend/services.py` | Accept JSON/RPC params, build `AnalysisRequest`, run pure analysis, then run MMFDB archival. |
...
| Tests | `tests/`, `test/fio/test_burst_pipeline_mmfdb.py` | Verify API contract, plugin behavior, and MMFDB provenance. |
...
- `mmfdb`: nested `MMFDBContext`.

MMFDB context is intentionally nested:
...
inside `MMFDBContext` so the request boundary stays stable.

## GUI MMFDB Output Preflight

The GUI exposes MMFDB as an output mode alongside file outputs such as CSV
(`.bur`) and HDF5. Selecting MMFDB does not remove the other output choices;
MMFDB-only runs are valid, while zip/remove-folder controls remain tied to file
outputs.

Before batch processing starts with MMFDB selected, `gui/tool.py` resolves each
raw TTTR file to its content MD5 and asks MMFDB whether that content is already
...
raw artifacts in the nested `mmfdb` request context. If a raw input has no sample
...
MMFDB run before any burst processing starts. Completing it registers or selects a
...
`mmfdb.source_artifact_ids`.

Selected-file previews and filter-refresh diagnostics do not perform MMFDB
archival. Only the explicit batch process action with MMFDB selected owns this
preflight.

## Setup Storage

Detector and PIE-window definitions are MMFDB setup definitions. The detector
wizard persists setups to `mmfdb_setup` via `load_detector_setups()` and
...
`tttr_detector_setup:bh_spc_130`. GUI MMFDB preflight ensures the selected setup
row exists before processing starts and puts that ID in `mmfdb.setup_id`.
...
`mmfdb_operation.setup_id`. Consumers should use the operation setup link rather
than parsing detector definitions from artifact metadata.

## Pure Analysis Contract

`analyze_request(request)` is pure with respect to MMFDB. It may read TTTR files
...
- `mmfdb_artifacts`: filled by the service/archival layer, not by pure analysis.
- `warnings`: non-fatal warnings, including MMFDB archival warnings.
...
MMFDB registration happens after pure analysis succeeds:

```text
analyze_files_handler()
  -> analysis_request_from_payload()
  -> analyze_request()
  -> BurstMMFDBPipeline().register_run(request, result)
  -> result.mmfdb_artifacts + result.warnings
```
...
`api/mmfdb.py` uses PRD-03 result-registry APIs:
...
It accepts `mmfdb.MMFDBClientBase` implementations, not just the
local SQLite `MFDatabase` repository. This keeps plugins compatible with future
remote or in-process MMFDB clients that implement the same artifact, object-store,
...
For each input file, `BurstMMFDBPipeline.register_run()` normalizes the file path
with `Path(path).resolve()`.

If `request.mmfdb.source_artifact_ids` contains that normalized path, the pipeline
...
- scalar parameters in `mmfdb_parameter`
...
`path`. Sidecars are convenience outputs; MMFDB consumers should treat
`burst_table` artifacts as the primary scientific result.

## Failure Semantics

MMFDB archival is best effort in user-facing flows:

- `mmfdb.enabled=false`: skip registration and return empty artifact maps.
...
`BurstMMFDBPipeline(db)` with an explicit temporary `MFDatabase`.
...
5. Put MMFDB registration in plugin-owned `api/mmfdb.py`.
6. Type plugin archival dependencies against `MMFDBClientBase`.
7. Use `register_result()` and typed payload models instead of manual MMFDB row
   writes.
8. Run MMFDB archival after successful analysis in `backend/services.py`.
9. Keep GUI code as a request builder and result viewer.
10. Preflight GUI-driven MMFDB output against the sample registry before starting
   irreversible processing.
11. Return warnings for optional archival failures instead of failing analysis.
12. Add focused MMFDB tests with a real temporary `MFDatabase`.
...
PYTHONPATH="modules/chinet:modules/imp-tricks/src:." \
  ...
  test/fio/test_burst_pipeline_mmfdb.py
