...
2. **Save to spectra.db.** Persist through the single canonical entry point
   ...
   **not** the live MMFDB.
3. **Integrate into MMFDB.** ``MFDatabase.import_reference_set`` pulls the staging
   DB into the live, dictionary-driven MMFDB (carrying category + provenance
...
def get_db() -> FluorophoreDatabase:
    ...
    # same source the download/* scripts write to); mmfdb-admin's
    # `csc fluorophore import-reference-set` pulls it into the live MMFDB.
