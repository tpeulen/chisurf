"""Dedicated "Add to MMFDB" panel — session-aware, endpoint + auth.

Adding scraped components to the MMFDB is its own step, configured by an AutoForm
(``endpoint_auth.view.json``). It reuses the **current user session**: if the
session user is an MMFDB administrator, the components are added with no login
...
- **Local file** — write directly into a local MMFDB SQLite file (the resolved
  live MMFDB by default). The active user must be an administrator (checked
...
class AddToMmfdbPanel(QtWidgets.QWidget):
    """Add the staging components to the MMFDB, reusing the current user session."""
    ...
    def __init__(self, db, parent: QtWidgets.QWidget | None = None) -> None:
        ...
        layout.addWidget(QtWidgets.QLabel("<b>Add staging components to the MMFDB</b>"))
        ...
        row.addStretch()
        self._add_btn = QtWidgets.QPushButton("⬆ Add all to MMFDB")
    ...
    def _server_client(self):
        """Authenticated MMFDB client for the configured server (session-first)."""
        from chisurf.plugins.core.mmfdb_admin.gui.client import MMFDBClient
        from chisurf.plugins.core.mmfdb_admin.gui.session import cache_session, cached_token

        m = self._model
        host, cmd, pub = m.host, int(m.cmd_port), int(m.pub_port)
        client = MMFDBClient(host=host, cmd_port=cmd, pub_port=pub)

        # SSO: reuse a session token cached earlier this ChiSurf session (e.g.
        # from a previous mmfdb-admin login) — no password needed.
    ...
    def _refresh_session(self) -> None:
        ...
        if ok:
            ...
            self._session_label.setText(
                f"🔒 Session user <b>{user}</b> may not add to the MMFDB ({note}). "
