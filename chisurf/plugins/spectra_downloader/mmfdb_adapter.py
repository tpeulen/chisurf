"""MMFDB-backed fluorophore database adapter for the development plugin."""
...
# The canonical ``category`` drives the mmfdb-admin "Spectra" radio tabs; the
...
class FluorophoreDatabase(MFDatabase):
    """Legacy fluorophore database facade backed by canonical MMFDB tables."""
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        """Open a fluorophore database stored with the MMFDB schema.
        ...
        readonly : bool, default=False
            Open the database in read-only mode when supported by MMFDB.
        **kwargs
    ...
    def add_probe_type(
        ...
        """Add or return a probe type from the MMFDB probe-type table."""
        del details
        with self.conn:
            self.conn.execute(
    ...
    def get_probe_types(self) -> list[dict[str, Any]]:
        """Return probe types with legacy ``id`` and MMFDB ``type_id`` keys."""
    ...
    def _normalize_optical_property_keys(self) -> None:
        """Rewrite stored optical property aliases to canonical MMFDB keys."""
        rows = self.conn.execute(
    ...
    def add_spectrum(
        ...
        """Store a spectrum as NumPy ``float64`` blobs in MMFDB."""
    ...
    def add_probe_image(
        ...
        """Store a probe structure image in the MMFDB image table."""
        with self.conn:
            now = _utc_now()
            self.conn.execute(
                """INSERT INTO images
