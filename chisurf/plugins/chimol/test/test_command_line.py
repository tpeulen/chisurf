...
from chisurf.plugins.chimol.chimol.renderer.ui.command_line import CommandLine
...
def test_clicking_the_prompt_focuses_it_and_places_the_caret():
    ...
    from chisurf.plugins.chimol.chimol.renderer.ui.command_line import PROMPT
...
# ── the browser's command layer ──────────────────────────────────────────
class _StubViewer:
    """Enough of the browser viewer for the command set to act on.

    Not a mock of every method: the commands that matter here are the ones that
    parse arguments, and those reach the panel and the background, which are
    real objects.
    """
    ...
    def __init__(self) -> None:
        ...
        self.atom_count = 1363
        self.turned: list[tuple[str, float]] = []
        self.framed: list[float] = []
        self.reset = 0

    def turn(self, axis: str, angle: float) -> None:
        self.turned.append((axis, angle))

    def frame(self, buffer: float = 0.0) -> None:
        self.framed.append(buffer)
...
def _commands():
    from chisurf.plugins.chimol.chimol.web.commands import BrowserCommands

    viewer = _StubViewer()
    commands = BrowserCommands(viewer)
    commands.report = viewer.gui.command_line.append_message
    viewer.gui.set_run_command(commands)
    return viewer, commands


def test_the_browser_commands_parse_pymol_syntax():
    import math

    viewer, commands = _commands()
    commands("bg_color white")
    assert viewer.background == (1.0, 1.0, 1.0)

    commands("turn x, 90")
    assert viewer.turned == [("x", pytest.approx(math.pi / 2))]

    commands("set seq_view, off")
    assert viewer.gui.sequence_visible is False
    commands("set seq_view, on")
    assert viewer.gui.sequence_visible is True


def test_an_unknown_command_is_refused_by_name():
    """Silence is the failure mode that costs the most; say what was unknown."""
    _stub, commands = _commands()
    with pytest.raises(ValueError, match="unknown command: colr"):
        commands("colr red")
    with pytest.raises(ValueError, match="unknown setting: shininess"):
        commands("set shininess, 4")


def test_a_refused_command_reaches_the_prompt_log():
    """Through the prompt, which is where the error has to be visible."""
    viewer, _bound = _commands()
    line = viewer.gui.command_line
    line.set_focus(True)
    line.set_text("colr red")
    line.key(keys.KEY_RETURN, "", 0)
    assert line.log[-1].kind == "error"
    assert "unknown command: colr" in line.log[-1].text


def test_the_browser_completes_its_own_command_names():
    _stub, commands = _commands()
    assert "bg_color" in commands.completions("bg", 2)
    assert commands.completions("bg_color wh", 11) == [], (
        "arguments are not completed here, and a wrong pool is worse than none"
    )
