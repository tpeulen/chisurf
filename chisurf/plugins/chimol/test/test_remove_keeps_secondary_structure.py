...
@pytest.mark.skipif(not _PDB.exists(), reason="needs the fetched 148l with its waters and substrate")
def test_removing_the_waters_keeps_the_helices():
    ...
    keys_before = list(zip(np.asarray(state.residue_chain_ids).tolist(), np.asarray(state.residue_ids).tolist()))
    ...
    keys_after = list(zip(np.asarray(state.residue_chain_ids).tolist(), np.asarray(state.residue_ids).tolist()))
