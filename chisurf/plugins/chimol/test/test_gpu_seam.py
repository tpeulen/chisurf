"""The engine reaches the GPU through one seam, and that seam does not lie.

Two things are checked here, and they fail in opposite directions.

``test_only_the_native_backend_names_wgpu`` fails when the engine grows a
...
def _sources():
    """Yield every shipped Python module of the plugin.
