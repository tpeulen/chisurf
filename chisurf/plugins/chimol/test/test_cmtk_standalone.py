"""cmtk is a library: it imports nothing of chimol outside itself, and its name map is current."""
...
_ROOT = pathlib.Path(chimol.__file__).resolve().parents[1]

_PROBE = r'''
import importlib, json, sys
import chimol.cmtk as cmtk
for name in cmtk.CONTROL_MODULES:
    importlib.import_module(f"chimol.cmtk.{name}")
for name in ("painter", "quad_painter", "style", "font", "atlas", "testing"):
    try:
        importlib.import_module(f"chimol.cmtk.{name}")
    except ImportError:
        pass
bad = sorted(m for m in sys.modules if m.startswith("chimol.") and not m.startswith("chimol.cmtk"))
...
def test_the_lazy_name_map_is_current():
    proc = subprocess.run([sys.executable, str(_ROOT / "tools" / "gen_cmtk_names.py"), "--check"],
