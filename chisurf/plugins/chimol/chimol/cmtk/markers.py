...
what a chimol plot needs today and are each one shape simpler than the rest
(triangle-up/down, plus, asterisk are straightforward additions later; see
``okf/plugins/chimol-cmtk.md``). Circle reuses
:func:`~chimol.cmtk.style.disc` -- the same scan-converted circle every
other round control in the toolkit already draws -- rather than a second
implementation; the other three are one or two
:meth:`~chimol.cmtk.painter.Painter.fill_triangle`/
:func:`~chimol.cmtk.painter.line` calls each.
"""
from __future__ import annotations

from . import painter as painter_mod
from . import style
...
def _cross(p, cx: float, cy: float, r: float, colour) -> None:
    ...
    w = max(r * 0.35, 1.0)
    painter_mod.line(p, cx - r, cy - r, cx + r, cy + r, w, colour)
    painter_mod.line(p, cx - r, cy + r, cx + r, cy - r, w, colour)
