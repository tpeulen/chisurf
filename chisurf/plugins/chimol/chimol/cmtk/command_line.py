...
-------------------------------
chimol's typed interface used to exist in exactly one place: a console docked
...
and it knows no GPU. :class:`~chimol.renderer.internal_gui.InternalGui` places
and paints it as quads, the same way it paints every other piece of chrome, and
each host feeds it keys -- Qt through ``keyPressEvent``, the browser through a
``keydown`` listener, both translating into :mod:`chimol.host.keys` first.
...
#: What the prompt says. PyMOL writes its own name here and so does this: the
#: line is drawn over a molecule, not in a terminal, so it has to declare what
#: it is a prompt *for*.
PROMPT = "ChiMOL>"
...
class CommandLine:
    ...
    run : callable, optional
        ...
        else that can run a chimol command passes its own.
    """
    ...
    def __init__(self, run: Callable[[str], None] | None = None) -> None:
    ...
    def submit(self) -> str:
        ...
        self.append(f"{PROMPT} {line}", "echo")
    ...
    def key(self, key: int, text: str = "", modifiers: int = 0) -> bool:
        ...
        ----------
        key : int
            One of :mod:`chimol.host.keys`' constants, or ``0`` for a key that
            only produces text.
        text : str
