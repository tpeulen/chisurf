...
DARK_PALETTE: tuple[Colour, ...] = (
    ...
    # #1e1e1e background; chimol's frame is (50, 50, 56) over a lit scene, and
...
#: The reference's ``GetLightPalette``, same halving. Nothing in chimol selects
...
#: ``build_tools/dev_utils/port_imgui_widget.py``'s ``extract_word_lists``, and
...
class Language:
    ...
    @staticmethod
    def glsl() -> Language:
        """GLSL -- and near enough WGSL, which is what chimol's shaders are."""
        return Language(
            name="GLSL",
            preprocess="#",
            single_line_comment="//",
    ...
    @staticmethod
    def chimol(commands: Iterable[str] = ()) -> Language:
        """Build chimol's own command language from the command registry.

        The reference ships eleven languages and none of them is the one the
        user actually types in this application. A chimol script is a list of
        commands -- ``fetch``, ``show``, ``color``, ``select`` -- with ``#``
        comments and PyMOL-style selection strings, and the command names are
        *already* enumerated by :mod:`chimol.cmd.registry`. So they are passed
        in rather than duplicated: a command added to the registry colours in
        the editor without anyone remembering to add it here.
        ...
        return Language(
            ...
            has_double_quoted_strings=True,
            string_escape="\\",
            keywords=frozenset(str(name) for name in commands),
            identifiers=frozenset(
                "all none hetatm polymer protein nucleic solvent water backbone sidechain "
                "hydro visible enabled name resn resi chain segi elem index byres bychain "
                "within around expand and or not".split()
            ),
            number_pattern=_PY_NUMBER,
        )
...
class Document:
    ...
    def identifiers(self) -> set[str]:
        ...
        There is no autocomplete here, but the chimol command line's completion
...
class TextEditor:
    ...
    *n* is at ``n * text_width("W")``. The chrome's font is Menlo, so this
    ...
    def modified(self) -> bool:
        """Whether anything has been edited since the last :meth:`set_text`."""
    ...
    def key(self, key: int, text: str = "", modifiers: int = 0) -> bool:
        ...
        ----------
        key : int
            One of :mod:`chimol.host.keys`' ``KEY_*`` constants, or ``0`` for a
    ...
    def draw(self, p: Painter, x: float, y: float, w: float, h: float) -> None:
        ...
        self.visible_columns = max(int((w - gutter) / self._glyph_w), 1)
