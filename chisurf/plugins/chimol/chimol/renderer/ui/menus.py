...
---------------------------------------------------
:mod:`~chimol.renderer.internal_gui` already draws menus against the same six
...
* **The column layout.** ``ImGuiMenuColumns`` lays a row out as four columns --
  ...
  is not ported (chimol has no per-item icons); the other three are.
...
* **Time.** The reference opens a submenu on hover after a delay, and keeps it
  ...
  clock and a hover feed, and chimol has neither here. Submenus open on
...
class Menu:
    ...
    title : str, optional
        ...
        none; chimol's do, because a context menu opened at the pointer has
