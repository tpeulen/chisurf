...
class SliderFloat:
    ...
    def __init__(
        ...
        self.value = float(min(max(value, self.v_min), self.v_max))
        self.fmt = fmt
        self._held = False
    ...
    def draw(self, p: Painter, x: float, y: float, w: float, h: float) -> None:
        ...
        # Thumb knob
        thumb_w = 8.0
        thumb_x = x + norm * max(w - thumb_w, 0.0)
    ...
    def step(self, delta: float) -> float:
        """Step value up/down."""
        self.value = float(min(max(self.value + delta, self.v_min), self.v_max))
        return self.value
    ...
    def set_fraction(self, fraction: float) -> float:
        ...
        self.value = self.v_min + fraction * (self.v_max - self.v_min)
        return self.value
