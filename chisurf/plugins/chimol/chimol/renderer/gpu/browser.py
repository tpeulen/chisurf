...
What this is
------------
The other side of :mod:`chimol.renderer.gpu.api`. On the desktop that surface is
...
#: What :func:`chimol.renderer.gpu.backend_name` reports for this backend.
name = "browser"
...
def request_adapter_sync(power_preference: Optional[str] = "high-performance"):
    ...
    if _ADAPTER is None:
        raise RuntimeError(
            ...
            "chimol.renderer.gpu.browser.request_adapter_async() before "
...
class _Device(_Js):
    """A ``GPUDevice``, plus the one convenience the engine expects of it."""
...
class _Adapter(_Js):
    ...
    async def request_device_async(self, **kwargs):
        ...
        self._device = _wrap(await self.js.requestDevice(to_js(_descriptor(kwargs))))
        return self._device
