"""Atomic masses and van der Waals radii. **Generated -- do not edit by hand.**

chimol owns this table. It used to live in ``chisurf.core.fio.structure.elements``
and be re-exported from here, on the reasoning that the *reader* needs it and a
reader is core rather than plugin work. Two things turned out to be true of
that arrangement: the generator (``analysis/make_elements.py``) was already
here in chimol, so only its *output* was in core; and nothing in ChiSurf ever
imported the table -- the only consumers were this module and that generator.
So core held a periodic table on chimol's behalf, and chimol could not be moved
without it.

chimol is becoming its own repository (``okf/plugins/chimol-relocation.md``),
where the dependency runs one way: ChiSurf may import chimol, never the
reverse. Structure data belongs to chimol; ChiSurf keeps what is specific to
experimental data and takes the rest from here.
