...
chimol owns this reader
-----------------------
It lived in ``chisurf.core.fio.trajectory`` and moved here on the rule that
ChiSurf does not own structures -- only what is specific to experimental data
-- and takes the rest from chimol (``okf/plugins/chimol-relocation.md``). The
file was already self-contained (numpy, pathlib, dataclasses; no ChiSurf
import of any kind), so the move is a relocation rather than a port, and
``chisurf.core.fio.trajectory`` now re-exports from here so its own dozen
