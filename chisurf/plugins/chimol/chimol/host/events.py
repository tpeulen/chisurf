...
class Rect:
    ...
    ``MolView.handle_rect_selection`` reaches the same picker either way, and a
...
class PointerEvent:
    ...
    Why this exists
    ---------------
    ``MolView.handle_mouse_click`` and the picker take an *event*: they ask it
...
class KeyEvent:
    ...
    ----------
    key : int
        One of :mod:`chimol.host.keys`' ``KEY_*`` values, or ``0`` for a key
