...
Qt-free :class:`~.view_model.RemoveClashesViewModel`. Imported (and thus
registered) by ``widget``. Mirrors the Align-Trajectory ``traj_align_io``
pattern.
"""
...
@register_section("traj_remove_clashes_io")
def traj_remove_clashes_io(model, target=None, **options):
    """AutoForm factory for the trajectory picker + save-clash-free button."""
    return _IoSection(model)


class _IoSection(QtWidgets.QWidget):
    """Trajectory row (browse + drag-drop) and the save-clash-free action button."""
...
__all__ = ["traj_remove_clashes_io"]
