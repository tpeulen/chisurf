...
from ..api import compute_filters, compute_filters_mfd
    ...
    def _compute_filters_anisotropy(self, chs: list[str]) -> None:
        ...
        try:
            ...
            if len(routing_chs) < 2:
                raise ValueError(
                    f"Detector '{det_name}' has <2 routing channels for Anisotropy mode"
                )
            ...
            self._result_anisotropy = compute_filters_mfd(
                total_par, total_perp, species_par, species_perp, metadata=metadata
            )
