"""Tests for the Converter hub (shared NavigationPanelTool shell)."""


def test_converter_uses_shared_navigation_shell(qapp, qtbot):
    from chisurf.gui.widgets.navigation import NavigationPanelTool
    from chisurf.plugins.tttr.converter.gui.tool import CONVERTER_PANELS, ConverterTool
    ...
    assert "Converter" in w.windowTitle()

    # one navigation entry per panel
    assert w.nav_list.count() == len(CONVERTER_PANELS)
...
def test_panels_are_data_driven_from_json():
    ...
    from chisurf.plugins.tttr.converter.gui import tool as tool_mod
    ...
    built = [p for p in tool_mod.CONVERTER_PANELS if not p.get("separator")]
    assert len(built) == len(tool_panels)
    assert all(callable(p["factory"]) for p in built)


def test_included_plugins_are_menu_hidden():
    """Every aggregated converter appears only inside the hub, not the ribbon."""
    import importlib
    import json
    import pathlib
    ...
    import chisurf.plugins.tttr.tttr_time_windows as tw

    manifest = json.loads((pathlib.Path(tw.__file__).parent / "manifest.json").read_text())
    assert manifest.get("menu_hidden") is True
...
def test_hub_manifest_is_visible_and_loads():
    ...
    assert manifest.id == "converter"
    assert manifest.entrypoints.gui == "chisurf.plugins.tttr.converter.gui.tool:ConverterTool"
