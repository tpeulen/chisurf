...
def test_register_services_registers_all_methods():
    ...
    assert set(d.handlers) == {
        "lut.autodetect_region", "lut.compute", "lut.apply_preview",
        "lut.settings_build", "lut.settings_load",
    }
...
def test_settings_build_and_load_round_trip(tmp_path):
    ...
    built = d.call("lut.settings_build", {
        "channel_luts": {"0": [0.0, 1.0, 2.0]}, "channel_shifts": {"0": 2},
        "reading_routine": "SPC-130", "out_path": out,
    })
...
def test_cli_compute_and_settings(tmp_path):
    ...
    r2 = runner.invoke(cli, ["settings", "--lut", f"0={lut_path}", "--shift", "0=3",
                             "-o", str(settings_path)])
