...
def _compute_handler(files: list[str], routine: str | None = None, n_bins: int = 0,
    ...
    return service_success({
        ...
        "noffset": tbl["noffset"], "total_counts": tbl["total_counts"],
    })


def _apply_preview_handler(path: str, channel: int, routine: str | None = None,
                           channel_luts: dict | None = None,
                           channel_shifts: dict | None = None,
                           coarsening: int = 1, **_: Any) -> dict:
...
def _settings_build_handler(channel_luts: dict, channel_shifts: dict | None = None,
                            reading_routine: str | None = None,
                            out_path: str | None = None, **_: Any) -> dict:
    luts = {int(k): np.asarray(v, dtype=float) for k, v in channel_luts.items()}
    d = settings.build_settings_dict(luts, {int(k): int(v) for k, v in (channel_shifts or {}).items()},
...
def _settings_load_handler(path: str, **_: Any) -> dict:
    ...
    return service_success({
        ...
        "channel_luts": {str(k): np.asarray(v).tolist() for k, v in loaded["channel_luts"].items()},
