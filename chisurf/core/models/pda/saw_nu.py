...
:class:`~chisurf.core.models.pda.pdagauss.PdaGaussianDistanceModel` machinery
...
class PdaSawNuDistances(FittingParameterGroup):
...
class PdaSawNuModel(PdaGaussianDistanceModel):
    """PDA model with a SAW-ν polymer inter-dye distance distribution."""

    name = "PDA-SAW-ν-distance"
    ...
    def __init__(
        ...
        nuisance=None,
        distances: PdaSawNuDistances | None = None,
        kw_hist: dict | None = None,
        **kwargs,
    ):
        """Initialize the SAW-ν PDA model (reuses the Gaussian-model machinery)."""
        if distances is None:
            distances = PdaSawNuDistances(name="pda_saw_nu", fit=fit, **kwargs)
