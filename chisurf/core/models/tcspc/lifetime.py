...
#: Deprecated alias. This was the additive "Lifetime (new)" prototype entry that
#: exercised the data-described editor next to the hand-written widget; that
#: editor is now the only one, so :class:`LifetimeModel` *is* the new model. Kept
#: because user copies of ``experiment_configs.yaml`` and pickled projects pin
#: class paths -- resolving one to ``None`` silently drops the menu entry.
...
#: Deprecated alias, for the same reason as :data:`LifetimeNewModel`: the
#: additive "Lifetime mixer (new)" entry has become the primary mixer, so
#: :class:`LifetimeMixtureModel` carries ``mix_model.view.json`` directly.
