"""Bridge between ndX's ``DataSelection`` gates and ChiSurf regions.

ndX grew its own selection hierarchy — a 1-D interval, a Mahalanobis
...
* **The mask convention is inverted.** ``DataSelection.get_mask`` returns
  ...
  is the same answer in ndX's direction, which is why it exists.
...
whatever it is handed and never imports ndX, so the dependency stays
one-directional.

Round-tripping matters for a reason beyond tidiness. ndX's own
...
def roi_from_selection(
    ...
    its class, so this never imports ndX.
...
def collection_from_selections(
    ...
    ``combine="and"`` because that is what ndX does: a point survives only
