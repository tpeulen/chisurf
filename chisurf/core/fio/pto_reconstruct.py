...
class AnalysisReconstructor:
    ...
    def run(self, artifact_name: str | None = None) -> dict[str, Any]:
        ...
        with Measurement.open(self.pto_path, writable=False) as measurement:
            ...
            target_artifact = (
                candidates[-1] if artifact_name is None else measurement.artifact(artifact_name)
            )
            target_uid = target_artifact.uid

            # Trace lineage back to instrument source
            measurement.lineage(target_uid)
            ...
            source_uid = measurement.tag(target_uid, "_mmfdb_edge.source_uid") or measurement.tag(
                target_uid, "_mmfdb_edge.source_node_id"
            )
            if not source_uid:
            ...
            if not source_uid:
                raise ValueError(
                    f"Could not resolve raw photon stream source for artifact {target_artifact.name}"
                )
            ...
            try:
                ...
                int(settings.get("min_photons", settings.get("l_min", 30)))
                int(settings.get("rate_window", settings.get("m_min", 5)))
                ...
                if n_bursts_original == n_bursts_reconstructed:
                    orig_first = (
                        np.asarray(orig_store["First Photon"])
                        if "First Photon" in orig_store
                        else np.asarray(orig_store["first_photon"])
                    )
                    orig_last = (
                        np.asarray(orig_store["Last Photon"])
                        if "Last Photon" in orig_store
                        else np.asarray(orig_store["last_photon"])
                    )

                    recon_first = (
                        reconstructed_props[:, 0].astype(np.int64)
                        if len(reconstructed_props.shape) == 2
                        else np.array([])
                    )
                    recon_last = (
                        reconstructed_props[:, 1].astype(np.int64)
                        if len(reconstructed_props.shape) == 2
                        else np.array([])
                    )

                    first_diff = (
                        np.max(np.abs(orig_first - recon_first))
                        if len(orig_first) == len(recon_first)
                        else 999
                    )
                    last_diff = (
                        np.max(np.abs(orig_last - recon_last))
                        if len(orig_last) == len(recon_last)
                        else 999
                    )
                ...
                irf_artifacts = [
                    ...
                    or measurement.tag(obj.uid, "_mmfdb_operation.operation_type")
                    == "tcspc_calibration"
                ]

                for irf_art in irf_artifacts:
                    irf_store = measurement.get_store(irf_art.uid)
                    irf_settings = json.loads(
                        measurement.tag(irf_art.uid, "_mmfdb_operation.settings_json") or "{}"
                    )
...
def reconstruct_analysis_from_pto(
    pto_path: str | pathlib.Path, artifact_name: str | None = None
) -> dict[str, Any]:
