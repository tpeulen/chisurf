...
def _safe_float(
    value: typing.Any, default: typing.Optional[float] = None
...
def read_tcspc_csv(
    ...
    **kwargs,
    ...
    csvSetup = chisurf.core.fio.ascii.Csv(*args, **kwargs)
    csvSetup.load(
        filename,
        skiprows=skiprows,
        ...
        if polarization == "vv":
            ...
            if "VM" not in available_channels:
                ...
                if "VV" not in available_channels or "VH" not in available_channels:
                    raise ValueError(
                        "VM channel not found and cannot be calculated (missing VV or VH)"
                    )
                vv = data["VV"]
                vh = data["VH"]
            ...
            if "VM" in available_channels:
                ...
                raise ValueError(
                    "Cannot determine polarization. Available channels: "
        ...
        if n_data_sets >= 2:
            ...
            for i, yi in enumerate(y_rebin[:2]):  # Only take first 2 for VV/VH
                ...
                data = chisurf.core.data.DataCurve(
                    x=x, y=yi, ex=ex, ey=eyi, experiment=experiment, name=name, **kwargs
                )
            ...
            data = chisurf.core.data.DataCurve(
                x=x, y=y_rebin[0], ex=ex, ey=ey[0], experiment=experiment, name=name, **kwargs
            )
            data.filename = filename
            _describe(data)
            data_curves.append(data)
    ...
    _annotate_anisotropy_meta(
        data_group=data_group, g_factor=g_factor, l1=l1, l2=l2, source=calibration_source
    )
    return data_group
