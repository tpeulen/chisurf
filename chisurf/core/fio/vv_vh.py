...
def write_vv_vh(
    filename: str | Path,
    vv: ArrayLike | None = None,
    vh: ArrayLike | None = None,
    ...
    footer: str | Iterable[str] | None = None,
) -> Path:
    """
    ...
    **kwargs
    ...
    if len(set(lengths)) > 1:
        raise ValueError(
            f"All channels must have same length. Got: {dict(zip(channels.keys(), lengths))}"
        )
    ...
    footer_lines = [f"format_version: {format_version}", f"channels: {', '.join(channel_list)}"]
    ...
    if footer is not None:
        ...
        footer = (
            newline.join(f"#{line}" for line in footer_lines) + newline + footer
            if footer
            else newline.join(f"#{line}" for line in footer_lines)
        )
    else:
...
def read_vv_vh(
    ...
    **kwargs
    ...
    channel_data = {
        ch: arr[i * n_points : (i + 1) * n_points].copy() for i, ch in enumerate(channel_list)
    }
    ...
    if (
        not return_metadata
        and split
        and set(channel_list) == {"VV", "VH"}
        and channels is None
        and is_legacy_version
    ):
...
def _read_vv_vh_legacy(
    ...
    if data_text.strip():
        try:
            arr = np.loadtxt(
                io.StringIO(data_text), dtype=dtype, delimiter=delimiter, comments=comments
            )
        except Exception:
