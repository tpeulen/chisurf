...
def model_capabilities(model: dict | str, provider: str = "") -> set[str]:
    ...
    if any(
        _metadata_mentions(value, "text", "chat", "completion", "response")
    ...
    is_non_text_model = any(marker in normalized_id for marker in non_text_markers) or any(
        any(marker in value for marker in non_text_markers) for value in metadata_values
    )
...
def split_models_by_capability(
    models: list[dict | str], provider: str = ""
