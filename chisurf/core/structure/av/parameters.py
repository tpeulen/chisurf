"""Parameter groups for the dye-diffusion model — ChiSurf's editable surface.
...
raised ``ImportError`` — its ``fps_.pyx`` had not existed for a long time),
``DiffusionSimulation``/``DiffusionSimulationParameter``, and
``_quenching_rate_per_frame``, which is now
:func:`IMP.bff.quenching_rate_per_frame`.
"""
