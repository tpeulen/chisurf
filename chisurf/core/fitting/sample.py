...
#: Both the type and its spelling: ``from __future__ import annotations`` means
#: a signature carries the *string* 'float', never the type, so a table keyed by
...
def setting_label(name: str) -> str:
    """Return the display name of a setting.

    Parameters
    ----------
    name : str
        Setting or parameter name.
...
def sampler_settings(name: str) -> list:
    ...
    for param in inspect.signature(func).parameters.values():
        ...
        kind = _SETTING_KINDS.get(annotation)
