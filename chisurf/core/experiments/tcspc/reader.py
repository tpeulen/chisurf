...
class TCSPCReader(ExperimentReader):
    ...
    def _default_anisotropy_calibration(cls):
        ...
        g_factor = cls._safe_float(
            anisotropy_settings.get("g_factor", tcspc_settings.get("g_factor", 1.0)), 1.0
        )
        ...
        try:
            for curve in data_group:
                ...
                curve_meta.setdefault(
                    "g_factor", group_meta.get("g_factor", calibration["g_factor"])
                )
                ...
                if "excitation_period_ns" in group_meta:
                    curve_meta.setdefault(
                        "excitation_period_ns", group_meta["excitation_period_ns"]
                    )
        except Exception:
            pass
    ...
    def read(self, filename: typing.Optional[str] = None, *args, **kwargs) -> typing.Any:
        ...
        if reading_routine == "auto" or (
            reading_routine == self.reading_routine and self.reading_routine == "auto"
        ):
        ...
        if reading_routine == "csv":
            ...
            if reading_routine == "yaml":
                ...
                data_set.load(file_type=file_type, filename=filename)
                data_set.experiment = self.experiment
                data_group = chisurf.core.data.DataGroup([data_set])
            else:
                chisurf.logging.warning(
                    f"Reading routine '{reading_routine}' not supported. Created empty DataGroup"
                )
                data_group = chisurf.core.data.DataGroup([])
