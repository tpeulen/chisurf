...
def add_dataset(experiment_reader, **kwargs):
    ...
    return core_data.add_dataset(
        experiment_reader=experiment_reader, _from_controller=True, **kwargs
    )
...
def remove_datasets(dataset_indices: typing.List[int]):
    ...
    return core_data.remove_datasets(
        dataset_indices=list(dataset_indices or []), _from_controller=True
    )
...
def group_datasets(dataset_indices: typing.List[int]):
    ...
    return core_data.group_datasets(
        dataset_indices=list(dataset_indices or []), _from_controller=True
    )
...
def ungroup_datasets(dataset_indices: typing.List[int]):
    ...
    return core_data.ungroup_datasets(
        dataset_indices=list(dataset_indices or []), _from_controller=True
    )
