        ...
        if self.skip_blacklisted and plugin_name in self.blacklisted:
            if self.result_callback:
                self.result_callback(
                    plugin_name, False, "Skipped: Blacklisted due to repeated failures"
                )
        ...
        try:
            ...
            if hasattr(sys, "modules"):
                ...
                for module_name in sys.modules:
                    if any(
                        plugin_dir in str(module_name)
                        for plugin_info in self.plugins_to_test
    ...
    def _test_plugin(self, plugin_info):
        ...
        try:
            ...
            has_dangerous = _contains_dangerous_operations(
                script_content, dangerous_call_names, dangerous_prefixes
            )

            if has_dangerous:
                return (
                    False,
                    "Plugin contains potentially dangerous operations - skipped for safety",
                )
    ...
    def _test_gui_plugin_safely(self, script_file, script_content):
        ...
        try:
            ...
            for import_line in import_lines:
                try:
                    ...
                    if any(
                        ...
                        for critical in ["dll load failed", "no module named"]
                    ):
            ...
            if critical_failures:
                return (
                    False,
                    f"GUI plugin has critical import issues: {'; '.join(critical_failures[:2])}",
                )
            # If there are minor import issues but syntax is OK, consider it a partial success
            elif failed_imports:
                return (
                    True,
                    f"GUI plugin syntax OK, minor import issues: {'; '.join(failed_imports[:2])}",
                )
            else:
    ...
    def _test_plugin_chisurf_way(self, script_file, script_content):
        ...
        try:
                    ...
                    try:
                        ...
                        if has_qt:
                            ...
                            try:
                                ...
                                return (
                                    True,
                                    "GUI/Qt plugin syntax OK (Qt/GUI execution blocked for safety)",
                                )
                            except SyntaxError as e:
                        ...
                        if has_dangerous:
                            return (
                                False,
                                "Plugin contains potentially dangerous operations - skipped for memory safety",
                            )
                        ...
                        try:
                            ...
                            if any(
                                qt_term in str(e).lower()
                                for qt_term in ["qt", "gui", "widget", "application"]
                            ):
                            ...
                            if any(
                                qt_term in str(e).lower()
                                for qt_term in ["qt", "gui", "widget", "application", "sip"]
                            ):
                        ...
                        return (
                            True,
                            "Plugin executed successfully (comprehensive Qt blocked, memory protected)",
                        )
            ...
            if context is None:
                ...
                context = {"__name__": "plugin", "__package__": package_name}
            ...
            if wizard_path.exists():
                ...
                return mock_main_window.onRunMacro(
                    str(wizard_path), executor="exec", globals=context
                )
            elif init_path.exists():
