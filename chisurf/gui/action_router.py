...
class ActionRouter:
    ...
    def __init__(self):
        self._connections: list[tuple[typing.Any, str, dict]] = []

    def connect(
        self,
        ui_element: typing.Any,
        action_name: str,
        payload: dict | typing.Callable | None = None,
        **kwargs,
    ):
        """
    ...
    def dispatch(self, action_name: str, payload: dict | None = None):
        ...
        **kwargs,
        ...
        if isinstance(ui_element, QtWidgets.QAction):
            ...
            chisurf.logging.warning(
                f"QtActionRouter: Unknown UI element type {type(ui_element)}. Using default 'triggered' if available."
            )
            signal = getattr(ui_element, "triggered", None)

        if signal is None:
            chisurf.logging.error(
                f"QtActionRouter: Could not find a default signal for {ui_element}"
            )
            return
            ...
            if payload is not None:
                if callable(payload):
                    try:
                        ...
                        chisurf.logging.error(
                            f"Error resolving dynamic payload for '{action_name}': {e}"
                        )
                        return
                else:
