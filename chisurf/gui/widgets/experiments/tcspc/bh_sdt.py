...
class TcspcSDTWidget(QtWidgets.QWidget):
    ...
    def rep_rate(self) -> float:
        ...
        """
        return 1.0 / (self._sdt.measure_info[self.curve_number]["rep_t"] * 1e-3)[0]
    ...
    def curve(self) -> chisurf.core.data.DataCurve:
        ...
        d = chisurf.core.data.DataCurve(setup=self, x=self.times, y=y, ey=ey, name=self.name)
        return d

    def onOpenFile(self, filename: str = None, curve_nbr: int = 0, *args, **kwargs):

        if filename is None:
            filename = chisurf.gui.widgets.get_filename("Open BH-SDT file", "SDT-files (*.sdt)")
