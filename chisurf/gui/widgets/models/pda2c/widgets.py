...
from chisurf.core.models.pda2c.anisotropy import (
    Pda2cAnisotropyModel,
    Pda2cAnisotropyNuisance,
    Pda2cAnisotropySpecies,
)
from chisurf.core.models.pda2c.nusiance import Background, Pda2cFretNuisance, Pda2cPhotonRange
from chisurf.core.models.pda2c.pdagauss import Pda2cGaussianDistanceModel, Pda2cGaussianDistances
from chisurf.core.models.pda2c.simple import Pda2cSimpleModel, ProbCh0
...
class Pda2cPhotonRangeWidget(QtWidgets.QGroupBox):
...
class Pda2cFretNuisanceWidget(QtWidgets.QGroupBox, Pda2cFretNuisance):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super().__init__(*args, **kwargs)
        try:
            Pda2cFretNuisance.__init__(self, **kwargs)
        except Exception:
            # If already initialized or kwargs not applicable, continue
            pass
        if hide_generic:
...
class Pda2cGaussianDistancesWidget(Pda2cGaussianDistances, QtWidgets.QWidget):

    def update(self, *__args):
        """Synchronize UI widgets with the underlying Gaussian component parameters."""
        Pda2cGaussianDistances.finalize(self)
        QtWidgets.QWidget.update(self, *__args)
        # Synchronize UI widgets with underlying parameters
        try:
    ...
    def append(self, mean: float = 50.0, sigma: float = 5.0, amplitude: float = 1.0):
        ...
        Pda2cGaussianDistances.append(self, mean=mean, sigma=sigma, amplitude=amplitude)
    ...
    def pop(self):
        ...
        if len(self._amplitudes) == 0:
            return
        Pda2cGaussianDistances.pop(self)
...
class Pda2cAnisotropySpeciesWidget(Pda2cAnisotropySpecies, QtWidgets.QWidget):

    def update(self, *__args):
        """Synchronize UI widgets with the underlying anisotropy species parameters."""
        # Keep amplitudes normalized etc.
        Pda2cAnisotropySpecies.finalize(self)
        QtWidgets.QWidget.update(self, *__args)
        try:
            for w, v in zip(self._amp_widgets, self.amplitudes):
                w.finalize()
    ...
    def append(self, amplitude: float = 1.0, r: float = 0.3):
        ...
        Pda2cAnisotropySpecies.append(self, amplitude=amplitude, r=r)
    ...
    def pop(self):
        ...
        if len(self._amplitudes) == 0:
            return
        Pda2cAnisotropySpecies.pop(self)
...
def get_distribution(fit, kw_hist):
    ...
    # :meth:`Pda2cGaussianDistanceModel._get_1d_residuals` so that the
...
def _get_gaussian_component_curves_for_pda(
    ...
    if not isinstance(model, Pda2cGaussianDistanceModel):
        return []

    distances = getattr(model, 'distances', None)
    if not isinstance(distances, Pda2cGaussianDistances):
        return []
...
class Pda2cSimpleModelWidget(ModelWidget, Pda2cSimpleModel):
    ...
    def __init__(
        ...
        **kwargs
        ...
        photon_nuisance = Pda2cPhotonRange(name='pda_photon_range', fit=fit, **kwargs)
        photon_range = Pda2cPhotonRangeWidget(nuisance=photon_nuisance)
...
class Pda2cAnisotropyModelWidget(ModelWidget, Pda2cAnisotropyModel):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # Pda2cAnisotropySpecies object is used for both GUI and backend,
        # mirroring the pattern used by the discrete and Gaussian PDA
        # models.
        species_widget = Pda2cAnisotropySpeciesWidget(fit=fit, name="pda_aniso_species")
...
class Pda2cGaussianDistanceModelWidget(ModelWidget, Pda2cGaussianDistanceModel):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # Pda2cGaussianDistanceModel (including creation of self.fret_parameters
        ...
        # GaussianModelWidget.
        nuisance = Pda2cFretNuisanceWidget(fit=fit, **kwargs)
        photon_range = Pda2cPhotonRangeWidget(nuisance=nuisance)
        distances = Pda2cGaussianDistancesWidget(fit=fit, **kwargs)
