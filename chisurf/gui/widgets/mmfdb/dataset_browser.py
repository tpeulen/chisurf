"""Reusable MMFDB dataset-picker widget and dialog.

Provides :class:`MmfdbDatasetBrowser` (standalone widget) and
:class:`MmfdbDatasetPickerDialog` (modal dialog with
:meth:`MmfdbDatasetPickerDialog.pick_dataset`) for selecting a dataset
registered in the MMFDB object store.

All MMFDB communication goes through an RPC client (duck-typed with a
...
class MmfdbDatasetBrowser(QtWidgets.QWidget):
    """Reusable widget for browsing MMFDB datasets.
...
class MmfdbDatasetPickerDialog(QtWidgets.QDialog):
    """Modal dialog wrapping :class:`MmfdbDatasetBrowser`.
    ...
    def __init__(
        ...
        super().__init__(parent)
        self.setWindowTitle("Select Dataset from MMFDB")
        self.resize(800, 450)
        self._selection: DatasetSelection | None = None

        layout = QtWidgets.QVBoxLayout(self)
        self.browser = MmfdbDatasetBrowser(
            client=client,
            kinds=kinds,
            formats=formats,
            parent=self,
    ...
    def pick_dataset(
        ...
        client : object, optional
            ...
            ``None`` immediately (MMFDB not connected).
        ...
        if client is None:
            return None
        dialog = MmfdbDatasetPickerDialog(
            client=client,
            kinds=kinds,
            formats=formats,
            scope=scope,
