...
class OptimizationSettingsModel:
    ...
    def __init__(self, settings: dict = None):
        ...
        for key, value in self._leastsq.items():
    ...
    def view_spec(self):
        ...
        optimiser = {
            ...
            "sections": [
                {"type": "value", "attr": f"leastsq_{key}",
                 "kind": "int" if isinstance(value, bool) is False and isinstance(value, int) else "float",
                 "label": key, "decimals": 10,
                 "description": f"scipy leastsq '{key}'."}
                for key, value in sorted(self._leastsq.items())
                if isinstance(value, (int, float)) and not isinstance(value, bool)
            ],
        }
        return load_view_spec({"sections": [sampler, optimiser]})
