...
def _plugin_icon_from_metadata(
    package_dir: Path, module_path: str, module_name: str, logger
) -> QIcon:
...
class CategoryMethodsMixin:
    ...
    def _setup_quick_access_bar(self):
        ...
        try:
            self.ribbon_bar.quickAccessToolBar()
        ...
        if toolbar_actions:
            ...
            for action in toolbar_actions:
                if action and not action.isSeparator():
                    ...
                    if action.icon() and (
                        action.text() in ["New", "Open", "Save", "Settings", "Quit"]
                    ):
        ...
        try:
            ...
            for _order, plugin_name, info in ordered:
                try:
                    ...
                    icon = _plugin_icon_from_metadata(
                        package_dir, module_path, module_name, self.logger
                    )
            ...
            if setup_plugins:
                ...
                if any(plugin["hierarchy_parts"] for plugin in setup_plugins):
                    ...
                    self._build_nested_plugin_structure(setup_plugins)
                    ...
                    self.logger.info(
                        f"Added {len(setup_plugins)} setup plugins to Main category (small button organization)"
                    )
            else:
        ...
        try:
            ...
            if help_plugins:
                ...
                if any(plugin["hierarchy_parts"] for plugin in help_plugins):
                    # Has hierarchy - use nested structure
                    self._build_nested_plugin_structure(help_plugins)
                    ...
                    for plugin_info in help_plugins:
                        ...
                        self.logger.info(
                            f"Added {len(help_plugins)} help plugins to Main category (small button organization)"
                        )
            else:
