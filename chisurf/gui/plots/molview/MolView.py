...
try:
    ...
    from qtpy.QtOpenGL import QGLFormat, QGLWidget  # type: ignore[attr-defined]
...
class MolQtWidget(QGLWidget):
    ...
    def __init__(self, parent, enableUi=True, File="", play=False, sequence=False):
        ...
        try:
            cs.logging.info(
                "MolQtWidget.__init__: PyMOL started (enableUi=%s, sequence=%s)", enableUi, sequence
            )
        except Exception:
            pass
    ...
    def openFile(self, File, frame=None, mode=None, verbose=True, object_name=None):
        if isinstance(File, str):
            if os.path.isfile(File):
                ...
                if verbose:
                    print(f"Pymol opening file: {File}")
    ...
    def paintGL(self):
        ...
        self.mapToGlobal(QtCore.QPoint(0, self.height())).y()
...
class ControlWidget(QtWidgets.QWidget):
    ...
    def __init__(self, parent=None):
        ...
        uic.loadUi(
            os.path.join(os.path.dirname(os.path.abspath(__file__)), "molViewControlWidget.ui"),
            self,
        )
    ...
    def onHighlightQuencher(self):
        ...
        for res_name in quencher:
            pymol.cmd.do(f"hide lines, resn {res_name}")
            pymol.cmd.do(f"show sticks, resn {res_name}")
            pymol.cmd.do(f"color red, resn {res_name}")
...
class MolView(Plot):
    ...
    def __init__(self, fit=None, enableUi=False, mode="cartoon", sequence=True, **kwargs):
        ...
        try:
            cs.logging.info(
                ...
                getattr(structure, "name", getattr(structure, "filename", "unknown"))
    ...
    def set_bfactor(self, bfact=None):
        ...
        if bfact is not None:
            ...
            counter = index_min
            for b in bfact:
                cmd.alter(f"p and resi {counter} and n. CA", f"b={b}")
                counter += 1
    ...
    def colorStates(self, rgbColors):
        ...
        for i, r in enumerate(rgbColors):
            color = f"[{r[0] / 255.0}, {r[1] / 255.0}, {r[2] / 255.0}]"
