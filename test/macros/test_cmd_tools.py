    ...
    def test_fcs_convert(self):
        ...
        for k in test_files:
            ...
            for dt in data_types:
                _, output_filename = tempfile.mkstemp(suffix=".tmp")
                ...
                args["reader_name"] = dt
                curve = chisurf.core.fio.fluorescence.fcs.read_fcs(**args)
                self.assertEqual(np.allclose(curve_ref.x, curve.x), True)
                self.assertEqual(np.allclose(curve_ref.y, curve.y), True)
