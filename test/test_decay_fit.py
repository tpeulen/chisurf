...
def test_recovers_two_lifetimes():
    ...
    truth = 0.6 * synthetic_decay(N, [1.2], bin_width=DT, normalize=True) + 0.4 * synthetic_decay(
        N, [4.0], bin_width=DT, normalize=True
    )
...
def test_with_irf():
    ...
    res = fit_lifetime_components(
        truth, bin_width=DT, n_components=1, irf=irf, tau_bounds=(0.5, 6.0)
    )
...
def test_fits_irf_fwhm():
    ...
    res = fit_lifetime_components(
        truth, bin_width=DT, n_components=1, fit_irf=True, irf_fwhm0=0.2, tau_bounds=(0.5, 6.0)
    )
...
def test_periodic_convolution_fits_wraparound_decay():
    ...
    truth = 60_000.0 * synthetic_decay(
        N, [1.2], bin_width=DT, irf=irf, normalize=True, period=period
    ) + 40_000.0 * synthetic_decay(N, [4.0], bin_width=DT, irf=irf, normalize=True, period=period)
    common = dict(
        bin_width=DT,
        n_components=2,
        fit_irf=True,
...
def test_fits_scatter_and_background_fractions():
    ...
    bare = fit_lifetime_components(
        truth, bin_width=DT, n_components=1, fit_irf=True, irf_fwhm0=0.2, tau_bounds=(0.5, 6.0)
    )
    full = fit_lifetime_components(
        truth,
        bin_width=DT,
        n_components=1,
        fit_irf=True,
...
def test_fits_irf_width_shift_and_skew_jointly():
    ...
    truth = 60_000.0 * synthetic_decay(
        N, [1.2], bin_width=DT, irf=irf, normalize=True
