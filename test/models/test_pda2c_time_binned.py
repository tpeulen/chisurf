...
def _dynamic_fit(observation_time, rate, total=3e5, seed=1, states=(40.0, 62.0, 0.5)):
    ...
    fit = fit_mod.Fit(model_class=Pda2cDynamicTwoStateModel, data=_pda_data(observation_time))
    model = fit.model
    r1, r2, x1 = states
...
def test_the_model_takes_its_observation_time_from_the_dataset(qapp):
    ...
    for window in (1e-3, 4e-3):
        fit = fit_mod.Fit(model_class=Pda2cDynamicTwoStateModel, data=_pda_data(window))
...
def test_data_without_an_observation_time_falls_back_to_the_window_bound(qapp):
    ...
    fit = fit_mod.Fit(model_class=Pda2cDynamicTwoStateModel, data=data)
...
def _n_state_fit(n_states=3):
    ...
    fit = fit_mod.Fit(model_class=Pda2cDynamicNStateModel, data=_pda_data(2e-3))
