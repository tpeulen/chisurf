...
def run_comprehensive_benchmark():
    ...
    common_actions = [
        ...
        (
            "parameter.scan",
            {"parameter_name": "tau1", "fit_index": 0, "scan_range": (1.0, 5.0), "n_steps": 10},
        ),
    ]
    ...
    for action_name, payload in common_actions:
        try:
            ...
            print(
                f"{action_name:25} | {benchmark_result['avg_execution_time_ms']:6.2f} ms | {benchmark_result['memory_used_bytes']:8} bytes | {benchmark_result['executions_per_second']:6.1f} ops/sec"
            )
        except Exception as e:
    ...
    for ops in [100, 500, 1000]:
        try:
            ...
            print(
                f"{ops:4} operations | {history_result['total_time_seconds']:6.2f} s | {history_result['operations_per_second']:8.1f} ops/sec | {history_result['avg_time_per_operation_ms']:6.2f} ms/op | {history_result['success_rate']:5.1f}% success"
            )
        except Exception as e:
