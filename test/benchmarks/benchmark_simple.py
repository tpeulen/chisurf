...
def benchmark_action_controller():
    ...
    try:
        ...
        if registry:
            ...
            print(
                f"Registry operations      | {registry_time:6.3f} ms | {len(action_list)} actions | {len(catalog)} catalog entries"
            )
        else:
    ...
    print("=== Summary ===")
    active_actions = sum(
        1 for v in results.values() if isinstance(v, dict) and v.get("handler_exists")
    )
    total_actions = len(
        [v for v in results.values() if isinstance(v, dict) and "handler_exists" in v]
    )
