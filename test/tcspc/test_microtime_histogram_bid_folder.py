...
try:
    ...
    bid_dir_check = (
        "if len(bid_files) > 0:" in source_code
        and "bid_directory = Path(bid_files[0]).parent" in source_code
    )
    ...
    if bid_check and bid_dir_check and log_check and priority_check:
        ...
        print(
            "3. Fall back to TTTR folder or selected file directory if no BID/BUR files are present"
        )
    else:
        print(
