...
def test_da_green_is_quenched_relative_to_donor_only():
    donor = [1.0, 4.0]
    d_only = fret_species_patterns(
        N, _species(state="d_only", donor_spectrum=donor), dt=DT, normalize=False
    )
    da = fret_species_patterns(
        N,
        _species(state="da", donor_spectrum=donor, transfer_efficiency=0.6),
        dt=DT,
        normalize=False,
    )
...
def test_da_red_has_sensitized_rise():
    ...
    sp = _species(
        ...
        crosstalk=CrosstalkFactors(gamma=1.0),
    )
...
def test_efficiency_vs_distance_modes_agree_at_matched_e():
    ...
    dist = fret_species_patterns(
        ...
        _species(
            ...
            fret_mode="distance",
            forster_radius=52.0,
            distance_rows=[{"mean": 52.0, "sigma": 0.0, "amplitude": 1.0}],
        ),
        dt=DT,
        normalize=False,
    )
    eff = fret_species_patterns(
        N,
        _species(state="da", donor_spectrum=donor, fret_mode="efficiency", transfer_efficiency=0.5),
        dt=DT,
        normalize=False,
    )
    t = np.arange(N) * DT
...
def test_distributed_distance_broadens_donor_decay():
    ...
    from chisurf.core.fluorescence.fret.species_decay import _resolve_fret, _spectrum_pairs
...
def test_gamma_scales_red_relative_to_green():
    ...
    lo = fret_species_patterns(
        N, _species(state="da", crosstalk=CrosstalkFactors(gamma=0.5)), dt=DT, normalize=False
    )
    hi = fret_species_patterns(
        N, _species(state="da", crosstalk=CrosstalkFactors(gamma=2.0)), dt=DT, normalize=False
    )
...
def test_polarized_keys_and_anisotropy_ordering():
    sp = _species(
        state="a_only", anisotropy=Anisotropy(acceptor_r0=0.4, acceptor_rho=2.0, r_inf=0.0)
    )
    pol = fret_species_patterns(N, sp, dt=DT, polarized=True, normalize=False)
    ...
    sp = fret_species_from_dict(
        {
            ...
            "distance": 45.0,
            "forster_radius": 52.0,
            "donor_spectrum": [1.0, 3.8],
            "acceptor_spectrum": [1.0, 1.9],
...
def test_anisotropy_spectrum_multi_component():
    ...
    multi = Anisotropy(
        donor_spectrum=[{"amplitude": 0.2, "rho": 0.5}, {"amplitude": 0.2, "rho": 8.0}], r_inf=0.02
    )
