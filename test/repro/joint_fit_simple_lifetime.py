...
def main():
    ...
    fit_donor = chisurf.core.fitting.fit.FitGroup(
        data=decay_donor_only, model_class=chisurf.core.models.description.tcspc_lifetime
    )
    ...
    fit_fret = chisurf.core.fitting.fit.FitGroup(
        data=decay_fret, model_class=chisurf.core.models.description.tcspc_lifetime
    )
    ...
    model_fret.parameter_dict["tL1"].value  # Same as donor due to linking

    # For a simple estimate, we could compare amplitudes or add a second component
    print(f"\nDonor lifetime: {donor_lifetime:.3f} ns")
    print("Note: This simple example links the lifetimes. For actual FRET analysis,")
    print("you would need to use the GaussianModel or create a more complex model.")
