...
class Parameter:
    ...
    def __post_init__(self) -> None:
        if self.bounds_on and self.lb > self.ub:
            raise ValueError(
                f"Invalid bounds for {self.name or 'parameter'}: lb={self.lb} > ub={self.ub}"
            )
...
class GaussianMixtureHistogramFitter:
    ...
    def pdf(self, x_vals: np.ndarray) -> np.ndarray:
        ...
        for w, c in zip(weights, self.components):
            pdf_total += float(w) * norm.pdf(
                x_vals, loc=float(c.mu.value), scale=float(c.sigma.value)
            )
        return pdf_total
    ...
    def component_pdfs(self, x_vals: np.ndarray) -> list[np.ndarray]:
        ...
        weights = np.array([c.population.value for c in self.components], dtype=float)
        return [
            float(w) * norm.pdf(x_vals, loc=float(c.mu.value), scale=float(c.sigma.value))
            for w, c in zip(weights, self.components)
        ]
    ...
    def fit_to_histogram(
        ...
        def apply_x(xvec: np.ndarray) -> None:
            ...
            if free_pop_components:
                ...
                fixed_sum = float(
                    sum(float(c.population.value) for c in self.components if c.population.fixed)
                )
...
print("  Using tttrlib.TTTR.burst_search() with:")
...
def process_tttr_file(tttr_file, config):
    ...
    try:
        ...
        if filter_mode == "count_rate":
            print(
                f"  Applying count rate filter (n_ph_max={config['PHOTON_THRESHOLD']}, "
        ...
        if config["USE_GAP_FILL"] and config["MAX_GAP"] > 0:
            ...
            print(
                f"  After gap filling (max_gap={config['MAX_GAP']}): {np.sum(selection):,} photons"
            )
        ...
        print(
            f"  Burst size: min={burst_sizes.min()}, max={burst_sizes.max()}, "
...
def generate_burst_dataframe(
    start_stop, filename, tttr, windows, detectors, include_interleaved_zeros=True
):
    """
...
def process_tttr_file(tttr_file, config):
    ...
    try:
        ...
        if filter_mode == "count_rate":
            print(
                f"  Applying count rate filter (n_ph_max={config['MIN_PH']}, "
                f"window={config['COUNT_RATE_WINDOW_MS']}ms)..."
            )
            filter_mask = count_rate_filter(
                tttr,
        ...
        # Use tttrlib's burst_search method
        print("  Running tttrlib.TTTR.burst_search():")
...
def make_fret_panels(
    all_dataframes,
    output_path,
    gaussians: list[GaussianComponent] | None = None,
    n_gaussians=3,
    bins_prox=50,
    bins_dt=50,
    bins_rate=50,
    ...
    # ------------------------------------------------------------------
    if (
        "Number of Photons (green)" in combined_df.columns
        and "Number of Photons (red)" in combined_df.columns
    ):
    ...
    for i in range(n_components):
        print(
            f"  Component {i + 1}: weight={weights[i]:.4f}, mean={means[i]:.4f}, sigma={sigmas[i]:.4f}"
        )
    ...
    hist_counts, bin_edges = np.histogram(
        prox_valid.to_numpy(), bins=bins_x, range=(prox_min, prox_max)
    )
    bin_centers = 0.5 * (bin_edges[:-1] + bin_edges[1:])
    ...
    # ------------------------------------------------------------------
    has_dt = (
        "Duration (green) (ms)" in combined_df.columns
        and "Duration (red) (ms)" in combined_df.columns
    )
    has_rates = (
        "Green Count Rate (KHz)" in combined_df.columns
        and "Red Count Rate (KHz)" in combined_df.columns
    )
...
if len(hdf5_files) > 0:
    ...
    print("=" * 60)
    print("✓ Data type: ALEX (separate routing channels)")
...
print(
    f"Trace length: {len(intensity_trace_green)} bins ({len(intensity_trace_green) * bin_width_ms:.1f} ms)"
)
...
gs = fig.add_gridspec(
    3, 3, hspace=0.05, wspace=0.05, height_ratios=[1, 4, 0.2], width_ratios=[4, 1, 0.2]
)
...
if (
    ...
    combined_df["Proximity Ratio"] = combined_df.apply(
        lambda row: (
            row["Number of Photons (red)"]
            / (row["Number of Photons (red)"] + row["Number of Photons (green)"])
            if (row["Number of Photons (red)"] + row["Number of Photons (green)"]) > 0
            else 0
        ),
        axis=1,
    )
