...
parameter network -- are handled by :mod:`chinet.graph`, the in-tree graph layer
...
def _python_sources():
    """Yield every Python source file under ``chisurf/``, ``modules/chinet/`` and ``test/``.

    Yields
    ------
    pathlib.Path
    """
    for folder in ("chisurf", "modules/chinet", "test"):
...
def test_no_module_imports_networkx():
    ...
    assert not offenders, (
        "networkx is no longer a dependency; use `from chinet import graph as cg` "
