...
def test_fit_save_endpoint(server_client):
    ...
    with tempfile.NamedTemporaryFile(suffix=".csv", delete=False) as f:
        tmp = f.name
    try:
        result = client.fit__save(
            fit_index=ft["fit_index"], filename=tmp, file_type="csv", save_curves=True
        )
        assert result.get("ok") is True
    finally:
...
def test_fit_set_dataset_endpoint(server_client):
    ...
    ds2 = client.call(
        ...
        {
            "reader_name": "SDS2",
            "filename": "/tmp/sds2.dat",
            "name": "Data2",
            "curve_data": {"x": [0.0], "y": [2.0]},
        },
    )
...
def test_fit_set_result_idx_endpoint(server_client):
    ...
    ds = client.call(
        "dataset.load",
        {
            "reader_name": "SRIReader",
            "filename": "/tmp/sri.dat",
            "name": "ResultIdxTest",
            "curve_data": {"x": _DECAY_X, "y": _DECAY_Y},
        },
    )
    assert ds.get("ok") is True
    ft = _create_tcspc_fit(client, ds["dataset_index"])
...
def test_event_broadcast_on_dataset_add(server_client):
    ...
    result = client.call(
        "dataset.load",
        {
            "reader_name": "EvtReader",
            "filename": "/tmp/evt.dat",
            "name": "EventTest",
            "curve_data": {"x": [0.0], "y": [1.0]},
        },
    )
...
def test_event_broadcast_on_dataset_remove(server_client):
    ...
    client.subscribe(topic="dataset.removed", callback=_on_event)
    import time
...
def test_parameter_link_endpoint(server_client):
    ...
    ds = client.call(
        "dataset.load",
        {
            "reader_name": "LinkReader",
            "filename": "/tmp/link.dat",
            "name": "LinkTest",
            "curve_data": {"x": [0.0], "y": [1.0]},
        },
    )
...
def test_event_broadcast_on_fit_run(server_client):
    ...
    client.subscribe(topic="fit.ran", callback=_on_event)
    import time
