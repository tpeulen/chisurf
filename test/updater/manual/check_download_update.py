...
def main() -> None:
    """Download (but do not install) the latest update, logging progress.

    A manual diagnostic script, not a pytest test: it makes a real network
    call against the live update server. Guarded behind __name__ == "__main__"
    so pytest's --doctest-modules collection (which imports every .py file
    under test/ looking for doctests) does not execute it merely by
    importing this module -- that hung Linux/macOS CI collection for 14+
    minutes with no per-test timeout able to catch it, since the hang was in
    collection, before any test's timeout wrapper is active.
    """
