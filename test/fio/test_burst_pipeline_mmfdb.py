...
def _result(input_path: Path, *, bur_path: Path | None = None) -> AnalysisResult:
    ...
    return AnalysisResult(
        ...
        dataframes={
            str(input_path): [{"First Photon": 0, "Last Photon": 10, "Number of Photons": 11}]
        },
...
def test_registers_raw_input_when_source_missing(db, tmp_path: Path) -> None:
    ...
    registration = BurstMMFDBPipeline(db, session=db.test_session).register_run(
        _request(input_path), _result(input_path, bur_path=bur_path)
    )
...
def test_reuses_source_artifact_without_duplicate_raw_input(db, tmp_path: Path) -> None:
    ...
    input_path = _write_input(tmp_path)
    bur_path = _write_bur(tmp_path)
    source_id = register_raw_measurement(str(input_path), db=db, session=db.test_session)
    request = _request(input_path)
    request.mmfdb.source_artifact_ids = {str(input_path.resolve()): source_id}

    registration = BurstMMFDBPipeline(db, session=db.test_session).register_run(
        request, _result(input_path, bur_path=bur_path)
    )
...
def test_registers_native_bur_table_from_rows_when_no_bur_path(db, tmp_path: Path) -> None:
    ...
    assert payload.decode("utf-8") == ("First Photon\tLast Photon\tNumber of Photons\n0\t10\t11\n")
...
def test_burst_table_operation_links_selected_setup(db, tmp_path: Path) -> None:
    ...
    registration = BurstMMFDBPipeline(db, session=db.test_session).register_run(
        _request(input_path, setup_id=setup_id), _result(input_path)
    )
    conn = getattr(db, "conn")
    row = conn.execute(
...
def test_enabled_archival_without_authenticated_session_writes_nothing(db, tmp_path: Path) -> None:
    """An explicit database is insufficient: archival needs a verified caller."""
    input_path = _write_input(tmp_path)

    registration = BurstMMFDBPipeline(db=db).register_run(_request(input_path), _result(input_path))

    assert registration.input_artifacts == {}
    assert registration.burst_table_artifacts == {}
    assert registration.warnings == ["MMFDB archival refused: Authenticated MMFDB session required"]
    assert db.conn.execute("SELECT COUNT(*) FROM mmfdb_artifact").fetchone()[0] == 0


def test_enabled_archival_rejects_mismatched_session_identity(db, tmp_path: Path) -> None:
    ...
    assert registration.input_artifacts == {}
    assert registration.warnings == ["MMFDB archival refused: MMFDB session identity mismatch"]
    assert db.conn.execute("SELECT COUNT(*) FROM mmfdb_artifact").fetchone()[0] == 0
...
def test_nonexistent_setup_emits_warning_and_leaves_setup_id_null(db, tmp_path: Path) -> None:
