...
def test_store_chinet_session_writes_session_node_parameter_and_dependency_rows(tmp_path) -> None:
    ...
    with MFDatabase(db_path) as db:
        ...
        artifacts = db.conn.execute(
            ...
            "FROM mmfdb_artifact ORDER BY artifact_id"
        ).fetchall()
        ...
        edges = db.conn.execute(
            ...
            "FROM mmfdb_edge WHERE relationship_type = 'parameter_depends_on'"
        ).fetchall()
        assert len(edges) == 1
...
def test_connect_to_db_can_use_mmfdb_backend(tmp_path) -> None:
    db_path = tmp_path / "chinet_mmfdb_backend.db"
    session = _connected_session()
    source_out = session.nodes["source"].outputs["out"]
    follower_in = session.nodes["follower"].inputs["in"]
    ...
    try:
        ...
        clear_mmfdb_backend(close=True)
