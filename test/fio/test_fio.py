...
def test_ascii_save_load(tmp_path):
    ...
    chisurf.core.fio.ascii.save_xy(
        filename=str(filename), x=x, y=y, fmt="%f\t%f\n", header_string="x\ty\n"
    )

    x2, y2 = chisurf.core.fio.ascii.load_xy(
        filename=str(filename), usecols=(0, 1), delimiter="\t", skiprows=1
    )
...
def test_csv_class(tmp_path):
    ...
    reference_data = np.vstack(
        [reference_x, reference_y, np.zeros_like(reference_x), np.ones_like(reference_y)]
    ).T
    ...
    chisurf.core.fio.ascii.save_xy(
        filename=filename, x=reference_x, y=reference_y, fmt="%f\t%f\n", header_string="x\ty\n"
    )

    # CSV class
    csv = chisurf.core.fio.ascii.Csv(filename=filename, skiprows=0, use_header=True)
...
def test_parse_string_pdb():
    ...
    atoms_reference = np.array([[7.71, 28.561, 39.546], [8.253, 29.664, 38.758]])
...
def test_photons(d):
    ...
    photons = chisurf.core.fio.fluorescence.photons.Photons(
        d["files"], reading_routine=d["routine"]
    )
...
def test_read_tcspc_csv_vv_vh(polarization, expected_ref):
    ...
    ref_vv_slice = np.array([1.0, 4.0, 2.0, 10.0, 6.0, 14.0, 13.0, 15.0, 8.0])
    ref_vh_slice = np.array([3.0, 5.0, 6.0, 10.0, 11.0, 10.0, 8.0, 15.0])

    decay_data_curve = chisurf.core.fio.fluorescence.tcspc.read_tcspc_csv(
        filename=filename, skiprows=0, dt=dt, is_vv_vh=True, polarization=polarization
    )
...
def test_read_tcspc_csv_vv_vh_complex():
    ...
    decay_data_curve_vm = chisurf.core.fio.fluorescence.tcspc.read_tcspc_csv(
        filename=filename, skiprows=0, dt=dt, is_vv_vh=True, polarization="vm", g_factor=g_factor
    )
...
def test_read_fcs_kristine():
    ...
    fcs_data_curve = chisurf.core.fio.fluorescence.fcs.read_fcs(
        reader_name="kristine", filename=filename
    )
...
def test_read_nonexistent_photon_file():
    ...
    try:
        chisurf.core.fio.fluorescence.photons.Photons(
            "non_existent_file.spc", reading_routine="bh132"
        )
...
def test_read_nonexistent_fcs():
    with pytest.raises((FileNotFoundError, IOError, AttributeError)):
        ...
        chisurf.core.fio.fluorescence.fcs.read_fcs(
            filename="non_existent_file.cor", reader_name="kristine"
        )
