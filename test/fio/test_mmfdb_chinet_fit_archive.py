...
class DummyLinearModelForMMFDB(ModelCurve):
    """Small concrete model used for MMFDB fit archive tests."""

    name = "DummyLinearModelForMMFDB"
...
def _make_dummy_fit() -> Fit:
    ...
    return Fit(model_class=DummyLinearModelForMMFDB, data=data)
...
def test_archive_fit_to_mmfdb_does_not_mutate_live_parameter_ports(tmp_path) -> None:
    fit = _make_dummy_fit()
...
def test_archive_fit_to_mmfdb_stores_fit_state_chinet_session_and_parameters(tmp_path) -> None:
    fit = _make_dummy_fit()
    params = fit.model.parameters_all_dict
    params["p0"].value = 2.0
    ...
    with MFDatabase(db_path) as db:
        ...
        op_links = db.conn.execute(
            ...
            "FROM mmfdb_operation_artifact WHERE operation_id = ?",
            ("op_fit",),
        ).fetchall()
        ...
        edges = db.conn.execute(
            "SELECT source_node_id, target_node_id, relationship_type FROM mmfdb_edge "
...
def test_archive_fit_to_mmfdb_rolls_back_after_fit_state_write_failure(
