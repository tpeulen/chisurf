...
def _reference_shape(brightness_b=None, power=20e-6, include_bunching=True):
    ...
    if k0 <= 0.0:
        g = 1.0 / (1.0 + 4.0 * KW["D"] * TAU / KW["w0"] ** 2) / np.sqrt(
            1.0 + 4.0 * KW["D"] * TAU / KW["z0"] ** 2)
    else:
        ...
        profile_b = (None if brightness_b is None else
                     sat.emission_profile(k_exc, DARK, EXC, brightness_b))
        v0 = np.pi ** 1.5 * KW["w0"] ** 2 * KW["z0"]
        g = sat.fcs_numerical_g_diff(TAU, r, z, profile, KW["D"], v_ref=v0,
                                     profile_b=profile_b)
    if include_bunching:
        # The engine's own bunching (already pinned below) closes the loop.
        g = g * sat.compute_bunching_factor(k0, DARK, EXC, Q, TAU,
...
def test_the_zero_power_limit_is_the_gaussian():
    got = sat.saturated_curve_shape(TAU, 0.0, 73000.0, DARK, EXC, Q,
                                    include_bunching=False, **KW)
    want = 1.0 / (1.0 + 4.0 * KW["D"] * TAU / KW["w0"] ** 2) / np.sqrt(
        1.0 + 4.0 * KW["D"] * TAU / KW["z0"] ** 2)
...
def test_bunching_handles_complex_relaxation_modes():
    ...
    K_eq = K.copy(); K_eq[-1, :] = 1.0
    b = np.zeros(3); b[-1] = 1.0
    p_eq = np.linalg.solve(K_eq, b)
    p_eq = np.maximum(p_eq, 0.0); p_eq /= p_eq.sum()
    norm = float(q @ p_eq) ** 2
    ...
    got = np.asarray(bff.fcs_bunching_factor(
        [float(v) for v in TAU], 0.0, [float(v) for v in cyc.ravel()],
        [0.0] * 9, 3, [float(v) for v in q]))
