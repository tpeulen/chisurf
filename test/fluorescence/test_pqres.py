...
def test_pqres_fcs_keeps_the_measured_amplitudes():
    ...
    for name, curve in curves.items():
        np.testing.assert_array_equal(curve["correlation_amplitudes"], np.asarray(raw[name]["Y"]))
        np.testing.assert_array_equal(curve["correlation_times"], np.asarray(raw[name]["X"]))
