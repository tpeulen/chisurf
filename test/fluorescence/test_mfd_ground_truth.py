...
def test_polarized_mode_emits_the_four_mfd_detectors():
    ...
    found = {name: int(np.count_nonzero(sim.stream == ch)) for name, ch in MFD_STREAMS.items()}
...
def test_the_polarization_split_follows_perrin():
    ...
    for rho in (0.5, 4.0):
        ...
        expected = perrin_steady_state_anisotropy(tau, rho, r0=sim.parameters.r0_fundamental)
...
def test_the_irf_shifts_the_mean_micro_time_by_its_centre():
    ...
    def mean_micro(**changes):
        ...
        return float(micro[np.isin(sim.stream, green)].mean() * sim.parameters.microtime_resolution)
...
def test_truth_bursts_are_single_molecule_transits():
    ...
    distinct = [len(set(molecule[a : b + 1][species[a : b + 1] >= 0].tolist())) for a, b in bursts]
...
def test_the_written_folder_reproduces_the_emitted_photons(tmp_path, definition):
    ...
    sim = simulate_smfret(**MFD)
    spans = (
        sim.true_bursts(min_photons=20)
        if definition == "truth"
...
def test_photons_do_not_sample_a_burst_uniformly_in_time(rate):
    ...
    *time* spent in each. A molecule is brightest at the centre of its transit,
    ...
    sim = simulate_smfret(**{**MFD, "n_photons": 300_000, "donor_only": 0.05}, rate_matrix=matrix)
...
def test_the_photon_weighted_window_recovers_the_generating_rate():
    ...
    folder = sim.write_folder(
        pathlib.Path(tempfile.mkdtemp()) / "rate", bursts="truth", stem="rate"
    )
    data = load_mfd_data(folder, min_green_photons=20)

    optics = Optics(r0=52.0, tau_d0=4.0, sigma=6.0, gamma=1.0, alpha=0.0, delta=0.0)
    states = [FretState(distance=d, name=n) for d, n in zip((66.2, 39.3), ("low", "high"))]
    ...
    def recover():
        ...
        return float(
            np.exp(
                minimize_scalar(
                    objective,
                    bounds=(np.log(200.0), np.log(40000.0)),
                    method="bounded",
                    options={"xatol": 5e-3},
                ).x
            )
...
def test_exchange_builds_a_dynamic_bridge_off_the_static_line():
    ...
    * the bursts there sit **above** the chord joining the two populations. A
    ...
    axes = HistogramAxes.default(n_ratio=48, n_micro_time=48, micro_time_range=(0.5, 6.5))
    ...
    for rate in (0.0, 1000.0):
        ...
        folder = sim.write_folder(root / f"r{int(rate)}", bursts="truth", stem=f"r{int(rate)}")
        ...
        if rate:
            mean = np.where(
                column > 0, (counts * micro[None, :]).sum(axis=1) / np.maximum(column, 1), np.nan
            )
            ...
            chord = ends[0] + (ratio - span[0]) * (ends[1] - ends[0]) / (span[1] - span[0])
