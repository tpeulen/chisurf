...
def dictionary():
    mmfdb = pytest.importorskip(
        "mmfdb.schema.pdbx_metadata",
        reason="mmfdb (modules/mmfdb/src) not on the path")
    return mmfdb.MmcifDictionary.load_bundled()


@pytest.mark.parametrize("path", yaml_files, ids=[p.name + ":" + p.parent.parent.name for p in yaml_files])
