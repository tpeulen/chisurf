...
def test_fresh_db_gets_full_schema():
    ...
    # Fresh creation is not reported as an upgrade from a prior schema.
...
def test_canonical_and_permissive_ddl_differ_only_by_check():
    ...
    for tn in schema._CANONICAL_TABLE_NAMES:
        ...
        assert "CHECK" in canonical or tn in ("mmfdb_object", "mmfdb_parameter"), (
...
def test_new_migration_step_applies_on_bump():
    ...
    orig_migrations = schema.MIGRATIONS.copy()
    next_version = orig_version + 1

    try:
        dummy_called = False

        def _dummy_migration(_conn):
            nonlocal dummy_called
            dummy_called = True

        # Create a current DB, then simulate one future migration.
        with tempfile.NamedTemporaryFile(suffix=".db", delete=False) as f:
            db_path = f.name
        try:
            conn = sqlite3.connect(db_path)
            schema.migrate_schema(conn)
            assert schema.get_schema_version(conn) == orig_version
            conn.close()

            schema.SCHEMA_VERSION = next_version
            schema.MIGRATIONS[next_version] = _dummy_migration

            conn2 = sqlite3.connect(db_path)
            report = schema.migrate_schema(conn2)
            assert report is not None
            assert report.to_version == next_version
            assert dummy_called, "future migration was not called"
            assert schema.get_schema_version(conn2) == next_version
            conn2.close()
        finally:
            os.unlink(db_path)
    finally:
        ...
        schema.MIGRATIONS.update(orig_migrations)
