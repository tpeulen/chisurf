...
def test_an_unmeasurable_pair_is_nan_in_the_matrix_not_zero():
    ...
    rng = np.random.default_rng(0)
    n = 400
    draws = np.column_stack([rng.normal(size=n), np.full(n, 2.5), rng.normal(size=n)])
...
def _fit_with_chain(draws, converged=True):
    ...
    fit.sampling_diagnostics = {
        "parameters": [{"name": nm, "rhat": rhat, "ess": ess} for nm in names]
    }
    return fit
...
def _banana_draws(n=4000, seed=2):
    ...
    t = rng.normal(size=n)
    return np.column_stack([t, t**2 + 0.1 * rng.normal(size=n), rng.normal(size=n)])
...
def test_the_view_says_nothing_extra_about_a_gaussian_posterior():
    ...
    n = 4000
    a = rng.normal(size=n)
    draws = np.column_stack([a, 0.8 * a + 0.6 * rng.normal(size=n), rng.normal(size=n)])
...
def test_prefixed_group_names_still_line_up_with_the_chain():
    ...
    fit.sampling_diagnostics = {
        "parameters": [{"name": f"1:{nm}", "rhat": 1.001, "ess": 2000.0} for nm in plain]
    }
