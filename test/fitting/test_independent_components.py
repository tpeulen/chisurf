...
def test_merged_marginals_match_a_joint_run():
    ...
    def _summary(sampler):
        ...
        return {
            e['name']: e for e in
            dg.summarize(np.asarray(r['chains']), names=r['parameter_names'])
        }

    decomposed = _summary(lambda f, m: chisurf.core.fitting.sample.
                          sample_independent_components(
                              fit=f, steps=4000, step_size=0.02, thin=1,
                              model=m, seed=5))
    joint = _summary(lambda f, m: chisurf.core.fitting.sample.walk_mcmc_blocked(
...
def test_sample_fit_can_target_the_group_joint_posterior(tmp_path, monkeypatch):
    ...
    report = chisurf.core.fitting.fit.sample_fit(
        ...
        steps=1500, thin=1, n_runs=2, global_posterior=True,
    )
    joint_model = posterior_model(fit)
