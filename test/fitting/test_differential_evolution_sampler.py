...
def test_it_beats_the_covariance_proposal_away_from_the_optimum():
    ...
    de = _ess_per_eval(
        lambda f: chisurf.core.fitting.sample.sample_differential_evolution(
...
def test_it_is_competitive_on_a_collinear_posterior():
    ...
    stretch_ess = _ess_per_eval(
        lambda f: chisurf.core.fitting.sample.sample_ensemble(f, steps=800, nwalkers=10, thin=1)
    )
    de = _ess_per_eval(
        lambda f: chisurf.core.fitting.sample.sample_differential_evolution(
...
def test_it_restores_the_fit_afterwards():
    ...
    chisurf.core.fitting.sample.sample_differential_evolution(fit=fit, steps=200, thin=1, seed=6)
