...
class Tests(unittest.TestCase):
    def test_get_instances(self):
        ...
        self.assertEqual(len(list(p1.get_instances())), initial_instances + 1)
    ...
    def test_arithmetics(self):
        ...
        p3 = p1 / p2
        self.assertEqual(p3.value, 2.0 / 3.0)
    ...
    def test_bounds(self):
        p1 = chisurf.core.parameter.Parameter(value=2.0, bounds_on=True, lb=1.0, ub=2.5)
        self.assertEqual(p1.value, 2.0)
        p1.value = 5.0
    ...
    def test_rep_str(self):
        ...
        self.assertEqual(p1.__repr__(), "22")
    ...
    def test_dict(self):
        ...
        self.assertEqual(p1.to_dict(), d3)
    ...
    def test_save_load(self):
        ...
        p1 = chisurf.core.parameter.Parameter(value=2.0, bounds_on=True, lb=1.0, ub=2.5)
        p1.save(filename, file_type="json")

        p2 = chisurf.core.parameter.Parameter()
        p2.load(filename=filename, file_type="json")

    def test_parameter_group_append(self):
        p1 = chisurf.core.parameter.Parameter(value=22, name="p1")
        p2 = chisurf.core.parameter.Parameter(value=11, name="p2")
        group_name = "Parameter Gruppe"
        pg = chisurf.core.parameter.ParameterGroup(name=group_name)
        self.assertEqual(pg.name, group_name)
        pg.append(p1)
        pg.append(p2)
        self.assertEqual(pg.values, [22, 11])
        self.assertListEqual(pg.parameter_names, ["p1", "p2"])
    ...
    def test_fitting_parameter(self):
        ...
        p3.from_dict(p2.to_dict())
        self.assertEqual(p3.link, p1)

        self.assertEqual(p2.fixed, fixed)
    ...
    def test_abs(self):
        ...
        p1 = chisurf.core.fitting.parameter.FittingParameter(value=value)
        p2 = abs(p1)
        self.assertEqual(abs(p1.value), p2.value)

    def test_parameter_get_set_state_roundtrip(self):
        """Parameter.get_state/set_state should round-trip basic port state."""
        p1 = chisurf.core.parameter.Parameter(value=2.0, bounds_on=True, lb=1.0, ub=3.0)
        s = p1.get_state()
    ...
    def test_parameter_group(self):
        ...
        class A(chisurf.core.parameter.ParameterGroup):
            def __init__(self):
                self.pv = chisurf.core.parameter.Parameter(value=11)
        ...
        #  value attribute
        a = A()
        self.assertEqual(a.pv.value, 11)
        a.pv = 22
        self.assertEqual(a.pv.value, 22)
