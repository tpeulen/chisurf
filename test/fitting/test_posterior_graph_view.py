...
def test_datasets_are_numbered_like_the_parameters_that_belong_to_them():
    ...
    assert not any(n.label.startswith("L") and n.label[1:].isdigit() for n in datasets), [
        n.label for n in datasets
    ]
...
def test_no_edge_is_drawn_through_a_node():
    ...
    for edge in view.edges:
        ...
        for node in view.nodes:
            ...
            assert distance > 0.08, f"edge {edge.source}->{edge.target} passes through {node.label}"
...
def test_strongly_correlated_parameters_are_placed_close_together():
    ...
    pairs = [(e, float(np.linalg.norm(pos[e.source] - pos[e.target]))) for e in view.edges]
