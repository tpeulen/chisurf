...
def test_gaussian_distances():
    view = _view(
        "tcspc_fret_gaussian",
        "tcspc_fret_gaussian.components.2",
        {
            "distance.mean.0": 41.0,
            "distance.sigma.0": 5.0,
            "distance.shape.0": 0.0,
            "distance.amplitude.0": 0.6,
            "distance.mean.1": 62.0,
            "distance.sigma.1": 9.0,
            "distance.shape.1": 0.0,
            "distance.amplitude.1": 0.4,
        },
    )
...
def test_discrete_distances():
    view = _view(
        "tcspc_fret_discrete",
        "tcspc_fret_discrete.components.2",
        {
            "distance.mean.0": 45.0,
            "distance.amplitude.0": 0.7,
            "distance.mean.1": 60.0,
            "distance.amplitude.1": 0.3,
        },
    )
...
def test_worm_like_chain(linker):
    view = _view(
        "tcspc_fret_worm_like_chain",
        None,
        {
            "chain.contour_length": 120.0,
            "chain.persistence_length": 25.0,
            "chain.linker_width": 5.0,
        },
...
def test_ising_chain():
    view = _view(
        "tcspc_fret_ising_chain",
        None,
        {
            "chain.n_residues": 30.0,
            "chain.b_structured": 3.5,
            "chain.b_unstructured": 7.0,
            "chain.coupling": 1.2,
            "chain.field": 0.3,
        },
    )
...
def test_a_polarized_fret_decay(polarization, code):
    ...
    view = _view(
        ...
        None,
        {
            "distance.mean.0": 44.0,
            "distance.sigma.0": 6.0,
            "distance.shape.0": 0.0,
            "distance.amplitude.0": 1.0,
            "anisotropy.r0": 0.38,
            "anisotropy.g": 1.3,
            "anisotropy.l1": reference["l1"],
            "anisotropy.l2": reference["l2"],
            "rotation.amplitude.0": reference["b0"],
            "rotation.time.0": reference["rho0"],
        },
...
def test_pddem(mode, rtol):
    ...
    view = _view(
        ...
        None,
        {
            "chromophore_a.amplitude.0": 1.0,
            "chromophore_a.tau.0": 1.2,
            "distance.mean.0": 45.0,
            "distance.sigma.0": 6.0,
            "distance.shape.0": 0.0,
            "distance.amplitude.0": 1.0,
            ...
            "pddem.pure_a": 0.1,
            "pddem.pure_b": 0.05,
            "pddem.excitation_a": 0.9,
            "pddem.excitation_b": 0.1,
            "pddem.emission_a": 0.2,
            "pddem.emission_b": 0.8,
        },
        scalars={"transfer_mode": mode},
    )
    np.testing.assert_allclose(
        np.asarray(view.y), np.asarray(reference["chisurf"]), rtol=rtol, atol=1e-9
    )
...
def test_acceptor_density(dimension, periodic):
    ...
    reference = json.loads(
        (pathlib.Path(__file__).parent / "data" / "acceptor_density_reference.json").read_text()
    )["curves"][f"{dimension}-{int(periodic)}"]
    view = _view(
        "tcspc_fret_acceptor_density",
        f"tcspc_fret_acceptor_density.dimensions.{dimension}",
...
def test_a_structure_ensemble():
    ...
    reference = json.loads(
        (pathlib.Path(__file__).parent / "data" / "fret_structure_reference.json").read_text()
    )["curve"]
    ...
    for canonical, value in {
        ...
        "instrument.scatter": 0.01,
        "instrument.background": 2.0,
        "distance.amplitude.0": 0.5,
        "distance.amplitude.1": 0.5,
    }.items():
        _set(problem, canonical, value)
    model.update()
