...
def test_laplace_and_sampling_joints_agree():
    ...
    mcmc = E.SamplingEngine(fit).add_joint_target(names).run(steps=5000, n_runs=2).joint(names)
...
def test_stored_engine_prefers_a_converged_chain(tmp_path, monkeypatch):
    ...
    before = {m.name: m.method for m in E.StoredEngine(fit).add_all_targets().run().marginals()}
    ...
    after = {m.name: m.method for m in E.StoredEngine(fit).add_all_targets().run().marginals()}
