...
def _fcs_d_2c(x, N_1, D_1, N_2, D_2, w_r, w_z, tau_T, Trip, y_0):
    return y_0 + _G * 1 / (N_1 + N_2) ** 2 * (
        1 + (Trip / (1 - Trip) * np.exp(-x / tau_T / 1e-6))
    ) * (
        (
            N_1
            * (1 / (1 + 4 * (D_1 * 1e-12) * x / (w_r * 1e-6) ** 2))
            * (1 / np.sqrt(1 + 4 * (D_1 * 1e-12) * x / (w_z * 1e-6) ** 2))
        )
        + (
            N_2
            * (1 / (1 + 4 * (D_2 * 1e-12) * x / (w_r * 1e-6) ** 2))
...
def _fcs_taud(x, N, tau_D, p, tau_T, Trip, y_0):
    return (
        ...
        * (1 + (Trip / (1 - Trip)) * np.exp(-x / tau_T / 1e-6))
        * (1 / (1 + x / (tau_D * 1e-6)))
        * (1 / np.sqrt(1 + (1 / p**2) * x / (tau_D * 1e-6)))
        + y_0
    )
...
def _fcs_taud_anom(x, N, tau_D, p, alpha, tau_T, Trip, y_0):
    return (
        ...
        * (1 / np.sqrt(1 + (1 / p**2) * (x / (tau_D * 1e-6)) ** alpha))
        + y_0
    )
...
def _fcs_flow(x, N, D, w_r, w_z, tau_T, Trip, y_0, v_flow):
    return (
        ...
        * (1 / (1 + 4 * (D * 1e-12) * x / (w_r * 1e-6) ** 2))
        * (1 / np.sqrt(1 + 4 * (D * 1e-12) * x / (w_z * 1e-6) ** 2))
        * np.exp(-((x * v_flow / w_r) ** 2) / (1 + 4 * (D * 1e-12) * x / (w_r * 1e-6) ** 2))
        + y_0
    )
...
def _fcs_ap(x, N, D, w_r, w_z, tau_T, Trip, A_ap, tau_ap, stretch, y_0):
    return (
        ...
        * (1 / (1 + 4 * (D * 1e-12) * x / (w_r * 1e-6) ** 2))
        * (1 / np.sqrt(1 + 4 * (D * 1e-12) * x / (w_z * 1e-6) ** 2))
        + A_ap * np.exp(-((x / tau_ap / 1e-6) ** stretch))
        + y_0
    )
...
def _fret_fccs(x, N_1, N_2, N_3, a, k_12, k_21, E_1, E_2, D, w_r, w_z, b, c, y_0):
    ...
    t1 = a * (
        _G
        * (1 / N_1)
        * (1 + (k_12 * k_21 * (E_1 - E_2) ** 2 / (k_21 * (1 - E_1) + k_12 * (1 - E_2)) ** 2) * ex)
        * diff
    )
    t2 = b * (
        _G
        * (1 / N_2)
        * (1 + (k_12 * k_21 * (E_1 - E_2) ** 2 / (k_21 * E_1 + k_12 * E_2) ** 2) * ex)
        * diff
    )
    t3 = c * (
        _G
        ...
        * (
            ...
            - (
                ...
                * k_21
                * (E_1 - E_2) ** 2
                / ((k_21 * (1 - E_1) + k_12 * (1 - E_2)) * (k_21 * E_1 + k_12 * E_2))
            )
            * ex
        )
        * diff
...
def _fcs_scan(x, N, D, w_r, w_z, tau_T, Trip, diam, freq, y_0):
    return (
        ...
        * (1 / (1 + 4 * (D * 1e-12) * x / (w_r * 1e-6) ** 2))
        * (1 / np.sqrt(1 + 4 * (D * 1e-12) * x / (w_z * 1e-6) ** 2))
        * np.exp(-(diam**2) * np.sin(np.pi * freq * 10**3 * x) ** 2 / (w_r**2 + 4 * D * x))
        + y_0
    )
...
def _sccf1(x, N, tau_D, p, alpha, tau_T, A, y_0, isCCF):
    return (
        ...
        * 1
        / N
        * (1 + (1 - 2 * isCCF) * A * np.exp(-x / tau_T / 1e-6))
        * (1 / (1 + (x / (tau_D * 1e-6)) ** alpha))
        * (1 / np.sqrt(1 + (1 / p**2) * (x / (tau_D * 1e-6)) ** alpha))
        + y_0
    )
...
def _sccf2(x, N, tau_D, p, alpha, tau_1, A_1, tau_2, A_2, y_0, isCCF):
    return (
        ...
        * (
            1
            + (1 - 2 * isCCF) * A_1 * np.exp(-x / tau_1 / 1e-6)
...
PAM_DEFAULTS = {
    "3D diffusion + background (tauD, triplet)": dict(
        Counts=100, BG=0, N=1, tau_D=30, p=5, tau_T=1, Trip=0.0, y_0=0
    ),
    "SCCF anomalous diffusion, 1 relaxation": dict(
        N=0.01, tau_D=2500, p=5, alpha=1, tau_T=1, A=1, y_0=0, isCCF=0
    ),
    "SCCF anomalous diffusion, 2 relaxation": dict(
        N=0.01, tau_D=2500, p=5, alpha=1, tau_1=1, A_1=1, tau_2=1, A_2=1, y_0=0, isCCF=0
    ),
    "SCCF anomalous diffusion, 3 relaxation": dict(
        N=0.01,
        tau_D=2500,
        p=5,
        alpha=1,
    ...
    "Full FCS (antibunching + 2 summed bunching + triplet)": dict(
        N=1,
        tau_D=60,
        p=5,
        tau_AB=1,
    ...
    "Full FCS (antibunching + 3 product bunching)": dict(
        N=1,
        tau_D=60,
        p=5,
        tau_AB=1,
