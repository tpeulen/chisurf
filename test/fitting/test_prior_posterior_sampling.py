...
def test_lnprob_parts_splits_likelihood_and_prior():
    ...
    lnlike, lnpr, chi2 = chisurf.core.fitting.fit.lnprob_parts(
        values, fit, bounds=bounds
    )
    ...
    chi2_data = chisurf.core.fitting.fit.get_chi2(
        values, model=fit.model, reduced=False
    )
...
def test_walk_mcmc_marginal_is_pulled_by_a_dominant_prior():
    ...
    r = chisurf.core.fitting.sample.walk_mcmc(
        fit=fit, steps=3000, step_size=0.01, temp=1.0, thin=1
    )
...
def test_walk_mcmc_reports_prior_and_data_chi2_separately():
    ...
    r = chisurf.core.fitting.sample.walk_mcmc(
        fit=fit, steps=200, step_size=0.01, temp=1.0, thin=1
    )
    assert 'lnprior' in r
    ...
    chi2_direct = chisurf.core.fitting.fit.get_chi2(
        state, model=fit.model, reduced=False
    )
...
def test_flat_prior_chain_is_unchanged_by_the_fix():
    ...
    r = chisurf.core.fitting.sample.walk_mcmc(
        fit=fit, steps=300, step_size=0.01, temp=1.0, thin=1
    )
    lnprior = np.asarray(r['lnprior'], dtype=float)
