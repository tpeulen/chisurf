...
def test_every_control_of_the_designer_file_is_still_there(controller):
    ...
    for name in ("button_fit", "button_sample", "button_auto_fit_range", "button_dataset_select"):
...
def test_action_buttons_have_one_height_and_mcts_is_baby_blue(controller):
    buttons = [
        ...
        for action in ("fit", "mcts", "sample", "settings", "auto_range", "select_dataset")
    ]
    ...
    qss = pathlib.Path("chisurf/gui/styles/widgets/fitting_buttons.qss").read_text()
...
def test_the_settings_offer_every_sampler_and_format_that_exists(controller):
    ...
    assert [name for name, _ in model.chain_format_options()] == list(fit_module.CHAIN_FORMATS)
...
def test_visiting_two_samplers_does_not_poison_the_next_run(controller, tmp_path, monkeypatch):
    ...
    fit_module.sample_fit(
        fit=controller.fit, target_directory=str(tmp_path), steps=40, n_runs=1, **kw
    )
