...
def pump(n=6):
    for _ in range(n):
        app.processEvents(); w.host.repaint()
pump()
...
w.surface.docks.focus("Selection"); pump()
w.host.grab().save(str(OUT / "globalview.after.selection.png"))
w.surface.docks.focus("Parameters"); w.surface.docks.focus("View"); pump()
w.host.grab().save(str(OUT / "globalview.after.parameters.png"))
w.model.representation = "factor graph"; w.model.include_fixed = True; w.model.relayout()
w.surface.docks.focus("Network"); pump()
w.host.grab().save(str(OUT / "globalview.after.factors.png"))
print(w.model.status)
w.model.representation = "network"; w.model.include_fixed = False; w.model.relayout()
w.show_guide(); tour = w._guided_tour; tour.start(1); pump(); app.processEvents()
w.grab().save(str(OUT / "globalview.after.tour.png"))
tour.stop()

# The inventory: every control the spec drew, every table column, every dock.
w.surface.docks.focus("Parameters"); w.surface.docks.focus("View"); pump()
w.surface.docks.focus("Network"); w.surface.docks.focus("Selection"); pump()
inventory = {
    "controls": sorted({n for f in w.surface.forms.values() for n in f.rects}),
    "columns": {name: [c.title for c in b.control.columns]
                for f in w.surface.forms.values() for name, b in f.tables.items()},
    "docks": [title for title, _region in __import__(
        "chisurf.plugins.core.globalview.gui.surface", fromlist=["DOCKS"]).DOCKS],
}
