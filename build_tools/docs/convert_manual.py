...
def main():
    ...
    # Write the *proposed* index beside the real one — never over it.
    #
    # ``index.rst`` is hand-maintained: the document's own order is one flat run
    # of headings, and the chapters a reader navigates by (the fitting
    # interface, the correlator, each worked example) exist only in the index.
    # Regenerating over it would throw that structure away silently, and the
    # help browser builds its tree from exactly this file. So the generated
    # version lands next to it and the difference is merged by hand.
    seen = set()
    unique = []
    ...
    idx = [
        "ChiSurf Manual (generated order)",
        "================================",
        "",
        ".. Generated by convert_manual.py. The published index is index.rst,",
        ".. which groups these pages into chapters; merge new entries into it.",
        "",
        ".. toctree::",
        "   :maxdepth: 2",
        "",
    ...
    idx.append("")
    (OUTPUT_DIR / "index.generated.rst").write_text("\n".join(idx), encoding="utf-8")
    print(f"Written {len(names)} RST files to {OUTPUT_DIR}")
    print("Proposed toctree written to index.generated.rst (index.rst is hand-maintained)")
