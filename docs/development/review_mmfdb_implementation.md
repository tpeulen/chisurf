...
This document is the current review artifact for the MMFDB implementation. It
...
- MMFDB/FDB tests under `test/fio/`
- Provenance plugin tests under `test/plugins/`
- `docs/prd_mmfdb_architecture.md`

Scope correction: `flr_*` tables and PDBx/PDB-IHM/mmCIF support are valid MMFDB
...
boundary between `flr_*`/PDBx/PDB-IHM and the `mmfdb_*` provenance core explicit.
...
2. The database schema still does not enforce the full `mmfdb_edge`
   relationship vocabulary.
3. Legacy MD5 compatibility writes corrupt checksum algorithm metadata.

The architectural direction is still correct: keep MMFDB relational. Do not move
...
```bash
python -m pytest test/fio/test_fdb_*.py test/fio/test_mmfdb_*.py -q --no-cov
```
...
- Existing warnings remain for deprecated `FluorophoreDatabase` compatibility
  ...
  MMFDB correctness blockers.

### Manual probes

The following behaviors were checked directly against a fresh MMFDB database:
...
- `add_provenance_edge(..., relationship_type="nonsense_rel")` raises
  `ValueError`.
- Direct SQL still accepts `mmfdb_edge.relationship_type = 'nonsense_rel'`.
...
- `store_chinet_session(...)` and `archive_fit_to_mmfdb(...)` now run inside
  `db.transaction()`.
...
- v18/v19 migration converts operation input/output edges into
  `mmfdb_operation_artifact` and keeps those relationships out of `mmfdb_edge`.
- Fresh databases skip experimental `mmfdb_sample` and `mmfdb_experiment`; `flr_*`
...
- `add_processing_run(...)` writes the operation first through
  `record_operation(...)`: `chisurf/core/mmfdb/repository.py:3160`.
- It then writes input links in a later loop:
  `chisurf/core/mmfdb/repository.py:3167`.
- The final audit row is also outside one explicit workflow transaction:
  `chisurf/core/mmfdb/repository.py:3177`.
- The MMFDB service handler calls `db.add_processing_run(...)`, then writes
  processed inputs and products in additional steps:
  `chisurf/plugins/core/mmfdb_admin/backend/measurement_services.py:1070`.
...
- A failed processing workflow can leave a real-looking operation with no valid
  input links.
- This is exactly the type of provenance corruption MMFDB is meant to prevent.
...
- Missing raw input artifact leaves no `mmfdb_operation`, no
  `mmfdb_operation_artifact`, no `mmfdb_audit_log`, and no partial output artifacts
  for the attempted processing ID.
- Missing processed input artifact in
  `mmfdb.v1.processing.record` / compatibility service leaves zero partial rows.
...
### P1 - `mmfdb_edge` does not enforce the full relationship vocabulary at the DB boundary

Evidence:

- Fresh schema only prevents `input_to` and `produced` in `mmfdb_edge`:
  `chisurf/core/mmfdb/schema.py:825`.
- Built-in allowed relationship values are defined in vocabulary bootstrap:
  `chisurf/core/mmfdb/schema.py:1015`.
...
- This undermines the PRD requirement that MMFDB be future-proof and auditable.

Required fix:

- Keep the repository validation, but add DB-level enforcement too.
- Because `relationship_type` is extensible through `mmfdb_vocabulary`, prefer
  ...
  - reject values for which no active row exists in `mmfdb_vocabulary` with
    `field_name = 'relationship_type'`
- Add a schema migration, likely v20, that:
  - scans existing invalid `mmfdb_edge` relationship values
...
- `add_artifact(...)` accepts a legacy `md5` argument and forwards it as
  canonical `checksum`: `chisurf/core/mmfdb/repository.py:670`.
- It does not pass `checksum_algorithm="md5"`, so `register_artifact(...)`
  applies its default `sha256`: `chisurf/core/mmfdb/repository.py:2233`.
...
- Split responsibility without changing storage:
  ...
  - `MMFDBWorkflowWriter`: high-level atomic workflow entrypoints used by plugins.
...
- Repo search test prevents new canonical code from importing old MMFDB authority
...
Make MMFDB, the multiparameteric fluorescence database, production-ready as a
...
- Do not reintroduce persisted `input_to` or `produced` rows in `mmfdb_edge`.
...
- Operation input/output links are stored only in `mmfdb_operation_artifact`.
- `mmfdb_edge` stores only non-operation relationships.
- `mmfdb_edge.relationship_type` is enforced by active `mmfdb_vocabulary` rows at
...
- `mmfdb.v1.processing.record` or equivalent service handlers
...
- Add triggers for `mmfdb_edge` insert/update.
- Reject `input_to` and `produced`.
- Reject relationship values not present as active
  `mmfdb_vocabulary(field_name='relationship_type', value=...)`.
...
Update `docs/prd_mmfdb_architecture.md` after the code changes:

- State that `flr_*` and PDBx/PDB-IHM are first-class domain tables.
- State that `mmfdb_*` is the provenance core.
...
- Plugin manifest methods exactly match registered `mmfdb.v1.*` methods.
...
6. Split or clearly quarantine compatibility code from canonical repository
   code.
7. Update `docs/prd_mmfdb_architecture.md` to match final code.
...
- Direct SQL cannot insert invalid `mmfdb_edge.relationship_type` values.
- Legacy MD5 writes are labeled as MD5.
- Focused MMFDB/FDB tests pass.
- Chinet rollback tests pass.
- The broader `test/fio/test_fdb_*.py test/fio/test_mmfdb_*.py` suite passes.
- The PRD and implementation agree on `flr_*`, PDBx/PDB-IHM, and `mmfdb_*`
  responsibilities.
