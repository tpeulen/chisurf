...
# PRD: MMFDB Viewer + Future-Ready Node Editor Workflows

## Summary

Enhance the MMFDB viewer so users can inspect the full experiment data chain:
...
- [x] Keep this PRD at `docs/prd_mmfdb_viewer_node_editor_workflows.md`.
- [x] Mark checklist items as implementation progresses.
- [x] Do not add a new graph-rendering dependency.
- [x] Do not add an MMFDB schema migration for this task.
...
- [x] Existing MMFDB UI bug: provenance upstream/downstream handlers require
...
- [x] Existing node editor detail: edge port indexes are combined
      `port_items` indexes, not separate input/output indexes. For MMFDB
      adapter nodes with one input and one output, provenance edges must use
      `source_port=1` and `target_port=0`.

## Product Requirements

- [x] The MMFDB viewer shows experiment-linked raw data, processing runs,
...
- [x] Provenance tab shows an edge table, JSON/details panel, and embedded node
      editor graph.
- [x] Provenance graph is read-only in the MMFDB viewer.
...
- [x] Add `mmfdb_graph_to_node_editor_graph(graph: dict[str, Any]) -> dict[str, Any]`.
...
- [x] Top-level output includes:
  ...
  - [x] `meta.schema_name: "mmfdb.provenance.node_editor.v1"`
  - [x] `nodes`
  - [x] `edges`
- [x] Node IDs are stable: `f"{node_type}:{node_id}"`.
- [x] Each MMFDB node uses:
  - [x] `type: "mmfdb_record"`
  - [x] one input port: `{"name": "in", "type": "mmfdb"}`
  - [x] one output port: `{"name": "out", "type": "mmfdb"}`
  - [x] `config.record` containing the original MMFDB node
  - [x] `config.node_type`
  - [x] `config.node_id`
  - [x] `config.workflow_runtime: None`
  - [x] `collapsed: False`
- [x] Each MMFDB edge uses:
...
- [x] `Load full graph` calls `export_provenance_graph`, converts via
      `mmfdb_graph_to_node_editor_graph`, loads with `node_editor.load_graph_dict`,
...
- [x] Add condition and probe RPC helpers behind the MMFDB client boundary.
...
- [x] Empty MMFDB graph converts to `{version: 1, nodes: [], edges: []}` with
      provenance metadata.
...
- [ ] Verify no Qt crash constructing `MMFDBWidget` headlessly.
...
- [ ] If Qt environment supports it, run the new MMFDB widget smoke test.
...
- [x] Node editor graph schema remains suitable for future user-defined
      workflows.
- [x] Current MMFDB RPC key mismatches are fixed.
- [x] Provenance RPC calls use correct parameters.
- [x] All new tests pass.
- [x] Existing node editor and MMFDB plugin tests pass.
...
- [x] MMFDB provenance uses `meta.purpose="provenance_view"` and must remain
      read-only.
...
- [x] `NodeType.runtime`, `NodeType.operation`, and `NodeType.executor` are
      optional future hooks, not required for MMFDB provenance display.
...
- [x] Previous functional blockers for provenance display are fixed:
  ...
  - [x] Empty-experiment processed-product scoping returns an empty list.
  - [x] Processed-product row count reads the top-level `row_count` field.
- [x] Architecture blocker resolved: `MMFDBClient` now defaults to `ZmqClient`;
...
- [x] **P1: `MMFDBClient` now defaults to a ZMQ JSON-RPC client.**
  ...
  - [x] Added `MMFDBClient._make_zmq_client()`.
...
- [x] `/Users/tpeulen/mambaforge/envs/arm64/bin/python3 -m ruff check --select F,I,W chisurf/gui/widgets/node_editor/node_viewer.py chisurf/gui/widgets/node_editor/editor.py chisurf/gui/widgets/node_editor/tests/test_node_viewer.py chisurf/gui/widgets/node_editor/edge_item.py chisurf/gui/widgets/node_editor/graph.py chisurf/gui/widgets/node_editor/registry.py chisurf/gui/widgets/node_editor/scene.py chisurf/plugins/core/mmfdb_admin/gui/client.py chisurf/plugins/core/mmfdb_admin/gui/provenance_graph.py chisurf/plugins/core/mmfdb_admin/gui/tool.py chisurf/plugins/core/mmfdb_admin/backend/services.py chisurf/plugins/sample_database/gui/client.py test/plugins/test_provenance_graph_adapter.py test/plugins/test_sample_database_plugin.py chisurf/gui/widgets/node_editor/tests/test_round_trip.py chisurf/gui/widgets/node_editor/tests/test_read_only.py chisurf/gui/widgets/node_editor/tests/test_graph_headless.py`
      passed.
- [x] `git diff --check -- chisurf/gui/widgets/node_editor chisurf/plugins/core/mmfdb_admin chisurf/plugins/sample_database test/plugins/test_provenance_graph_adapter.py test/plugins/test_sample_database_plugin.py docs/prd_mmfdb_viewer_node_editor_workflows.md`
...
- [x] **P2: MMFDB ZMQ transport errors are handled at widget startup.**
      `MMFDBWidget` now accepts an injectable client, keeps a Refresh action
  ...
  - [x] Added `MMFDBWidget(client=...)` test injection.
...
- [ ] **P2: NodeViewer JSON-RPC provider API is still not implemented.**
      This is acceptable only if explicitly scoped as future work. Current MMFDB
      provenance display still calls `provenance.dependencies.*` and
      `provenance.graph.export` through `MMFDBClient`, not `nodeviewer.graph.get`.
...
- [x] Add startup transport-error handling for `MMFDBWidget`.
...
- [ ] Re-run `ruff check --select F,I,W` on touched node editor and MMFDB files.
...
- [ ] Must not import Qt.
- [ ] Must not import ZMQ.
- [ ] Must not import MMFDB repository or plugin backend modules.
...
- [ ] MMFDB provider maps MMFDB provenance records into the shared node graph
      schema.
...
- [ ] Move provider-neutral conversion helpers to core or server-side provider
      modules.
- [ ] Create `chisurf/plugins/core/mmfdb_admin/backend/nodeviewer_provider.py`.
- [ ] Register the MMFDB provenance provider under provider id
      `mmfdb.provenance`.
...
- [ ] It must be usable without MMFDB imports.
...
## MMFDB GUI Refactor Checklist

- [ ] `MMFDBClient` production default uses `ZmqClient`.
- [ ] `MMFDBClient` accepts host/port or a configured JSON-RPC endpoint.
- [ ] `MMFDBClient` accepts an injected fake/in-process client for tests.
- [ ] `MMFDBClient` never imports `mmfdb.admin.backend`.
- [ ] `MMFDBClient` never registers services.
- [ ] `MMFDBWidget` uses only `MMFDBClient` and pure GUI/core helpers.
- [ ] `MMFDBWidget` has no direct imports from `chisurf.core.fio.mmcif.db`.
- [ ] `MMFDBWidget` has no direct imports from `chisurf.core.mmfdb.repository`.
- [ ] Add JSON-RPC methods for sample conditions:
  - [ ] `mmfdb.sample_conditions.get`
  - [ ] `mmfdb.sample_conditions.save`
  - [ ] `mmfdb.sample_conditions.list`
- [ ] Add JSON-RPC methods for probes:
  - [ ] `mmfdb.probes.list`
  - [ ] `mmfdb.probes.get`
  - [ ] optional `mmfdb.probes.optical_properties.get`
...
## MMFDB Provenance NodeViewer Checklist

- [ ] Add provider id `mmfdb.provenance`.
...
- [ ] Provider includes `meta.provider_id = "mmfdb.provenance"`.
...
- [ ] Existing MMFDB provenance JSON/ZIP export RPCs remain available.
...
- [ ] Test that `MMFDBClient()` production path constructs a ZMQ JSON-RPC client.
- [ ] Test that `MMFDBClient(fake_client)` still works for unit tests.
- [ ] Test that importing `mmfdb.admin.gui.tool` does not import MMFDB repository
      modules.
- [ ] Test that `MMFDBWidget.save_condition()` calls a client method, not a
      repository.
- [ ] Test that `MMFDBWidget.fill_probes()` calls a client method, not a
      repository.

NodeViewer API tests:

- [ ] Test `NodeViewerWidget` can load a graph without MMFDB imports.
...
- [ ] Test `nodeviewer.graph.get` for `mmfdb.provenance` full graph.
...
- [ ] Open MMFDB plugin as a GUI client.
...
- [ ] In-process dispatch is limited to explicit tests or explicit development
      fallback code.
- [ ] MMFDB GUI contains no direct database/repository/backend service imports.
...
- [ ] MMFDB provenance display can load through `nodeviewer.graph.get`.
- [ ] If `nodeviewer.graph.get` remains future scope, the PRD and completion
      report explicitly say that MMFDB provenance still uses the legacy
      provenance RPCs over ZMQ JSON-RPC.
- [ ] `MMFDBWidget` handles disconnected ZMQ transport during startup without
      raising out of `__init__`.
- [ ] All new transport-boundary tests pass.
- [ ] All new layout-regression tests pass.
- [ ] Existing MMFDB provenance and node editor tests still pass.
