...
Owner: MMFDB implementation agents
...
MMFDB needs a hard-cut authorization architecture before production use. The
...
MMFDB access to non-public objects must require authentication. Public mode
...
- [ ] MMFDB is in development; breaking RPC, schema, GUI, and test changes are
  allowed.
...
- [ ] Require authentication for every non-public MMFDB object read.
- [ ] Require authentication for every MMFDB write, delete, share, ownership
...
- [ ] Update MMFDB GUI/client, startup login, user editor, setup services,
...
- [ ] Do not preserve legacy `sample_database.*` authorization behavior if it
  conflicts with the new MMFDB contract.
...
- **Object**: any MMFDB-protected resource addressed by `(object_type,
  object_id)`.
...
- [ ] `artifact`: `mmfdb_artifact`
- [ ] `operation`: `mmfdb_operation`
- [ ] `operation_artifact`: `mmfdb_operation_artifact`
- [ ] `parameter`: `mmfdb_parameter`
- [ ] `setup`: `mmfdb_setup`
- [ ] `branch`: `mmfdb_branch`
- [ ] `edge`: `mmfdb_edge`
...
- [ ] Other-write and other-manage modes are invalid for MMFDB unless an admin
...
Add schema version after the current MMFDB schema version.
...
- [ ] `mmfdb.auth.login(user_id, password, client_metadata=None)`
- [ ] `mmfdb.auth.logout(auth)`
- [ ] `mmfdb.auth.me(auth)`
- [ ] `mmfdb.auth.sessions.list(auth, user_id=None)`
- [ ] `mmfdb.auth.sessions.revoke(auth, session_id)`
...
- [ ] Failed login attempts are written to `mmfdb_auth_attempt`.
...
Create `chisurf.core.mmfdb.auth`.
...
- [ ] `MMFDBClient.login(...)` stores the token.
- [ ] `MMFDBClient.logout()` revokes and clears the token.
- [ ] `MMFDBClient.me()` returns current authenticated user.
- [ ] `MMFDBClient` injects `auth` automatically into protected calls.
- [ ] `MMFDBClient` can make explicit anonymous public-read calls before login.
...
- [ ] `mmfdb.groups.list(auth)`
- [ ] `mmfdb.groups.get(auth, group_id)`
- [ ] `mmfdb.groups.create(auth, group)`
- [ ] `mmfdb.groups.update(auth, group)`
- [ ] `mmfdb.groups.delete(auth, group_id)`
- [ ] `mmfdb.groups.members.list(auth, group_id)`
- [ ] `mmfdb.groups.members.add(auth, group_id, user_id, role='member')`
- [ ] `mmfdb.groups.members.remove(auth, group_id, user_id)`
- [ ] `mmfdb.permissions.get(auth, object_type, object_id)`
- [ ] `mmfdb.permissions.chmod(auth, object_type, object_id, mode)`
- [ ] `mmfdb.permissions.chown(auth, object_type, object_id, owner_user_id)`
- [ ] `mmfdb.permissions.chgrp(auth, object_type, object_id, owner_group_id)`
- [ ] `mmfdb.permissions.grant(auth, object_type, object_id, subject_type,
  subject_id, permissions, effect='allow')`
- [ ] `mmfdb.permissions.revoke(auth, entry_id)`
...
- [ ] MMFDB plugin manifest RPC method list
- [ ] sample database compatibility plugin or remove it if the hard cut makes it
  obsolete
- [ ] `test/fio/test_mmfdb_user_management.py`
- [ ] MMFDB/FDB provenance tests under `test/fio/`
- [ ] plugin tests that call MMFDB services
...
- [ ] Replace login service with `mmfdb.auth.login`.
...
Phase 4: protected MMFDB objects
...
- [ ] Update `MMFDBClient` token handling.
- [ ] Update startup login.
- [ ] Update user editor.
- [ ] Update MMFDB plugin user/group/permissions UI.
...
- [ ] Run MMFDB user/group tests.
- [ ] Run MMFDB/FDB provenance tests.
- [ ] Run MMFDB plugin tests.
...
- [ ] Fresh DB creates `mmfdb_group`.
- [ ] Fresh DB creates `mmfdb_group_member`.
- [ ] Fresh DB creates `mmfdb_object_acl`.
- [ ] Fresh DB creates `mmfdb_acl_entry`.
- [ ] Fresh DB creates `mmfdb_session`.
- [ ] Fresh DB creates `mmfdb_auth_attempt`.
...
- [ ] No MMFDB protected write succeeds without auth.
- [ ] `MMFDBClient` injects auth automatically after login.
