...
`PDA3c`, pairing with `c2PDA` for the two-colour method, so the colour count
...
bursts. It is the three-colour counterpart of {doc}`PDA <pda2c>`, and it shares
that page's central idea — the noise is computed, not fitted — but almost
nothing of its machinery.

For the workflow in ChiSurf, see the guide {doc}`/guides/42_pda3c`.
...
This is where PDA3c departs from two-colour PDA in kind, not degree. Two-colour
PDA builds an **S1S2 count matrix** and fits a one-dimensional projection of it;
PDA3c fits the **per-burst likelihood** directly. There is no histogram to
