...
:strong:`Fig.2 Accompanying software modules.` ChiSurf is accompanied by software for burst-wise single molecule spectroscopy (ndXplorer), fluorescence correlation spectroscopy (tttrlib), and time-resolved image spectroscopy (clsmview), and additional software for statistical analysis. Histograms over multiparameter fluorescence spectroscopy data and sampled parameters can be computed for sub-ensemble analysis (Burst-wise single molecule spectroscopy). Photon traces can be analyzed for fluorescence correlation spectroscopy, FCS. Data collected on a microscope equipped with time-resolved detection can be processed for FLIM and for pixel-grouped analysis (Fluorescence Lifetime Image Microscopy, FLIM).
