"""Study / project entity access for :class:`~mmfdb.repository.MFDatabase`.

Provides the ``mmfdb_study`` surface (studies, members, configurable fields, and
...
class StudyMixin:
    ...
    def create_study(
        ...
        *,
        ...
        with self._transaction():
            self.conn.execute(
                "INSERT INTO mmfdb_study (study_id, name, description, "
    ...
    def get_study(self, study_id: str) -> dict[str, Any] | None:
        ...
        return self.dao.get("mmfdb_study", study_id, pk_column="study_id")
    ...
    def list_studies(
        ...
        rows = self.conn.execute(
            f"SELECT * FROM mmfdb_study WHERE {' AND '.join(where)} ORDER BY name",
            params,
        ).fetchall()
    ...
    def add_study_member(
        ...
        now = _utc_now()
        with self._transaction():
            if self.dao.list("mmfdb_study_member", filters={
                "study_id": study_id, "member_type": member_type, "member_id": member_id,
            }, limit=1):
                return
            next_id = (
                self.conn.execute(
                    "SELECT COALESCE(MAX(member_row_id), 0) FROM mmfdb_study_member"
                ).fetchone()[0]
                + 1
            )
            self.conn.execute(
                "INSERT INTO mmfdb_study_member (member_row_id, study_id, member_type, "
    ...
    def list_study_members(
        ...
        sql = (
            "SELECT member_type, member_id, role FROM mmfdb_study_member "
    ...
    def list_studies_for_member(
        ...
        return [
            ...
            for r in self.conn.execute(
                "SELECT DISTINCT study_id FROM mmfdb_study_member "
    ...
    def set_study_field(self, study_id: str, key: str, value: str) -> None:
        ...
        with self._transaction():
            existing = self.dao.list(
                "mmfdb_study_key_value", filters={"study_id": study_id, "key": key}, limit=1
            )
            if existing:
                self.conn.execute(
                    "UPDATE mmfdb_study_key_value SET value = ?, updated_at = ? "
                    "WHERE kv_id = ?",
            ...
            next_id = (
                self.conn.execute(
                    "SELECT COALESCE(MAX(kv_id), 0) FROM mmfdb_study_key_value"
                ).fetchone()[0]
                + 1
            )
            self.conn.execute(
                "INSERT INTO mmfdb_study_key_value (kv_id, study_id, key, value, "
    ...
    def get_study_fields(self, study_id: str) -> dict[str, str]:
        ...
        rows = self.dao.list(
            "mmfdb_study_key_value", filters={"study_id": study_id}, order_by="key"
        )
    ...
    def backfill_studies_from_project_ids(self) -> dict[str, int]:
        ...
        for project_id, sample_ids in by_project.items():
            existing = self.conn.execute(
                "SELECT study_id FROM mmfdb_study WHERE name = ? AND deleted_at IS NULL "
            ...
            for sample_id in sample_ids:
                before = self.conn.execute(
                    "SELECT COUNT(*) FROM mmfdb_study_member WHERE study_id = ? "
