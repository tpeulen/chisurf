...
from mmfdb.schema._sqlutil import _json_dumps, _json_loads, _utc_now
...
class AnalysisMixin:
    ...
    def get_analysis_run_full(self, analysis_id: str) -> dict[str, Any] | None:
        ...
        param_rows = self.dao.list(
            "mmfdb_parameter", filters={"operation_id": analysis_id}, order_by="parameter_id"
        )
        run["parameters"] = []
        ...
        grouped_rows = self.conn.execute(
            """SELECT op.operation_id AS analysis_id, op.operation_type AS analysis_type,
               ...
               FROM mmfdb_edge AS pe
               JOIN mmfdb_operation AS op ON op.operation_id = pe.target_node_id
    ...
    def delete_analysis_run(self, analysis_id: str) -> None:
        parameter_ids = [
            ...
            for row in self.dao.list(
                "mmfdb_parameter", filters={"operation_id": analysis_id}, include_deleted=True
            )
        ]
        ...
        with self.conn:
            ...
            for parameter_id in parameter_ids:
                ...
                self.conn.execute(
                    "UPDATE mmfdb_parameter SET deleted_at = ? WHERE parameter_uuid = ?",
                    (now, parameter_id),
                )
            self.conn.execute(
                """UPDATE mmfdb_edge SET deleted_at = ?
            ...
            self.conn.execute("UPDATE mmfdb_operation SET deleted_at = ? WHERE operation_id = ?", (now, analysis_id))
        self.add_audit_log(
            action="delete",
            target_type="analysis_run",
    ...
    def get_analysis_parameter(self, parameter_uuid: str) -> sqlite3.Row | dict[str, Any] | None:
        rows = self.dao.list(
            "mmfdb_parameter", filters={"parameter_uuid": parameter_uuid}, include_deleted=True, limit=1
        )
        if rows:
            d = rows[0]
            d["analysis_id"] = d.pop("operation_id", None)
