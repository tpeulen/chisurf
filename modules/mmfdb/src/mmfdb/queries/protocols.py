"""Protocol entity access for :class:`~mmfdb.repository.MFDatabase`.

Provides the ``mmfdb_protocol`` surface (named, versioned, append-only procedures
...
class ProtocolMixin:
        ...
        *,
        ...
        with self._transaction():
            prev = self.conn.execute(
                "SELECT COALESCE(MAX(version), 0) FROM mmfdb_protocol "
            ...
            self.conn.execute(
                "INSERT INTO mmfdb_protocol (protocol_id, name, version, category, "
    ...
    def get_protocol(
        ...
        if version == "latest":
            rows = self.dao.list(
                "mmfdb_protocol", filters={"name": name},
                order_by="version", descending=True, limit=1,
            )
        else:
            rows = self.dao.list(
                "mmfdb_protocol", filters={"name": name, "version": int(version)}, limit=1
            )
    ...
    def get_protocol_by_id(self, protocol_id: str) -> dict[str, Any] | None:
        ...
        return self.dao.get("mmfdb_protocol", protocol_id, pk_column="protocol_id")

    def list_protocol_versions(self, name: str) -> list[dict[str, Any]]:
        """Return all versions of a protocol ``name``, oldest first."""
        return self.dao.list("mmfdb_protocol", filters={"name": name}, order_by="version")
    ...
    def list_protocols(
        ...
        latest = (
            "version = (SELECT MAX(p2.version) FROM mmfdb_protocol p2 "
            "WHERE p2.name = mmfdb_protocol.name AND p2.deleted_at IS NULL)"
        )
        ...
        rows = self.conn.execute(
            f"SELECT * FROM mmfdb_protocol WHERE {' AND '.join(where)} ORDER BY name",
            params,
        ).fetchall()
    ...
    def get_protocol_parameter_schema(
        ...
        (``mmfdb_operation_parameter_def``) — ``{name: OperationParameterDef}`` — so a
