...
_NODE_REF_RE = re.compile(r"^\s*mmfdb://([a-z_]+)/(.+?)\s*$")
...
_NODE_LABEL_COLUMNS: dict[str, tuple[str, str, str]] = {
    ...
    "artifact": ("mmfdb_artifact", "artifact_id", "artifact_kind"),
    "operation": ("mmfdb_operation", "operation_id", "operation_type"),
}


def parse_node_ref(value: Any) -> tuple[str, str] | None:
    """Parse an ``mmfdb://<node_type>/<node_id>`` reference string.
...
class ArtifactOpsMixin:
    ...
    def update_operation(self, operation_id, **kwargs):
        ...
        with self._transaction():
            self.conn.execute(f"UPDATE mmfdb_operation SET {', '.join(cols)} WHERE operation_id = ?", vals)

    def update_operation_settings(self, operation_id, settings: dict):
        now = _utc_now()
        blob = _json_dumps(settings)
        with self._transaction():
            self.conn.execute("UPDATE mmfdb_operation SET settings_json = ?, updated_at = ? WHERE operation_id = ?", (blob, now, operation_id))
    ...
    def get_operations(self, operation_type=None, status=None, workflow_id=None):
        if workflow_id:
            ...
            query = "SELECT * FROM mmfdb_operation WHERE 1=1 AND deleted_at IS NULL"
            params = []
            if operation_type:
        ...
        return self.dao.list(
            "mmfdb_operation", filters=filters or None, order_by="created_at", descending=True
        )
    ...
    def update_artifact(self, artifact_id, **kwargs):
        ...
        with self._transaction():
            self.conn.execute(f"UPDATE mmfdb_artifact SET {', '.join(cols)} WHERE artifact_id = ?", vals)
    ...
    def delete_artifact(self, artifact_id: str) -> dict[str, Any]:
        ...
        with self._transaction():
            self.conn.execute(
                "UPDATE mmfdb_artifact SET deleted_at = ?, updated_at = ? WHERE artifact_id = ?",
                (now, now, artifact_id),
            )
            self.conn.execute(
                "UPDATE mmfdb_operation_artifact SET deleted_at = ? WHERE artifact_id = ?",
                (now, artifact_id),
            )
            self.conn.execute(
                "UPDATE mmfdb_artifact_owner SET deleted_at = ? WHERE artifact_id = ?",
                (now, artifact_id),
            )
            self.conn.execute(
                """UPDATE mmfdb_edge SET deleted_at = ?
    ...
    def get_artifacts(self, artifact_kind=None):
        ...
        return self.dao.list("mmfdb_artifact", filters=filters, order_by="artifact_id")
    ...
    def remove_operation_artifact(self, operation_id, artifact_id):
        with self._transaction():
            now = _utc_now()
            self.conn.execute(
                "UPDATE mmfdb_operation_artifact SET deleted_at = ? "
    ...
    def add_provenance_edge(
        ...
        **kwargs,
        ...
        with self._transaction():
            ...
            # mmfdb_edge's only unique key is its autoincrement edge_id (never
    ...
    def get_downstream_artifacts(self, artifact_id):
        return self.dao.list("mmfdb_edge", filters={"source_node_id": artifact_id})

    def get_upstream_artifacts(self, artifact_id):
        return self.dao.list("mmfdb_edge", filters={"target_node_id": artifact_id})
    ...
    def get_downstream_dependencies(
        ...
        if not node_type or not node_id:
            return []
        from mmfdb.provenance.graph import traverse_canonical_graph
    ...
    def export_provenance_graph(
        ...
        """
        from mmfdb.provenance.graph import normalize_node_type, traverse_canonical_graph
        ...
        for n_type, n_id in sorted(node_keys):
            ...
            if norm_type == "artifact":
                row = self.dao.get("mmfdb_artifact", n_id, include_deleted=True)
                if row:
                    node_dict.update(row)
            elif norm_type == "operation":
                row = self.dao.get("mmfdb_operation", n_id, include_deleted=True)
                if row:
                    node_dict.update(row)
    ...
    def get_artifact_impact(self, node_id: str) -> list[str]:
        ...
        transitive operation-graph descendants this also follows ``mmfdb_edge`` usage
    ...
    def get_artifact_compute_spec(self, artifact_id: str):
        ...
        """
        from mmfdb.provenance.compute_spec import get_compute_spec
    ...
    def find_raw_artifact_by_md5(self, content_md5: str) -> str:
        ...
        row = self.conn.execute(
            """SELECT artifact.artifact_id
               FROM mmfdb_artifact artifact
               JOIN mmfdb_object object_ref
    ...
    def register_artifact(
        ...
        """Register or update an artifact in the canonical MMFDB tables.
    ...
    def get_artifact(self, artifact_id: str) -> dict[str, Any] | None:
        ...
        return self.dao.get("mmfdb_artifact", artifact_id, include_deleted=True)
    ...
    def list_artifacts(
        ...
        query = "SELECT * FROM mmfdb_artifact WHERE 1=1 AND deleted_at IS NULL"
        params: list[Any] = []
    ...
    def record_operation(
        ...
        now = _utc_now()
        with self._transaction():
            operation_existed = _exists(self.conn, "mmfdb_operation", "operation_id", operation_id)
            self.conn.execute(
                """INSERT INTO mmfdb_operation (
            ...
            if not operation_existed:
                ...
                if not _exists(self.conn, "mmfdb_branch", "branch_uuid", active_branch_uuid):
                    self.dao.insert(
                        "mmfdb_branch",
                        {
                ...
                self.dao.update(
                    "mmfdb_branch", active_branch_uuid, {"head_operation_id": operation_id}
                )
            self.add_audit_log(
    ...
    def get_operation(self, operation_id: str) -> dict[str, Any] | None:
        ...
        return self.dao.get("mmfdb_operation", operation_id, include_deleted=True)
    ...
    def record_operation_link(
        ...
        with self._transaction():
            self.conn.execute(
                """INSERT INTO mmfdb_operation_artifact (
    ...
    def record_operation_with_artifacts(
        ...
        with self._transaction():
            ...
            for art in input_payloads:
                ...
                artifact_existed = _exists(self.conn, "mmfdb_artifact", "artifact_id", artifact_id)
            ...
            for param in parameter_payloads:
                ...
                parameter_existed = _exists(self.conn, "mmfdb_parameter", "parameter_uuid", parameter_uuid)
        ...
        **kwargs,
        ...
        with self._transaction():
            missing_inputs = [
                ...
                if not _exists(self.conn, "mmfdb_artifact", "artifact_id", raw_id)
            ]
            if missing_inputs:
    ...
    def get_provenance_edges(self, **kwargs) -> list[dict[str, Any]]:
        # 1. Fetch edges from mmfdb_edge
        query = "SELECT * FROM mmfdb_edge WHERE 1=1 AND deleted_at IS NULL"
        params = []
        ...
        # 2. Fetch edges from mmfdb_operation_artifact
        oa_query = "SELECT * FROM mmfdb_operation_artifact WHERE 1=1 AND deleted_at IS NULL"
        oa_params = []
        ...
        for r in oa_rows:
            art_kind = "artifact"
            art_row = self.dao.get("mmfdb_artifact", r["artifact_id"], include_deleted=True)
            if art_row:
                art_kind = art_row["artifact_kind"]
            op_type = "processing_run"
            op_row = self.dao.get("mmfdb_operation", r["operation_id"], include_deleted=True)
            if op_row:
    ...
    def get_processing_run_full(self, run_id: str) -> dict[str, Any] | None:
        ...
        op_arts = self.dao.list(
            "mmfdb_operation_artifact", filters={"operation_id": run_id, "direction": "input"}
        )
    ...
    def _materialize_metadata_ref(
        ...
        another entity *is* a link: if *value* is an ``mmfdb://<type>/<id>``
        reference, an idempotent ``mmfdb_edge`` is created from the owning node to
        ...
        exists = self.conn.execute(
            "SELECT 1 FROM mmfdb_edge WHERE source_node_type = ? AND source_node_id = ? "
        ...
        now = _utc_now()
        self.conn.execute(
            "INSERT INTO mmfdb_edge (source_node_type, source_node_id, target_node_type, "
