"""Branch / version-control access for :class:`~mmfdb.repository.MFDatabase`.

Provides the ``mmfdb_branch`` surface (create/fork branches, jump a user to an
...
class BranchMixin:
    ...
    def create_branch(
        ...
        if head_operation_id is not None:
            if not _exists(self.conn, "mmfdb_operation", "operation_id", head_operation_id):
                raise ValueError(f"Operation {head_operation_id!r} does not exist")

        now = _utc_now()
        with self._transaction():
            if self.dao.list("mmfdb_branch", filters={"name": name}, limit=1):
                raise ValueError(f"Branch name {name!r} already exists")

            self.conn.execute(
                """INSERT INTO mmfdb_branch (
    ...
    def jump_user_to_operation(
        ...
        if not _exists(self.conn, "mmfdb_operation", "operation_id", operation_id):
    ...
    def get_branch(self, branch_uuid_or_name: str) -> dict[str, Any] | None:
        row = self.conn.execute(
            "SELECT * FROM mmfdb_branch WHERE (branch_uuid = ? OR name = ?) AND deleted_at IS NULL",
            (branch_uuid_or_name, branch_uuid_or_name)
        ).fetchone()
        return _row_to_dict(row)

    def list_branches(self) -> list[dict[str, Any]]:
        return self.dao.list("mmfdb_branch", order_by="name")
    ...
    def update_branch_head(self, branch_uuid: str, head_operation_id: str | None) -> None:
        ...
        with self._transaction():
            self.conn.execute(
                "UPDATE mmfdb_branch SET head_operation_id = ?, updated_at = ? WHERE branch_uuid = ?",
    ...
    def delete_branch(self, branch_uuid: str) -> None:
        ...
        with self._transaction():
            ...
            now = _utc_now()
            self.conn.execute(
                "UPDATE mmfdb_branch SET deleted_at = ?, updated_at = ? WHERE branch_uuid = ?",
                (now, now, branch_uuid)
            )
            self.add_audit_log(
    ...
    def set_user_active_branch(self, user_id: str, branch_uuid: str) -> None:
        with self._transaction():
            if not _exists(self.conn, "flr_sample_users", "user_id", user_id):
                raise ValueError(f"User {user_id!r} does not exist")
            if not _exists(self.conn, "mmfdb_branch", "branch_uuid", branch_uuid):
