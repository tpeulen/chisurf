...
class SampleMixin:
    ...
    def link_artifact_to_sample(self, artifact_id: str, sample_id: str) -> None:
        ...
        if not artifact_id or not sample_id:
            return
        if self.dao.list("mmfdb_edge", filters={
    ...
    def set_object_sample_id(self, object_uuid: str, sample_id: str | None) -> None:
        ...
        with self.conn:
            self.conn.execute(
                "UPDATE mmfdb_object SET metadata_json = ? WHERE object_uuid = ?",
    ...
    def lookup_sample_by_md5(self, content_md5: str) -> str | None:
        ...
        str or None
            ...
            linked yet.
        """
        rows = self.dao.list("mmfdb_object", filters={"content_md5": content_md5}, include_deleted=True, limit=1)
        if not rows:
            return None
        try:
