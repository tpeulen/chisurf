...
class ObjectStoreMixin:
    ...
    def put_object(
        ...
        with self._transaction():
            existing = self.dao.get(
                "mmfdb_object", ref.md5, pk_column="content_md5", include_deleted=True
            )
            if existing:
                self.conn.execute(
                    "UPDATE mmfdb_object SET refcount = refcount + 1 WHERE content_md5 = ?",
                    (ref.md5,),
                )
    ...
    def delete_object(self, object_uuid: str) -> dict[str, Any]:
        ...
        with self._transaction():
            if refcount <= 1:
                self.conn.execute(
                    "DELETE FROM mmfdb_object WHERE object_uuid = ?",
                    (object_uuid,),
                )
                store = self._get_object_store()
                store.delete(md5)
                ...
                self.conn.execute(
                    "UPDATE mmfdb_object SET refcount = refcount - 1 WHERE object_uuid = ?",
                    (object_uuid,),
                )
                self.add_audit_log(
                    action="dereference",
