"""Operation-parameter access for :class:`~mmfdb.repository.MFDatabase`.
...
class ParameterMixin:
        ...
        if artifact_id is not None:
            ...
            operation_ids = [
                ...
                for row in self.conn.execute(
                    "SELECT DISTINCT operation_id FROM mmfdb_operation_artifact WHERE artifact_id = ? AND deleted_at IS NULL",
                    (artifact_id,),
                ).fetchall()
            ]
            ...
            query = f"SELECT * FROM mmfdb_parameter WHERE operation_id IN ({placeholders}) AND deleted_at IS NULL ORDER BY parameter_id"
            return [dict(row) for row in self.conn.execute(query, params).fetchall()]
        filters = {"operation_id": operation_id} if operation_id is not None else None
        return self.dao.list("mmfdb_parameter", filters=filters, order_by="parameter_id")
    ...
    def delete_parameter(self, param_id):
        ...
        with self._transaction():
            now = _utc_now()
            self.dao.soft_delete("mmfdb_parameter", param_id, pk_column="parameter_uuid", deleted_at=now)
            self.dao.soft_delete("mmfdb_parameter", param_id, pk_column="parameter_id", deleted_at=now)
