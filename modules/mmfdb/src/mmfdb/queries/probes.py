...
class ProbeMixin:
    ...
    def import_reference_set(
        ...
        *,
        ...
        spectra.db into this MMFDB, carrying the scraper-assigned category and
        ...
        try:
            # Fetch source probe types and build a map to MMFDB type_ids
            ...
            # Fetch MMFDB probe types, creating missing ones on the fly
            mmfdb_types = {
                r["type_name"]: r["type_id"]
                for r in self.conn.execute("SELECT type_id, type_name FROM probe_types").fetchall()
            }

            def _resolve_mmfdb_type(source_type_id: int) -> int:
                """Map a source type_id to the corresponding MMFDB type_id."""
                ...
                # Map source categories to MMFDB canonical type names
                mmfdb_type_name_map = {
                    "atto": "organic_dye",
                ...
                canonical = mmfdb_type_name_map.get(short_name, short_name)
                if canonical not in mmfdb_types:
                    ...
                    if existing:
                        mmfdb_types[canonical] = existing[0]["type_id"]
                    else:
                        mmfdb_types[canonical] = self.dao.insert(
                            "probe_types",
                            {"type_name": canonical, "display_name": canonical.replace("_", " ").title()},
                        )
                return mmfdb_types[canonical]
            ...
            for src_p in src_probes:
                ...
                # DB and the live MMFDB stay aligned.
                src_category = ""
                ...
                mmfdb_type_id = _resolve_mmfdb_type(int(src_p["type_id"]))
