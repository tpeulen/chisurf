"""Lifecycle state-machine access for :class:`~mmfdb.repository.MFDatabase`.

Provides the ``mmfdb_state_transition`` surface (current state, history, and
...
``mmfdb.lifecycle.lifecycle``; this is the per-entity transition log.
"""
...
class LifecycleMixin:
    ...
    def get_state(self, entity_type: str, entity_id: str) -> str | None:
        ...
        row = self.conn.execute(
            "SELECT to_state FROM mmfdb_state_transition "
    ...
    def get_state_history(
        ...
        rows = self.conn.execute(
            ...
            "reason, operator_user_id, created_at FROM mmfdb_state_transition "
            "WHERE entity_type = ? AND entity_id = ? AND deleted_at IS NULL "
    ...
    def _state_transition_allowed(
        ...
        """
        row = self.conn.execute(
            "SELECT 1 FROM mmfdb_state_transition_rule "
    ...
    def transition_state(
        ...
        *,
        ...
        Validates against ``mmfdb_state_transition_rule`` (raises
        :class:`~mmfdb.lifecycle.lifecycle.StateTransitionError` on an illegal jump,
        ...
        if not self._state_transition_allowed(entity_type, current, to_state):
            raise StateTransitionError(
                ...
                "(no matching mmfdb_state_transition_rule)"
            )
        now = _utc_now()
        with self._transaction():
            next_id = (
                self.conn.execute(
                    "SELECT COALESCE(MAX(transition_id), 0) FROM mmfdb_state_transition"
                ).fetchone()[0]
                + 1
            )
            self.conn.execute(
        ...
        from mmfdb.lifecycle.events import EVENT_STATE_CHANGED, publish
