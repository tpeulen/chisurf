...
through the orthogonal ``mmfdb_reagent_usage`` table, so a result ties to exactly what
was used. Dictionary-declared schema (``mmfdb_reagent_lot`` / ``mmfdb_reagent_usage``);
these are thin free functions over an MMFDB handle.
"""
...
#: Extensible reagent classes (mirrors the ``mmfdb_reagent_lot.kind`` enum).
REAGENT_KINDS: frozenset[str] = frozenset(
    {"fluorophore", "buffer", "optical_filter", "kit", "other"}
)
#: What a lot can be linked to (mirrors the ``mmfdb_reagent_usage.target_type`` enum).
...
def add_reagent_lot(
    ...
    *,
    ...
    with db.transaction():
        db.conn.execute(
            "INSERT INTO mmfdb_reagent_lot (lot_id, kind, name, vendor, catalog_no, "
...
def link_reagent(
    ...
    with db.transaction():
        exists = db.conn.execute(
            "SELECT 1 FROM mmfdb_reagent_usage WHERE lot_id = ? AND target_type = ? "
        ...
        if exists:
            return
        db.conn.execute(
            "INSERT INTO mmfdb_reagent_usage (usage_row_id, lot_id, target_type, "
            "target_id, role, created_at, updated_at, deleted_at) "
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            (_next_id(db, "mmfdb_reagent_usage", "usage_row_id"), lot_id, target_type,
...
def list_reagents_for(db: Any, target_type: str, target_id: str) -> list[dict[str, Any]]:
    ...
    rows = db.conn.execute(
        "SELECT l.* FROM mmfdb_reagent_lot l "
        "JOIN mmfdb_reagent_usage u ON u.lot_id = l.lot_id "
...
def list_lots(
    ...
    *,
    ...
    rows = db.conn.execute(
        f"SELECT * FROM mmfdb_reagent_lot WHERE {' AND '.join(where)} ORDER BY kind, name",
        params,
    ).fetchall()
    return [dict(r) for r in rows]
