"""Simple API for plugins to register results in MMFDB."""
...
from mmfdb.provenance.operation_parameters import OperationParameterError
...
def _resolve_active_user_id() -> str:
    """Resolve the active MMFDB user ID from settings.
...
def register_result(
    ...
    setup_id : str, optional
        MMFDB setup identifier associated with the operation.
    ...
    if db is None:
        db = _get_global_db()
    if db is None:
        logger.warning("register_result: no MMFDB available; result not registered (kind=%s)", kind)
        return ""
    ...
    try:
        ...
        if parameters:
            from mmfdb.provenance.operation_parameters import validate_operation_parameters
...
def register_operation(
    ...
    (``mmfdb_operation_artifact``), ``derived_from`` edges (each output ← each
    input), and its parameters — validated against the operation type's `.dic`
    schema (``mmfdb_operation_parameter_def``) when ``validate`` is set. Parameters
    ...
    Returns the operation_id, or ``""`` when MMFDB is unavailable.
    """
    db = db if db is not None else _get_global_db()
    if db is None:
        logger.warning("register_operation: no MMFDB available (operation_type=%s)", operation_type)
        return ""
...
def register_raw_measurement(
    ...
    setup_id : str, optional
        MMFDB setup identifier associated with the measurement import.
    setup_version : int, optional
        Setup version stored in metadata for traceability.
    db : MMFDBClientBase, optional
    ...
    -------
    str
        Created artifact identifier, or ``""`` on optional MMFDB failure.
    """
...
def read_result(db: MMFDBClientBase, artifact_id: str) -> Any:
    ...
    if artifact.get("data_format") == "msgpack":
        from mmfdb.store.payload_codec import REGISTRY, PayloadSchemaError, decode_payload
...
def _validate_links(db: MMFDBClientBase, sample_id: str, parent_artifact_id: str) -> None:
...
def _store_data(
    ...
    kind : str
        MMFDB artifact kind requested by the caller.
    ...
    from mmfdb.store.payload_codec import encode_payload
...
def _coerce_payload(kind: str, data: Any, pandas_module: Any = None) -> tuple[str, Any]:
    ...
    """
    from mmfdb.store.payload_codec import REGISTRY
    from mmfdb.store.payload_models import BurstTable, GenericCurve, GenericTable
...
def _payload_from_dataframe(kind: str, df: Any) -> Any:
    ...
    """
    from mmfdb.store.payload_models import (
        BurstSelection,
        FcsCorrelation,
        Spectrum,
        TcspcDecay,
...
def _record_parameters(db: MMFDBClientBase, operation_id: str, parameters: dict[str, Any]) -> None:
    """Write parameters to ``mmfdb_parameter``.
...
def _record_one_parameter(
    db: MMFDBClientBase, operation_id: str, name: str, value: Any, role: str | None
) -> None:
    """Record a single ``mmfdb_parameter`` row, with an optional role index."""
