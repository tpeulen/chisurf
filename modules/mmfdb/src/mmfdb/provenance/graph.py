...
def get_canonical_neighbors(
    ...
    """Get direct neighbor edges and nodes in the specified direction for canonical MMFDB tables.
    ...
    if direction == "upstream":
        if norm_type == "artifact":
            ...
            for r in rows:
                op_type = "operation"
                op_row = conn.execute("SELECT operation_type FROM mmfdb_operation WHERE operation_id = ?", (r["operation_id"],)).fetchone()
                if op_row:
            ...
            for r in rows:
                art_type = "artifact"
                art_row = conn.execute("SELECT artifact_kind FROM mmfdb_artifact WHERE artifact_id = ?", (r["artifact_id"],)).fetchone()
                if art_row:
        ...
        # Search mmfdb_edge for non-operation relationships only.
        # Operation input/output (produced, input_to) is already covered by
        # mmfdb_operation_artifact above — including them from mmfdb_edge would
        ...
        if norm_type == "artifact":
            ...
            rows = conn.execute(query, (node_id,)).fetchall()
            op_type = "operation"
            op_row = conn.execute("SELECT operation_type FROM mmfdb_operation WHERE operation_id = ?", (node_id,)).fetchone()
            if op_row:
                op_type = op_row["operation_type"]
            for r in rows:
                art_type = "artifact"
