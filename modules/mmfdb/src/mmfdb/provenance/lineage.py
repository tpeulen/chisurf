...
`mmfdb_operation_artifact`/`mmfdb_edge` SQL for "what produced this", "what derives from
this", and "what used this".

The authoritative source is `mmfdb_operation_artifact` (the PRD-11 operation node's
...
`mmfdb_edge` rows, which this service does not depend on).
...
#: ``mmfdb_edge`` relationship types meaning "the *source* node was produced or
...
#: derivation is followed natively via ``mmfdb_operation_artifact`` — as are the
...
class Lineage:
    """Read-only lineage queries over ``mmfdb_operation_artifact``."""
    ...
    def _producing_ops(self, artifact_id: str) -> list[str]:
        ...
        return self._column(
            "SELECT DISTINCT operation_id FROM mmfdb_operation_artifact "
    ...
    def _op_inputs(self, operation_id: str) -> list[str]:
        return self._column(
            "SELECT DISTINCT artifact_id FROM mmfdb_operation_artifact "
    ...
    def what_used(self, node_id: str, *, max_depth: int = 100) -> list[str]:
        ...
        - the consumers reached over :data:`USAGE_RELATIONSHIPS` ``mmfdb_edge`` links
    ...
    def _edge_referrers(
        ...
        rows = self.conn.execute(
            "SELECT DISTINCT source_node_type, source_node_id FROM mmfdb_edge "
    ...
    def impact_of(
        ...
        *,
        ...
        ``mmfdb_edge`` usage relationships to consumers (operations contribute their
    ...
    def _artifact_label(self, artifact_id: str) -> str:
        row = self.conn.execute(
            "SELECT artifact_kind FROM mmfdb_artifact WHERE artifact_id = ?",
            (artifact_id,),
        ).fetchone()
    ...
    def _operation_label(self, operation_id: str) -> str:
        row = self.conn.execute(
            "SELECT operation_type FROM mmfdb_operation WHERE operation_id = ?",
            (operation_id,),
        ).fetchone()
        return (row[0] if row else "") or ""
