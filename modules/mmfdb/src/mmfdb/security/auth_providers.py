"""Pluggable authentication providers for MMFDB (PRD-59).
...
credentials. :mod:`mmfdb.security.login` orchestrates the rest: mapping the
identity onto an MMFDB user (matching / JIT-provisioning), syncing groups, and
minting a session.
...
any optional third-party import lazy so ``mmfdb`` still imports with only ``src``
on the path.
"""
...
class AuthIdentity:
    ...
    external_id : str
        Provider-native user identifier. For ``local`` this is the MMFDB
        ``user_id``; for LDAP the directory ``uid`` (used as the MMFDB user id on
        JIT provisioning and stored in ``flr_sample_users.external_id``).
    email : str or None
        Email address, used as the secondary match key.
    display_name : str or None
        Human-readable name for the (possibly JIT-provisioned) MMFDB user.
    is_admin : bool
        Whether the provider considers this identity an administrator.
    groups : tuple of str
        MMFDB group ids this identity currently belongs to (already mapped from the
        provider's native groups). Empty for ``local`` — local group membership
        is managed inside MMFDB and is not re-synced on login.
    managed_groups : tuple of str
        The full universe of MMFDB group ids this provider **authoritatively
...
def _require_ldap3():
    ...
    try:
        ...
        raise ImportError(
            ...
            "Install it (e.g. `pip install ldap3` or the mmfdb '[ldap]' extra)."
        ) from exc
    return ldap3
...
class LdapAuthProvider:
    ...
    (``memberOf``) are mapped to MMFDB group ids via ``group_map`` and to admin
    ...
    (ldap group → mmfdb group id), ``admin_groups`` (ldap groups granting admin).
    """
    ...
    def _map_groups(self, memberships: list[str]) -> tuple[tuple[str, ...], bool]:
        ...
        mmfdb_groups: list[str] = []
        is_admin = False
        for member in memberships:
            mapped = group_map.get(member)
            if mapped and mapped not in mmfdb_groups:
                mmfdb_groups.append(mapped)
            if member in admin_groups:
                is_admin = True
        return tuple(mmfdb_groups), is_admin

    def _managed_groups(self) -> tuple[str, ...]:
        """Return the full universe of MMFDB groups this directory config controls."""
    ...
    def authenticate(self, *, user_id: str, password: str = "") -> AuthIdentity | None:
        ...
        :class:`~mmfdb.security.auth.AuthError` only on misconfiguration (a failed
        service bind).
        """
