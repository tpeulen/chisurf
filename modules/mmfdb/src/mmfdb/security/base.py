"""Abstract MMFDB client contract for repository and remote implementations."""
...
class MMFDBClientBase(ABC):
    """Minimal MMFDB client interface used by plugins and registry helpers.

    Implementations may be local SQLite repositories, RPC clients, or test
    doubles. The interface intentionally exposes MMFDB operations, not SQLite
    internals, so plugin code does not depend on table layout details.
    """

    @abstractmethod
    def transaction(self) -> AbstractContextManager[Any]:
        """Return a transaction context manager for grouped MMFDB writes."""
    ...
    def put_object(
        ...
        """Store bytes or a file in the MMFDB object store."""
    ...
    def record_operation(self, operation_id: str, operation_type: str, **kwargs: Any) -> str:
        """Register or update an MMFDB operation."""
    ...
    def add_edge(
        ...
        **kwargs: Any,
        ...
        """Add a canonical provenance edge between MMFDB nodes."""
