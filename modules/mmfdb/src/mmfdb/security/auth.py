"""MMFDB authorization core: principals, token auth, permission evaluation, ACL mutation."""
...
def _dao(conn: sqlite3.Connection):
    ...
    (all MMFDB databases share the dictionary-generated schema), so the per-call
...
def authenticate_token(conn: sqlite3.Connection, token: str) -> Principal:
    ...
    _dao(conn).update(
        "mmfdb_session", token_hash, {"last_used_at": now}, pk_column="token_hash"
    )
    conn.commit()
...
def _user_group_ids(
    ...
    """Return all group IDs the user belongs to (via mmfdb_group_member)."""
    return [
        row["group_id"]
        for row in _dao(conn).list("mmfdb_group_member", filters={"user_id": user_id})
    ]
...
def _check_acls(
    ...
    """
    rows = _dao(conn).list(
        "mmfdb_acl_entry", filters={"object_type": object_type, "object_id": object_id}
    )
...
def _get_object_acl(
    ...
    while current_type and current_id:
        ...
        # Follow inheritance
        if current_type == "mmfdb_object_acl":
            break
...
def revoke_acl(
    ...
    """
    dao = _dao(conn)
    row = dao.get("mmfdb_acl_entry", entry_id, include_deleted=True)
    if row:
        require_access(conn, principal, row["object_type"], row["object_id"], PERM_MANAGE)
        dao.soft_delete("mmfdb_acl_entry", entry_id, deleted_at=_utc_now_iso())
...
def chmod(
    ...
    require_access(conn, principal, object_type, object_id, PERM_MANAGE)
    # raw: composite-key update — mmfdb_object_acl is keyed by UNIQUE(object_type,
    # object_id) with no single PK, which dao.update cannot target.
    conn.execute(
        "UPDATE mmfdb_object_acl SET mode = ?, updated_at = ? WHERE object_type = ? AND object_id = ?",
...
def chown(
    ...
    conn.execute(
        "UPDATE mmfdb_object_acl SET owner_user_id = ?, updated_at = ? WHERE object_type = ? AND object_id = ?",
...
def chgrp(
    ...
    # raw: composite-key update on UNIQUE(object_type, object_id) — see chmod.
    conn.execute(
        "UPDATE mmfdb_object_acl SET owner_group_id = ?, updated_at = ? WHERE object_type = ? AND object_id = ?",
...
def revoke_session(conn: sqlite3.Connection, session_id: str) -> None:
    ...
    _dao(conn).update("mmfdb_session", session_id, {"revoked_at": _utc_now_iso()})
...
def revoke_session_by_token(conn: sqlite3.Connection, token: str) -> None:
    ...
    token_hash = _hash_token(token)
    _dao(conn).update(
        "mmfdb_session", token_hash, {"revoked_at": _utc_now_iso()}, pk_column="token_hash"
    )
