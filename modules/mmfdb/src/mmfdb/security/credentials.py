"""Secure client-side storage for MMFDB session tokens."""
...
def credential_account(server_host: str, server_port: int, user_id: str) -> str:
    """Return the credential-store account key for an MMFDB session.
...
def store_runtime_session_token(server_host: str, server_port: int, user_id: str, token: str) -> bool:
    """Store an MMFDB session token for the current process only.
    ...
    user_id : str
        MMFDB user identifier.
    token : str
        Session token returned by the MMFDB backend.
...
def load_runtime_session_token(server_host: str, server_port: int, user_id: str) -> str | None:
    """Load an MMFDB session token kept for the current process.
...
def delete_runtime_session_token(server_host: str, server_port: int, user_id: str) -> bool:
    """Delete an MMFDB session token kept for the current process.
...
def rename_runtime_session_token(
    ...
    server_port : int
        MMFDB command port.
    old_user_id : str
        Previous MMFDB user identifier.
    new_user_id : str
        New MMFDB user identifier.
...
def store_session_token(server_host: str, server_port: int, user_id: str, token: str) -> bool:
    """Store an MMFDB session token in the OS credential store.
...
def load_session_token(server_host: str, server_port: int, user_id: str) -> str | None:
    """Load an MMFDB session token from the OS credential store.
...
def delete_session_token(server_host: str, server_port: int, user_id: str) -> bool:
    """Delete an MMFDB session token from the OS credential store.
...
LDAP_SERVICE_NAME = "ChiSurf MMFDB LDAP"
...
def store_ldap_bind_password(host: str, bind_dn: str, password: str) -> bool:
    ...
    is present (callers then source the secret from ``MMFDB_LDAP_BIND_PASSWORD`` or
