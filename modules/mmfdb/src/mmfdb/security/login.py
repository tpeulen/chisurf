"""Authentication orchestration: provider → MMFDB user → session (PRD-59).

``login()`` is the single entry point behind the ``mmfdb.security.auth.login`` RPC.
It selects an :class:`~mmfdb.security.auth_providers.AuthProvider`, authenticates,
resolves the provider identity onto an MMFDB ``flr_sample_users`` row (matching by
``(auth_provider, external_id)`` then ``email``, else JIT-provisioning for
external providers), syncs mapped groups into ``mmfdb_group_member``, and mints a
session via the existing :func:`~mmfdb.security.auth.create_session`.
...
def resolve_provider(
    ...
    *,
    ...
    MMFDB supports two providers: ``"local"`` (always available) and ``"ldap"``
    (its optional ``ldap3`` dependency is imported only when authenticating).
    Raises :class:`~mmfdb.security.auth.AuthError` for any other name.
    """
...
def _jit_enabled(config: dict[str, Any] | None) -> bool:
    """Whether external providers may JIT-provision an MMFDB user (default True)."""
    if not config:
        return True
...
def resolve_or_provision_user(
    ...
    *,
    ...
    """Map *identity* to an MMFDB ``user_id``.

    ``local`` identities are their own MMFDB user. External identities are matched
...
def _provision_user(conn: sqlite3.Connection, identity: AuthIdentity) -> str:
    """Create a new MMFDB user row from an external *identity* and base groups."""
    dao = _dao(conn)
...
def _remove_group_member(conn: sqlite3.Connection, group_id: str, user_id: str) -> None:
    ...
    conn.execute(
        "UPDATE mmfdb_group_member SET deleted_at = ? "
...
def sync_groups(conn: sqlite3.Connection, user_id: str, group_ids: tuple[str, ...]) -> None:
    """Ensure *user_id* belongs to each mapped MMFDB group id (additive)."""
...
def login(
    ...
    *,
    ...
    Raises :class:`~mmfdb.security.auth.AuthError` on throttle or bad credentials.
    ...
    sync_identity(conn, mmfdb_user_id, identity)
    ...
    session = create_session(
        conn,
        user_id=mmfdb_user_id,
        client_host=client_host,
        client_name=client_name,
        client_metadata=client_metadata,
    )
    record_auth_attempt(conn, mmfdb_user_id, True)
    conn.commit()
