"""Canonical identity / session context for MMFDB (PRD-17).
...
``cs_settings["mmfdb"]["default_user_id"]`` was read independently in ~10 sites and
...
from mmfdb.config import configured_default_user_id as _configured_default_user_id
...
def resolve_active_user_id(auth: dict[str, Any] | None = None, *, conn: Any = None) -> str:
    ...
    back to the configured ``mmfdb.default_user_id`` (then :data:`DEFAULT_USER_ID`).
    ...
    if auth is not None and conn is not None:
        try:
            from mmfdb.security.auth import AnonymousPrincipal, principal_from_rpc_auth
