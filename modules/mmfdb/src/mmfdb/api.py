...
from mmfdb.request_context import (
    APIRequestContext,
    bind_api_context,
    current_api_context,
    open_api_context,
...
@contextmanager
def _database(auth: dict[str, Any] | None) -> Iterator[MFDatabase]:
    """Borrow the request database, or own one for a direct API call."""
    context = current_api_context()
    if context is not None:
        yield context.database
        return
    with (
        open_api_context(auth, resolve_database_path()) as context,
        bind_api_context(context),
    ):
        yield context.database
...
def invoke_with_context(
    ...
    """Invoke an API handler using caller-owned request resources."""
    with bind_api_context(context):
...
def _check_api_auth(auth: dict[str, Any] | None) -> tuple[str | None, bool]:
...
def register_artifact(
    ...
    """Register or update an MMFDB artifact in the database.
...
def graph_upstream(
    ...
    with _database(auth) as db:
        ...
        from mmfdb.provenance.graph import traverse_canonical_graph as traverse
        edges = traverse(
            db.conn,
            node_type,
            node_id,
...
def save_chinet_session(
    ...
    """Save a canonical chinet session schema payload to MMFDB.
...
def get_chinet_session(artifact_id: str, auth: dict[str, Any] | None = None) -> dict[str, Any]:
    """Load a chinet session artifact from MMFDB.
...
def list_chinet_sessions(
    ...
    """List chinet session artifacts in MMFDB.
...
def restore_chinet_session(artifact_id: str, auth: dict[str, Any] | None = None) -> dict[str, Any]:
    """Restore a chinet session artifact from MMFDB.
