...
- per-entity-type **state vocabularies** in ``mmfdb_vocabulary``
  (``field_name = "state:<entity_type>"``), and
- the allowed **transition rules** in ``mmfdb_state_transition_rule``
  (``entity_type, from_state, to_state``; a null ``from_state`` is an allowed
  initial state).

The transition *log* (``mmfdb_state_transition``) is the source of truth for an
...
#: ``mmfdb_vocabulary.field_name`` for an entity type's states.
...
def bootstrap_lifecycle_defs(conn: sqlite3.Connection) -> None:
    ...
    States go into ``mmfdb_vocabulary`` (``field_name = "state:<entity_type>"``,
    ...
    try:
        with conn:
            for entity_type, ld in defs.items():
                ...
                conn.execute(
                    "DELETE FROM mmfdb_state_transition_rule WHERE entity_type = ?",
                    (entity_type,),
                )
            next_id = (
                conn.execute(
                    "SELECT COALESCE(MAX(rule_id), 0) FROM mmfdb_state_transition_rule"
                ).fetchone()[0]
                + 1
            )
            for entity_type, ld in defs.items():
                for from_state, to_state in ld.transitions:
                    conn.execute(
                        """INSERT INTO mmfdb_state_transition_rule
