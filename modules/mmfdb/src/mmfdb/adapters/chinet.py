...
#: in-process object registry, so the transparent "MMFDB-as-chinet-backend" integration
#: (connect_to_db("mmfdb", …) → write_to_db/read_from_db) is unsupported until it is
...
_MMFDB_BACKEND_UNSUPPORTED = (
    "The transparent MMFDB-as-chinet-backend integration requires chinet's removed "
    "DB.set_backend API (chinet <0.3); use store_chinet_session/load_chinet_session "
    "instead, or redesign MMFDBChinetBackend against chinet's current DB registry."
)
...
ParameterNameResolver = Callable[[str], str | None]
FitSerializer = Callable[[Any], dict[str, Any]]


def _lookup_flrcif_name(
    short_name: str,
    resolver: ParameterNameResolver | None = None,
) -> str | None:
    """Resolve a host parameter name without importing the host application.

    Parameters
    ----------
    short_name : str
        Host parameter short name.
    resolver : callable or None, optional
        Application-provided short-name resolver.
...
def _ensure_vocabulary(db: MFDatabase) -> None:
    """Ensure chinet vocabulary values are active in MMFDB.
...
def _artifact_id(prefix: str, object_id: str) -> str:
    """Build a stable MMFDB artifact identifier.
...
def _validate_parameter_payload(parameter: dict[str, Any]) -> None:
    """Validate an explicit MMFDB parameter payload before writing it.
...
def _validate_extensible_vocab(db: MFDatabase, field_name: str, value: str | None) -> None:
    """Validate an extensible MMFDB vocabulary value before writes.
    ...
    ------
    ValueError
        If the value is not active in MMFDB vocabulary.
    """
    if value is None:
        return
...
def _parameter_uuid_for_port(port: Any, explicit: dict[str, Any] | None = None) -> str:
    """Return the MMFDB parameter UUID for a chinet port.
...
def _parameter_payload_for_port(
    ...
    parameter_name_resolver: ParameterNameResolver | None = None,
) -> dict[str, Any]:
    """Build an MMFDB parameter payload for a chinet port.
    ...
    canonical_name = _lookup_flrcif_name(short_name, parameter_name_resolver) or short_name
...
def _store_parameters(
    ...
    """Store chinet port parameters in MMFDB.
...
def _find_session_artifact_id(db: MFDatabase, session_id: str) -> str | None:
    """Find the MMFDB artifact identifier for a chinet session id."""
...
class MMFDBChinetBackend:
    """Transparent MMFDB backend for chinet object persistence."""
    ...
    def close(self) -> None:
        """Close the underlying MMFDB connection."""
        self.db.close()

    def connect_object(self, obj: Any, *args: Any, **kwargs: Any) -> bool:
        """Connect a chinet object to the configured MMFDB backend."""
        client = _require_chinet()
    ...
    def disconnect_object(self, obj: Any) -> bool:
        """Disconnect a chinet object from the configured MMFDB backend."""
        return True

    def read_object(self, obj: Any, oid: str) -> bool:
        """Read a chinet session from MMFDB into ``obj``."""
        client = _require_chinet()
    ...
    def write_object(self, obj: Any) -> bool:
        """Write a chinet session, or its owning session, to MMFDB."""
        client = _require_chinet()
        session = obj if hasattr(obj, "nodes") else _find_owning_session(obj)
        if session is None:
            client.DB.register(obj)
...
def configure_mmfdb_backend(
    ...
    """Configure chinet transparent persistence through MMFDB.

    Parameters
    ----------
    db : MFDatabase or None, optional
        Existing MMFDB connection.
    db_path : str or None, optional
        SQLite MMFDB path used when ``db`` is omitted.
    operation_id : str or None, optional
        Default MMFDB operation identifier for writes.
    ...
    if not hasattr(client.DB, "set_backend"):
        raise NotImplementedError(_MMFDB_BACKEND_UNSUPPORTED)
    backend = MMFDBChinetBackend(
...
def clear_mmfdb_backend(close: bool = False) -> None:
    """Remove the configured chinet MMFDB backend.

    Parameters
    ----------
    close : bool, default=False
        Whether to close the backend MMFDB connection.
    """
    client = _require_chinet()
...
def store_chinet_session(
    ...
    """Store a chinet session and related parameters in MMFDB.
    ...
    with db.transaction():
        ...
        parameter_ids, port_mapping = _store_parameters(
            db,
            operation_id,
            session,
            parameters,
...
def load_chinet_session(db: MFDatabase, artifact_id: str) -> Any:
    """Load and reconstruct a chinet session from an MMFDB artifact.
...
def archive_fit_to_mmfdb(
    ...
    *,
    fit_serializer: FitSerializer,
    parameter_name_resolver: ParameterNameResolver | None = None,
) -> dict[str, Any]:
    """Archive one ChiSurf fit and its chinet-backed parameter graph to MMFDB.
    ...
    fit_state_payload = fit_serializer(fit)
    ...
    with db.transaction():
        result = store_chinet_session(
            ...
            parameter_name_resolver=parameter_name_resolver,
        )
        ...
        parameter_ids = _store_fit_state_parameters(
            db,
            operation_id,
            fit_state_payload,
            parameters,
...
def _store_fit_state_parameters(
    ...
    """Store fit-state parameters in MMFDB.
