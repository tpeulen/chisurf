"""Generate an MMFDB schema/API reference from the dictionary (PRD-26 Task 5).
...
Usage
-----
>>> from mmfdb.schema.docs_generator import generate_schema_reference
>>> md = generate_schema_reference()                       # all flr_/mmfdb_ tables
...
#: ``mmfdb_operation_parameter_def``); rendered here as the canonical reference.
...
_MMFDB_TABLE_PREFIXES = ("flr_", "mmfdb_")
...
def _wanted(table: str, tables: set[str] | None) -> bool:
    ...
    return table.startswith(_MMFDB_TABLE_PREFIXES)
...
def collect_schema(
    ...
    otherwise every ``flr_*``/``mmfdb_*`` table the dictionary declares is included.
    """
...
def generate_schema_reference(
    *,
    ...
    """Render the MMFDB schema/API reference as Markdown from the dictionary."""
    ...
    lines = [
        "# MMFDB schema reference",
        "",
        "_Generated from the flrCIF + `mmfdb_flr_ext.dic` dictionary by "
        "`mmfdb.schema.docs_generator` (PRD-26). Do not edit by hand — regenerate._",
        "",
        f"Tables: {len(schema)} · "
