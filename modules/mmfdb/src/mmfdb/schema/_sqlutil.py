"""Pure JSON / time helpers shared by the MMFDB repository and its query mixins.

These were defined inside ``repository.py``. They are extracted here so the
per-concern query mixins (``mmfdb.queries.*``) can use them without importing back
...
Callers import these helpers from this module; the repository composition root
does not re-export private persistence utilities.
"""
...
# underscore-prefixed names from ``mmfdb.repository``, which re-exports these.
