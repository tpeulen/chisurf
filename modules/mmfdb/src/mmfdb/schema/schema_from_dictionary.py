...
setup tables (mmfdb_setup_detector_channel, mmfdb_setup_pie_window) and the
TTTR-reading columns on mmfdb_setup.  Every column, type, PK, FK, and
default is derived from the ``_mmfdb_schema`` bridge attributes.
...
def _resolve_table_name(category_name: str, dic: MmcifDictionary) -> str:
    ...
    Uses the first item's ``_mmfdb_schema.table_name``, or falls back
...
def generate_create_table_for_category(
    ...
    dic : MmcifDictionary
        Loaded dictionary.
    category_name : str
        Category name (e.g. ``"mmfdb_setup_detector_channel"``).
...
def _category_dependency_order(categories: list[str]) -> list[str]:
    ...
    roots = [
        "mmfdb_object", "mmfdb_setup", "mmfdb_artifact", "mmfdb_operation",
        "mmfdb_branch", "mmfdb_group",
    ]
    for r in roots:
        if r in categories:
            ordered.append(r)
