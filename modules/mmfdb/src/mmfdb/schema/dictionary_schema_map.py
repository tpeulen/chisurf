...
class MappedColumn:
    """Mapping from one dictionary item to one MMFDB table column."""
...
class DictionarySchemaMap:
    """Map dictionary items to live MMFDB columns.

    Mapping rules are intentionally mechanical:

    1. If the dictionary item declares ``_mmfdb_schema.table_name`` or
       ``_mmfdb_schema.column_name``, use those values.
    ...
    def categories_without_live_table(self) -> list[str]:
        """Return flrCIF categories absent from the live MMFDB schema."""
        if not self.schema:
            return []
        return sorted(
            category
...
def build_dictionary_schema_map(
    ...
    """Build dictionary-item to MMFDB table/column mappings."""
...
def map_dictionary_item(
    ...
    """Return the mapped MMFDB table/column for one dictionary item."""
