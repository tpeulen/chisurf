...
CREATE_TABLES_SQL = [
    ...
    """CREATE TABLE IF NOT EXISTS flr_experiment (
        ...
        status TEXT,
        details TEXT,
        setup_definition_id TEXT REFERENCES mmfdb_setup(setup_id) ON DELETE SET NULL,
        created_at TEXT DEFAULT CURRENT_TIMESTAMP,
        updated_at TEXT DEFAULT CURRENT_TIMESTAMP,
        deleted_at TEXT
    )""",
    ...
    # Legacy fdb_setup (preserve as source for v17 backfill; canonical target is mmfdb_setup)
    # Canonical FDB Tables (v17 target architecture — preserved for migration backfill compat)
    # Canonical MMFDB Tables (v18 target architecture)
    ...
    """CREATE TABLE IF NOT EXISTS mmfdb_artifact (
        ...
        data_json TEXT,
        data_blob BLOB,
        object_uuid TEXT REFERENCES mmfdb_object(object_uuid),
    ...
    """CREATE TABLE IF NOT EXISTS mmfdb_operation (
        operation_id TEXT PRIMARY KEY,
        operation_type TEXT NOT NULL,
        experiment_id TEXT REFERENCES flr_experiment(experiment_id) ON DELETE CASCADE,
        setup_id TEXT REFERENCES mmfdb_setup(setup_id) ON DELETE SET NULL,
        settings_json TEXT,
        settings_hash TEXT,
    ...
    """CREATE TABLE IF NOT EXISTS mmfdb_operation_artifact (
        operation_id TEXT NOT NULL REFERENCES mmfdb_operation(operation_id) ON DELETE CASCADE,
        artifact_id TEXT NOT NULL REFERENCES mmfdb_artifact(artifact_id) ON DELETE CASCADE,
    ...
    """CREATE TABLE IF NOT EXISTS mmfdb_edge (
        ...
        operation_id TEXT REFERENCES mmfdb_operation(operation_id) ON DELETE SET NULL,
        settings_hash TEXT,
    ...
    """CREATE TABLE IF NOT EXISTS mmfdb_parameter (
        parameter_id INTEGER PRIMARY KEY AUTOINCREMENT,
        parameter_uuid TEXT UNIQUE NOT NULL,
        operation_id TEXT REFERENCES mmfdb_operation(operation_id) ON DELETE CASCADE,
        name TEXT NOT NULL,
        value REAL,
        standard_error REAL,
    ...
    _get_dict_ddl("mmfdb_setup_detector_channel"),
    ...
    # New MMFDB Tables (v18)
    """CREATE TABLE IF NOT EXISTS mmfdb_schema_version (version INTEGER)""",
    """CREATE TABLE IF NOT EXISTS mmfdb_vocabulary (
    ...
    """CREATE TABLE IF NOT EXISTS mmfdb_branch (
        branch_uuid TEXT PRIMARY KEY,
        name TEXT NOT NULL UNIQUE,
        description TEXT,
        parent_branch_uuid TEXT REFERENCES mmfdb_branch(branch_uuid),
        head_operation_id TEXT REFERENCES mmfdb_operation(operation_id),
        created_by_user_id TEXT REFERENCES flr_sample_users(user_id),
        created_at TEXT DEFAULT CURRENT_TIMESTAMP,
        updated_at TEXT DEFAULT CURRENT_TIMESTAMP,
        deleted_at TEXT
    ...
    # Auth tables (v25)
    """CREATE TABLE IF NOT EXISTS mmfdb_group (
    ...
    """CREATE TABLE IF NOT EXISTS mmfdb_group_member (
        group_id TEXT NOT NULL REFERENCES mmfdb_group(group_id),
        user_id TEXT NOT NULL REFERENCES flr_sample_users(user_id),
    ...
    """CREATE TABLE IF NOT EXISTS mmfdb_object_acl (
        object_type TEXT NOT NULL,
        object_id TEXT NOT NULL,
        owner_user_id TEXT NOT NULL REFERENCES flr_sample_users(user_id),
        owner_group_id TEXT REFERENCES mmfdb_group(group_id),
    ...
    """CREATE TABLE IF NOT EXISTS mmfdb_acl_entry (
    ...
    """CREATE TABLE IF NOT EXISTS mmfdb_session (
    ...
    """CREATE TABLE IF NOT EXISTS mmfdb_auth_attempt (
...
# hand-sync requirement (DATA-03).  Every canonical mmfdb_* table is
...
_CANONICAL_TABLE_DEFS: dict[str, _TableDef] = {
    ...
    "mmfdb_artifact": _TableDef([
        ...
        _Column("object_uuid", "TEXT", ref="mmfdb_object(object_uuid)"),
    ...
    "mmfdb_operation": _TableDef([
        ...
        _Column("setup_id", "TEXT", ref="mmfdb_setup(setup_id) ON DELETE SET NULL"),
    ...
    "mmfdb_operation_artifact": _TableDef([
        _Column("operation_id", "TEXT", nullable=False, ref="mmfdb_operation(operation_id) ON DELETE CASCADE"),
        _Column("artifact_id", "TEXT", nullable=False, ref="mmfdb_artifact(artifact_id) ON DELETE CASCADE"),
    ...
    "mmfdb_edge": _TableDef([
        ...
        _Column("operation_id", "TEXT", ref="mmfdb_operation(operation_id) ON DELETE SET NULL"),
        _Column("settings_hash", "TEXT"),
    ...
    "mmfdb_parameter": _TableDef([
        ...
        _Column("operation_id", "TEXT", ref="mmfdb_operation(operation_id) ON DELETE CASCADE"),
...
# for canonical mmfdb_* tables (so production DBs have validation).
...
CREATE_INDICES_SQL = [
    ...
    # Canonical MMFDB Indices
    "CREATE INDEX IF NOT EXISTS idx_mmfdb_artifact_kind ON mmfdb_artifact (artifact_kind)",
    "CREATE INDEX IF NOT EXISTS idx_mmfdb_artifact_data_format ON mmfdb_artifact (data_format)",
    "CREATE INDEX IF NOT EXISTS idx_mmfdb_artifact_experiment ON mmfdb_artifact (experiment_id)",
    "CREATE INDEX IF NOT EXISTS idx_mmfdb_operation_type ON mmfdb_operation (operation_type)",
    "CREATE INDEX IF NOT EXISTS idx_mmfdb_operation_experiment ON mmfdb_operation (experiment_id)",
    "CREATE INDEX IF NOT EXISTS idx_mmfdb_operation_setup ON mmfdb_operation (setup_id)",
    "CREATE INDEX IF NOT EXISTS idx_mmfdb_operation_status ON mmfdb_operation (status)",
    "CREATE INDEX IF NOT EXISTS idx_mmfdb_op_art_op ON mmfdb_operation_artifact (operation_id)",
    "CREATE INDEX IF NOT EXISTS idx_mmfdb_op_art_art ON mmfdb_operation_artifact (artifact_id)",
    "CREATE INDEX IF NOT EXISTS idx_mmfdb_edge_source ON mmfdb_edge (source_node_type, source_node_id)",
    "CREATE INDEX IF NOT EXISTS idx_mmfdb_edge_target ON mmfdb_edge (target_node_type, target_node_id)",
    "CREATE INDEX IF NOT EXISTS idx_mmfdb_edge_operation ON mmfdb_edge (operation_id)",
    "CREATE INDEX IF NOT EXISTS idx_mmfdb_parameter_operation ON mmfdb_parameter (operation_id)",
    "CREATE INDEX IF NOT EXISTS idx_mmfdb_parameter_uuid ON mmfdb_parameter (parameter_uuid)",
    "CREATE INDEX IF NOT EXISTS idx_mmfdb_setup_instrument ON mmfdb_setup (instrument_id)",
    "CREATE INDEX IF NOT EXISTS idx_mmfdb_setup_detector_channel_setup ON mmfdb_setup_detector_channel (setup_id)",
    "CREATE INDEX IF NOT EXISTS idx_mmfdb_setup_pie_window_setup ON mmfdb_setup_pie_window (setup_id)",
    ...
    # (mmfdb_sample/mmfdb_experiment indices removed in Phase 3 — tables are duplicates of flr_*)
    "CREATE INDEX IF NOT EXISTS idx_mmfdb_vocabulary_field ON mmfdb_vocabulary (field_name)",
    "CREATE INDEX IF NOT EXISTS idx_mmfdb_vocabulary_active ON mmfdb_vocabulary (is_active)",
    ...
    "CREATE INDEX IF NOT EXISTS idx_mmfdb_artifact_deleted_at ON mmfdb_artifact (deleted_at)",
    "CREATE INDEX IF NOT EXISTS idx_mmfdb_operation_deleted_at ON mmfdb_operation (deleted_at)",
    "CREATE INDEX IF NOT EXISTS idx_mmfdb_operation_artifact_deleted_at ON mmfdb_operation_artifact (deleted_at)",
    "CREATE INDEX IF NOT EXISTS idx_mmfdb_edge_deleted_at ON mmfdb_edge (deleted_at)",
    "CREATE INDEX IF NOT EXISTS idx_mmfdb_parameter_deleted_at ON mmfdb_parameter (deleted_at)",
    "CREATE INDEX IF NOT EXISTS idx_mmfdb_setup_deleted_at ON mmfdb_setup (deleted_at)",
    "CREATE INDEX IF NOT EXISTS idx_mmfdb_audit_log_deleted_at ON mmfdb_audit_log (deleted_at)",
    # (mmfdb_sample/mmfdb_experiment lifecycle indices removed in Phase 3)
    "CREATE INDEX IF NOT EXISTS idx_mmfdb_vocabulary_deleted_at ON mmfdb_vocabulary (deleted_at)",
    "CREATE INDEX IF NOT EXISTS idx_mmfdb_branch_deleted_at ON mmfdb_branch (deleted_at)",
    # Auth indices (v25)
    "CREATE INDEX IF NOT EXISTS idx_mmfdb_group_member_group ON mmfdb_group_member (group_id)",
    "CREATE INDEX IF NOT EXISTS idx_mmfdb_group_member_user ON mmfdb_group_member (user_id)",
    "CREATE INDEX IF NOT EXISTS idx_mmfdb_object_acl_owner ON mmfdb_object_acl (owner_user_id)",
    "CREATE INDEX IF NOT EXISTS idx_mmfdb_object_acl_owner_group ON mmfdb_object_acl (owner_group_id)",
    "CREATE INDEX IF NOT EXISTS idx_mmfdb_object_acl_object ON mmfdb_object_acl (object_type, object_id)",
    "CREATE INDEX IF NOT EXISTS idx_mmfdb_acl_entry_object ON mmfdb_acl_entry (object_type, object_id)",
    "CREATE INDEX IF NOT EXISTS idx_mmfdb_acl_entry_subject ON mmfdb_acl_entry (subject_type, subject_id)",
    "CREATE INDEX IF NOT EXISTS idx_mmfdb_session_token_hash ON mmfdb_session (token_hash)",
    "CREATE INDEX IF NOT EXISTS idx_mmfdb_session_expires ON mmfdb_session (expires_at)",
    "CREATE INDEX IF NOT EXISTS idx_mmfdb_session_user ON mmfdb_session (user_id)",
    "CREATE INDEX IF NOT EXISTS idx_mmfdb_auth_attempt_user ON mmfdb_auth_attempt (user_id, attempted_at)",
    "CREATE INDEX IF NOT EXISTS idx_mmfdb_auth_attempt_host ON mmfdb_auth_attempt (client_host, attempted_at)",
    # Object store indices (v27)
    "CREATE INDEX IF NOT EXISTS idx_mmfdb_object_md5 ON mmfdb_object (content_md5)",
    "CREATE INDEX IF NOT EXISTS idx_mmfdb_object_filename ON mmfdb_object (original_filename)",
    "CREATE INDEX IF NOT EXISTS idx_mmfdb_artifact_object_uuid ON mmfdb_artifact (object_uuid)",
]
...
def _ensure_mmfdb_setup_columns(conn: sqlite3.Connection) -> None:
    ...
    for col, col_type in [
        ...
        _ensure_column(conn, "mmfdb_setup", col, col_type)


def _ensure_user_auth_columns(conn: sqlite3.Connection) -> None:
    """Ensure auth columns exist on legacy ``flr_sample_users`` tables.

    Some user databases were already marked at the current schema version before
    the provider-aware login columns were added.  They skip the migration
    waterfall, so repair this table on every writable open.
    """
    for col, col_type in [
        ("is_admin", "INTEGER DEFAULT 0"),
        ("password_hash", "TEXT"),
...
def reconcile_current_schema(conn: sqlite3.Connection) -> None:
    """Idempotently reconcile a live database against the current code schema.

    During active MMFDB development, user databases can already be stamped with
    ``SCHEMA_VERSION`` while still missing tables or columns introduced by newer
    code.  Versioned migrations skip those databases, so this lightweight repair
    is used by explicit reconciliation migrations.
    """
    for sql in FRESH_DB_TABLES_SQL:
        conn.execute(sql)
    _ensure_canonical_columns(conn)
    from mmfdb.schema.pdbx_metadata import MmcifDictionary
    from mmfdb.schema.schema_from_dictionary import reconcile_schema
    reconcile_schema(conn, MmcifDictionary.load_bundled())
    _ensure_mmfdb_setup_columns(conn)
    _ensure_lifecycle_columns(conn)
    _ensure_user_auth_columns(conn)
    for sql in FRESH_DB_INDICES_SQL + MMFDB_EDGE_VOCABULARY_TRIGGER_SQL:
        try:
            conn.execute(sql)
        except sqlite3.OperationalError:
            pass
...
def _fix_operation_artifact_pk(conn: sqlite3.Connection) -> None:
    """Recreate ``mmfdb_operation_artifact`` with a 4-column composite PK.
    ...
    cur = conn.cursor()
    pk_cols = {
        r[1] for r in cur.execute("PRAGMA table_info(mmfdb_operation_artifact)").fetchall()
    }
    ...
    role_in_pk = any(
        r[1] == "role" and r[5]
        for r in cur.execute("PRAGMA table_info(mmfdb_operation_artifact)").fetchall()
    )
    if role_in_pk:
        return
    logger.info("Migrating mmfdb_operation_artifact: adding 'role' to composite PK")
    cur.execute("BEGIN")
    try:
        cur.execute("CREATE TABLE mmfdb_operation_artifact_new ("
                     "operation_id TEXT NOT NULL REFERENCES mmfdb_operation(operation_id) ON DELETE CASCADE, "
                     "artifact_id TEXT NOT NULL REFERENCES mmfdb_artifact(artifact_id) ON DELETE CASCADE, "
                     "direction TEXT NOT NULL, "
                     "role TEXT NOT NULL DEFAULT 'generic', "
                     ...
                     "PRIMARY KEY (operation_id, artifact_id, direction, role))")
        cur.execute("INSERT INTO mmfdb_operation_artifact_new "
        ...
        cur.execute("DROP TABLE mmfdb_operation_artifact")
        cur.execute("ALTER TABLE mmfdb_operation_artifact_new RENAME TO mmfdb_operation_artifact")
        conn.commit()
        logger.info("mmfdb_operation_artifact PK migration complete")
    except Exception:
        conn.rollback()
        logger.warning("mmfdb_operation_artifact PK migration skipped (table may already be correct)")
...
def _ensure_lifecycle_columns(conn: sqlite3.Connection, now: str | None = None) -> None:
    """Repair lifecycle columns on legacy MMFDB tables.
    ...
    lifecycle_table_config: list[tuple[str, bool, bool]] = [
        ("mmfdb_artifact", True, True),
        ("mmfdb_operation", True, True),
        ("mmfdb_parameter", True, True),
        ("mmfdb_setup", True, True),
        ("mmfdb_setup_detector_channel", True, True),
        ("mmfdb_setup_pie_window", True, True),
        ("mmfdb_setup_fcs_pair", True, True),
        ...
        ("mmfdb_operation_artifact", False, False),
        ("mmfdb_edge", False, False),
        ("mmfdb_vocabulary", False, False),
        ("mmfdb_group", False, False),
        ("mmfdb_group_member", False, False),
        ("mmfdb_object_acl", False, False),
        ("mmfdb_acl_entry", False, False),
        ("mmfdb_session", False, False),
        ("mmfdb_auth_attempt", False, False),
    ]
...
def bootstrap_identity(conn: sqlite3.Connection) -> None:
    ...
    with conn:
        ...
        if not row_main:
            conn.execute(
                "INSERT INTO mmfdb_branch (branch_uuid, name, description) VALUES (?, 'main', 'Default main branch')",
                (main_uuid,)
            )
        else:
...
def bootstrap_auth_groups(conn: sqlite3.Connection) -> None:
    ...
    with conn:
        for group_id, display_name, description in [
            ...
            conn.execute(
                """INSERT OR IGNORE INTO mmfdb_group (group_id, display_name, description, is_builtin)
                   VALUES (?, ?, ?, 1)""",
...
def bootstrap_vocabulary(conn: sqlite3.Connection) -> None:
    """Bootstrap built-in and legacy extensible vocabulary values in mmfdb_vocabulary.

    All enumeration values are read from the bundled mmCIF/flrCIF dictionary,
    including MMFDB extension categories defined in ``mmfdb_flr_ext.dic``.
    """
    from mmfdb.schema.pdbx_metadata import MmcifDictionary
    ...
    _field_to_item: dict[str, str] = {
        "artifact_kind": "_mmfdb_artifact.artifact_kind",
        "data_format": "_mmfdb_artifact.data_format",
        "operation_type": "_mmfdb_operation.operation_type",
        "parameter_type": "_mmfdb_parameter.parameter_type",
        "relationship_type": "_mmfdb_edge.relationship_type",
        "direction": "_mmfdb_operation_artifact.direction",
        "status": "_mmfdb_operation.status",
        "validation_status": "_mmfdb_operation.validation_status",
        "storage_mode": "_mmfdb_object.storage_mode",
        "lifecycle_status": "_mmfdb_branch.lifecycle_status",
    ...
    with conn:
        for field_name, item_path in _field_to_item.items():
            ...
            for val in values:
                conn.execute(
                    """INSERT OR IGNORE INTO mmfdb_vocabulary (
...
def _edge_has_relationship_constraint(conn: sqlite3.Connection) -> bool:
    row = conn.execute(
        "SELECT sql FROM sqlite_master WHERE type='table' AND name='mmfdb_edge'"
    ).fetchone()
...
def _convert_operation_edges(conn: sqlite3.Connection) -> dict[str, int | list[str]]:
    ...
    for row in conn.execute(
        "SELECT * FROM mmfdb_edge WHERE relationship_type IN ('input_to','produced')"
    ).fetchall():
        rel_type = row["relationship_type"]
        if rel_type == "input_to" and row["target_node_type"] in ("operation", "processing_run", "analysis_run"):
            conn.execute(
                """INSERT OR IGNORE INTO mmfdb_operation_artifact (
            ...
            skipped += 1
            warnings.append(f"Could not convert mmfdb_edge:{row['edge_id']} {rel_type}")
    conn.execute("DELETE FROM mmfdb_edge WHERE relationship_type IN ('input_to','produced')")
    return {"converted": converted, "skipped": skipped, "warnings": warnings}


def _ensure_mmfdb_edge_constraint(conn: sqlite3.Connection) -> dict[str, int | list[str]]:
    if _edge_has_relationship_constraint(conn):
        return {"converted": 0, "skipped": 0, "warnings": []}

    conversion = _convert_operation_edges(conn)
    for index_name in (
        "idx_mmfdb_edge_source", "idx_mmfdb_edge_target", "idx_mmfdb_edge_operation"
    ):
        conn.execute(f"DROP INDEX IF EXISTS {index_name}")

    legacy_table = "__mmfdb_edge_legacy"
    conn.execute(f"DROP TABLE IF EXISTS {legacy_table}")
    conn.execute("ALTER TABLE mmfdb_edge RENAME TO __mmfdb_edge_legacy")
    conn.execute(_CANONICAL_CHECK_DDL["mmfdb_edge"])
    inserted = _copy_non_operation_edges(conn, legacy_table, "mmfdb_edge")
    conn.execute("DROP TABLE __mmfdb_edge_legacy")
    for sql in CREATE_INDICES_SQL:
        if "idx_mmfdb_edge" in sql:
            conn.execute(sql)
    conversion["inserted"] = inserted
    return conversion


MMFDB_EDGE_VOCABULARY_TRIGGER_SQL = [
    """CREATE TRIGGER IF NOT EXISTS trg_mmfdb_edge_relationship_type_insert
    BEFORE INSERT ON mmfdb_edge
    FOR EACH ROW
    BEGIN
        SELECT CASE
            WHEN NEW.relationship_type IN ('input_to', 'produced') THEN
                RAISE(ABORT, 'mmfdb_edge.relationship_type must not be input_to or produced')
            WHEN NOT EXISTS (
                SELECT 1 FROM mmfdb_vocabulary
                WHERE field_name = 'relationship_type'
                  AND value = NEW.relationship_type
                  AND is_active = 1
            ) THEN
                RAISE(ABORT, 'mmfdb_edge.relationship_type is not an active vocabulary value')
        END;
    END""",
    """CREATE TRIGGER IF NOT EXISTS trg_mmfdb_edge_relationship_type_update
    BEFORE UPDATE OF relationship_type ON mmfdb_edge
    FOR EACH ROW
    BEGIN
        SELECT CASE
            WHEN NEW.relationship_type IN ('input_to', 'produced') THEN
...
def _ensure_mmfdb_edge_vocabulary_triggers(conn: sqlite3.Connection) -> dict[str, int | list[str]]:
        ...
        """SELECT edge_id, relationship_type
           ...
           WHERE relationship_type IN ('input_to', 'produced')
              OR NOT EXISTS (
                  SELECT 1 FROM mmfdb_vocabulary
                  WHERE field_name = 'relationship_type'
                    AND value = mmfdb_edge.relationship_type
                    AND is_active = 1
              )"""
    ).fetchall()
    if invalid_rows:
        ...
        raise ValueError(f"Invalid mmfdb_edge.relationship_type values: {details}")
    for sql in MMFDB_EDGE_VOCABULARY_TRIGGER_SQL:
        conn.execute(sql)
    return {"source": 0, "inserted": 0, "skipped": 0, "warnings": []}


FRESH_DB_SCHEMA_SQL = FRESH_DB_TABLES_SQL + FRESH_DB_INDICES_SQL + MMFDB_EDGE_VOCABULARY_TRIGGER_SQL


# ── Migration functions ────────────────────────────────────────────


def _rename_legacy_mfdb_tables(conn: sqlite3.Connection) -> None:
    """Rename pre-rebrand ``mfdb_*`` tables to the ``mmfdb_*`` namespace.

    The database namespace was rebranded MFDB → MMFDB (multimodal fluorescence
    database).  Existing databases carry tables named ``mfdb_*``; the current
    code declares them as ``mmfdb_*``.  This must run **before** any
    reconcile-based migration, otherwise ``CREATE TABLE IF NOT EXISTS`` would
    materialise empty ``mmfdb_*`` tables and orphan the legacy data.

    Idempotent: a no-op once no ``mfdb_*`` tables remain.  If a target
    ``mmfdb_*`` table already exists it is dropped only when empty (a stale
    reconcile artifact); a populated collision is left untouched for manual
    review.

    Parameters
    ----------
    conn : sqlite3.Connection
        Open connection to the database being migrated.
    """
    rows = conn.execute(
        "SELECT name FROM sqlite_master WHERE type='table' AND name GLOB 'mfdb_*'"
    ).fetchall()
    if not rows:
        return
    with conn:
        for (old_name,) in rows:
            new_name = "mmfdb_" + old_name[len("mfdb_"):]
            exists = conn.execute(
            ...
            if exists:
                ...
                if count == 0:
                    ...
                    logger.warning(
                        "MMFDB rename: both %s and populated %s exist — "
                        "leaving legacy table for manual reconciliation",
                        old_name, new_name,
                    )
                    continue
            conn.execute(f'ALTER TABLE "{old_name}" RENAME TO "{new_name}"')
        logger.info("Renamed %d legacy mfdb_* tables to mmfdb_* namespace", len(rows))


def _migrate_v1_fresh_db(conn: sqlite3.Connection) -> None:
    """Create a fresh database from the current declarative baseline."""
...
def _migrate_v41_reconcile(conn: sqlite3.Connection) -> None:
    """Reconcile databases stamped v40 before auth columns were complete.

    A prerelease v40 database could be marked current while still missing
    dictionary columns or provider-aware authentication fields.  Reconciliation
    belongs in this explicit one-time migration, not on every database open.
    """
    reconcile_current_schema(conn)


def _migrate_v42_rename_namespace(conn: sqlite3.Connection) -> None:
    """Rebrand MFDB → MMFDB: ensure legacy tables are renamed and reconcile.

    The actual ``mfdb_* → mmfdb_*`` rename happens up front in
    :func:`migrate_schema` (before any reconcile migration).  This step is the
    idempotent backstop plus a final reconcile to add any columns/tables new to
    the ``mmfdb_*`` namespace.
    """
...
def _bootstrap(conn: sqlite3.Connection) -> None:
    ...
    try:
        ...
        pass
    from mmfdb.provenance.operation_parameters import bootstrap_operation_parameter_defs
...
# v40: reconcile existing DBs to the canonical structure.
# v41: repair prerelease v40 databases missing dictionary/auth columns.
# v42: rebrand MFDB → MMFDB — rename mfdb_* tables to the mmfdb_* namespace.
...
def _ensure_canonical_columns(conn: sqlite3.Connection) -> None:
    ...
    ``reconcile_schema``, which only covers ``mmfdb_*`` extension tables. This adds
...
def _drop_legacy_tables(conn: sqlite3.Connection) -> None:
    """Drop duplicate (mmfdb_sample, mmfdb_experiment) and legacy (fdb_*) tables."""
    legacy = [
        "mmfdb_sample", "mmfdb_experiment", "mmfdb_microtime_shift",
