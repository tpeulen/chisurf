    ...
    def _parse_file(self, path: Path) -> None:
        ...
        with opener(path, "rt", encoding="utf-8") as f:
            for line in f:
                ...
                elif stripped.startswith("_mmfdb_schema.table_name") or stripped.startswith("_chisurf_schema.table_name"):
                    current_item.schema_table = self._extract_value(stripped)
                elif stripped.startswith("_mmfdb_schema.column_name") or stripped.startswith("_chisurf_schema.column_name"):
                    current_item.schema_column = self._extract_value(stripped)
                elif stripped.startswith("_mmfdb_schema.status") or stripped.startswith("_chisurf_schema.status"):
                    current_item.schema_status = self._extract_value(stripped)
                elif stripped.startswith("_mmfdb_schema.foreign_key") or stripped.startswith("_chisurf_schema.foreign_key"):
