...
# data from code. The actual data is stored in chisurf/core/mmfdb/data/*.json files.
...
@dataclass
class MmfdbVocabulary:
    """Extensible vocabulary entry (``mmfdb_vocabulary``)."""
...
# ── Canonical MMFDB Models ─────────────────────────────────────────────

@dataclass
class MmfdbObject:
    """Content-addressed object in the deduplicated object store (``mmfdb_object``)."""
...
class MmfdbArtifact:
    ...
    exported archive, figure, table, fit result, or manifest (``mmfdb_artifact``)."""
...
@dataclass
class MmfdbOperationArtifact:
    """Link table between operations and artifacts (inputs & outputs).
    Role is NOT NULL in the database (``mmfdb_operation_artifact``)."""
...
@dataclass
class MmfdbEdge:
    """General provenance relationship edge (``mmfdb_edge``).
    Never stores ``input_to`` or ``produced`` — those belong in
    ``MmfdbOperationArtifact``."""
...
@dataclass
class MmfdbParameter:
    """Semantically important fit or analysis parameter (``mmfdb_parameter``)."""
...
@dataclass
class MmfdbSetup:
    """Instrument and configuration setup version snapshot (``mmfdb_setup``)."""
...
@dataclass
class MmfdbAuditLog:
    """Change log entry for operations on database (``mmfdb_audit_log``)."""
...
FdbArtifact = MmfdbArtifact
FdbOperation = MmfdbOperation
FdbOperationArtifact = MmfdbOperationArtifact
FdbEdge = MmfdbEdge
FdbParameter = MmfdbParameter
FdbSetup = MmfdbSetup
FdbAuditLog = MmfdbAuditLog
