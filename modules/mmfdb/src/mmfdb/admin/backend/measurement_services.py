...
def register_measurement_services(
    ...
    *,
    ...
    if burst_selection_runner is not None:
        handlers["processing.burst_selection.run"] = lambda **params: run_burst_selection_handler(
            **params,
...
def run_burst_selection_handler(
    ...
    software_version: str | None = None,
    *,
    analyze_files: Callable[..., dict[str, Any]] | None = None,
) -> dict[str, Any]:
    ...
    if analyze_files is None:
        return service_error(
            "Burst Selection execution is unavailable without a host-provided runner",
...
def list_processed_data_handler(
    ...
    with MFDatabase(resolve_database_path()) as db:
        if processing_id:
            ...
            query = (
                "SELECT DISTINCT artifact.* FROM mmfdb_artifact AS artifact "
                "JOIN mmfdb_operation_artifact AS link "
            ...
            with db.transaction():
                ...
                if archive_object:
                    ...
                    if isinstance(stored_payload["extra"], dict):
                        stored_payload["extra"]["mmfdb_project_archive"] = {
                            "object_uuid": archive_object.get("object_uuid"),
                            "artifact_id": f"{project_id}_archive",
                            "filename": archive_object.get("original_filename"),
        ...
        with MFDatabase(resolve_database_path()) as db:
            ...
            if isinstance(payload, dict):
                ...
                if isinstance(extra, dict):
                    archive_info = extra.get("mmfdb_project_archive")
