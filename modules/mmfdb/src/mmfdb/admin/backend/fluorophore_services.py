    ...
    *,
    ...
    if deterministic_checks is None:
        return service_error(
            "AI triage is unavailable without a host-provided checker",
    ...
    with _db() as db:
        ...
        result = deterministic_checks(probe_data)
...
def handle_find_duplicates(
    ...
    with _db() as db:
        rows = db.conn.execute(
            "SELECT p.probe_id, p.chromophore_name, p.category, p.source, p.verification_status "
            "FROM probes p WHERE p.deleted_at IS NULL"
        ).fetchall()
...
def register_services(
    ...
    *,
    ...
    The mmfdb-admin ``ServiceDispatcher`` invokes a handler with a single
    ...
    if deterministic_checks is not None:
        dispatcher.register(
            ...
            _kw(lambda **params: handle_run_ai_triage(
                **params,
