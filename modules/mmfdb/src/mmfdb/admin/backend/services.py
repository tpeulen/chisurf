"""ServiceDispatcher-compatible handlers for the mmfdb-admin."""
...
from mmfdb.admin.backend._service_errors import OPERATION_FAILED, service_error
...
from mmfdb.admin.backend.password_services import (
    evaluate_password,
    hash_password,
)
...
from mmfdb.request_context import open_api_context
from mmfdb.samples import reagents
...
def register_services(
    ...
    *,
    ...
    pipeline_get: Callable[..., Any] | None = None,
    ...
    # Versioned mmfdb.v1.* services share one authenticated request context.
    import mmfdb.api as mmfdb_api
    for name, handler_name in VERSIONED_MMFDB_METHODS.items():
        handler = getattr(mmfdb_api, handler_name)

        def _make_v1_handler(h):
            def _v1_handler(params):
                call_params = dict(params)
                auth = call_params.pop("auth", None)
                with open_api_context(auth) as context:
                    require_authenticated(context.principal)
                    return mmfdb_api.invoke_with_context(h, call_params, context)
            return _v1_handler
    ...
    if pipeline_list is not None:
        dispatcher.register(
            "mmfdb.pipelines.list",
            lambda params: list_pipelines_handler(**params, list_pipelines=pipeline_list),
        )
    if pipeline_get is not None:
        dispatcher.register(
            "mmfdb.pipelines.get",
            lambda params: get_pipeline_handler(**params, get_pipeline=pipeline_get),
        )
    if pipeline_runs is not None:
        dispatcher.register(
            "mmfdb.pipelines.runs",
            lambda params: list_pipeline_runs_handler(**params, list_pipeline_runs=pipeline_runs),
        )
...
def _resolve_owner_id(db: "MFDatabase", auth: dict[str, Any] | None) -> str | None:
    """Resolve the acting user for MMFDB dataset access (PRD-17 canonical resolver).
    ...
    ``mmfdb.default_user_id`` — the same identity registration stamps — so reads and
    writes agree.
    """
    ...
    from mmfdb.security.session import resolve_active_user_id
...
def _validate_mmfdb_methods_in_manifest(manifest_path: str | Path | None = None) -> list[str]:
    ...
    -------
    list of str
        Fully qualified ``mmfdb.v1.*`` method names absent from the manifest.
...
def create_calibration_handler(
    ...
    from mmfdb.provenance.result_registry import register_calibration
...
def list_pipelines_handler(
    ...
    auth: dict[str, Any] | None = None,
    *,
    list_pipelines: Callable[..., list[dict[str, Any]]] | None = None,
) -> dict[str, Any]:
    """List stored pipeline definitions scoped mine/public/all."""
    if list_pipelines is None:
        return service_error(
            "Pipeline browsing is unavailable without a host adapter",
...
def get_pipeline_handler(
    ...
    auth: dict[str, Any] | None = None,
    *,
    get_pipeline: Callable[..., Any] | None = None,
) -> dict[str, Any]:
...
def list_pipeline_runs_handler(
    ...
    auth: dict[str, Any] | None = None,
    *,
    list_pipeline_runs: Callable[..., list[dict[str, Any]]] | None = None,
) -> dict[str, Any]:
    """List runs (newest first) with operation counts, optionally for one pipeline."""
    if list_pipeline_runs is None:
        return service_error(
            "Pipeline browsing is unavailable without a host adapter",
            error_code=OPERATION_FAILED,
        )
...
def lifecycle_definitions_handler(auth: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    from mmfdb.lifecycle.lifecycle import load_lifecycle_defs
...
def _rename_user_id_references(conn: Any, old_user_id: str, new_user_id: str) -> None:
    """Rename a user ID and all username-based MMFDB references.
    ...
    new_user_id : str
        New username.
    """
    logging.info("MMFDB users: renaming user_id '%s' to '%s'", old_user_id, new_user_id)
    table_columns = (
        ...
        ("mmfdb_operation", "operator_user_id"),
        ("mmfdb_audit_log", "operator_user_id"),
        ("mmfdb_branch", "created_by_user_id"),
        ("mmfdb_group", "created_by_user_id"),
        ("mmfdb_group_member", "user_id"),
        ("mmfdb_group_member", "created_by_user_id"),
        ("mmfdb_object_acl", "owner_user_id"),
        ("mmfdb_acl_entry", "created_by_user_id"),
        ("mmfdb_session", "user_id"),
        ("mmfdb_auth_attempt", "user_id"),
    )
    ...
    try:
        with conn:
            ...
            if _table_has_column(conn, "mmfdb_acl_entry", "subject_id"):
                conn.execute(
                    "UPDATE mmfdb_acl_entry SET subject_id = ? WHERE subject_type = 'user' AND subject_id = ?",
...
def _project_row_dict(row: Any) -> dict[str, Any]:
    """Return a GUI-friendly project record from an MMFDB project operation row."""
    d = _json_row(row)
        ...
        if setup_id:
            ...
            rows = [
                dict(r) for r in db.conn.execute(
                    "SELECT * FROM mmfdb_setup_detector_channel WHERE deleted_at IS NULL ORDER BY id"
                ).fetchall()
            ]
...
def list_pie_windows_handler(
    ...
    with MFDatabase(resolve_database_path()) as db:
        ...
        if setup_id:
            ...
            rows = [
                dict(r) for r in db.conn.execute(
                    "SELECT * FROM mmfdb_setup_pie_window WHERE deleted_at IS NULL ORDER BY id"
                ).fetchall()
            ]
        ...
        if setup_id:
            rows = [
                dict(row) for row in db.conn.execute(
                    "SELECT * FROM mmfdb_setup_fcs_pair "
            ...
            rows = [
                dict(row) for row in db.conn.execute(
                    "SELECT * FROM mmfdb_setup_fcs_pair WHERE deleted_at IS NULL ORDER BY id"
                ).fetchall()
            ]
...
def _require_auth(auth: dict[str, Any] | None, conn: Any) -> Any:
    ...
    if not auth or not auth.get("token"):
        ...
        logging.warning("MMFDB auth: rejecting write with invalid, expired, or revoked session token")
