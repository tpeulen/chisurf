...
Migrated from the standalone fluorophore_db plugin into mmfdb-admin (PRD-06). It
operates directly on the live MMFDB via the core repository — no legacy adapter —
...
def _open_db() -> Iterator["object"]:
    """Open the configured MMFDB for the duration of a command."""
...
@click.group()
def cli() -> None:
    """Manage and curate fluorophores in the MMFDB."""
...
def import_reference_set(source: str | None, mark_verified: bool, replace: bool) -> None:
    """Import the scraped reference dataset into the MMFDB."""
...
def ai_triage_cli(status: str, provider: str | None, as_json: bool) -> None:
    """Explain that AI triage is a host application extension."""
    raise click.ClickException(
        "AI triage is not part of standalone MMFDB; invoke it through a host adapter"
    )


# ---- authentication (`csc fluorophore auth …` / `mmfdb-admin auth …`) ----


@cli.group("auth")
def auth() -> None:
    """Authenticate against MMFDB (local / LDAP), headless."""
...
def auth_whoami(token: str, as_json: bool) -> None:
    ...
    from mmfdb.security.auth import authenticate_token
