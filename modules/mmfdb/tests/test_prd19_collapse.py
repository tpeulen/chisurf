"""Tests for PRD-19: MMFDB collapse to one flrCIF-rooted family.

Verifies that:
1. Model vocabulary constants match the .dic enumerations.
2. reconcile_schema creates missing extension tables and columns.
3. _drop_legacy_tables removes all fdb_* and duplicate mmfdb_* tables.
4. A fresh MFDatabase has no fdb_*, mmfdb_sample, or mmfdb_experiment tables.
...
from mmfdb.samples.sample_manager import create_sample, get_sample_full_description
from mmfdb.models import SampleDefinition, EntityDefinition, ProbeDefinition
...
def test_reconcile_schema_creates_missing_extension_tables() -> None:
    ...
    try:
        ...
        conn.execute("CREATE TABLE IF NOT EXISTS mmfdb_setup (setup_id TEXT PRIMARY KEY, name TEXT)")
        conn.commit()

        # Verify extension tables do not exist yet
        cursor = conn.execute(
            "SELECT name FROM sqlite_master WHERE type='table' AND name LIKE 'mmfdb_%'"
        )
        existing = {row[0] for row in cursor.fetchall()}
        assert "mmfdb_operation" not in existing
        ...
        assert "mmfdb_operation" in after, "mmfdb_operation table not created"
        assert "mmfdb_operation_artifact" in after
        assert "mmfdb_edge" in after
        assert "mmfdb_parameter" in after
        assert "mmfdb_object" in after
        assert "mmfdb_artifact" in after
        assert "mmfdb_branch" in after
    finally:
        conn.close()
        os.remove(path)
...
def test_reconcile_schema_adds_missing_columns() -> None:
    ...
    conn, path = _create_conn()
    try:
        # Create mmfdb_setup table missing the name column
        conn.execute("""
        ...
        cursor = conn.execute("PRAGMA table_info(mmfdb_setup)")
...
def test_drop_legacy_tables_removes_fdb_tables() -> None:
    """_drop_legacy_tables removes all fdb_* and duplicate mmfdb_* tables."""
    conn, path = _create_conn()
    try:
        # Create fdb_ tables and duplicate mmfdb_ tables
        legacy_tables = [
        ...
        assert "mmfdb_setup" in after, "non-legacy table should survive"
    finally:
        conn.close()
        os.remove(path)
...
def test_fresh_database_has_no_legacy_tables() -> None:
    ...
    try:
        ...
        # No legacy tables
        for prefix in ["fdb_", "mmfdb_sample", "mmfdb_experiment"]:
        ...
        # mmfdb extension tables should exist
        mmfdb_tables = [t for t in table_names if t.startswith("mmfdb_")]
        assert len(mmfdb_tables) > 0, "No mmfdb_ extension tables found"
    finally:
        db.close()
        os.remove(path)
