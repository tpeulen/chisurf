...
`mmfdb_protocol` is `.dic`-declared (created by reconcile_schema); operations carry
...
def test_protocol_table_and_operation_columns_exist(db):
    ...
    assert "mmfdb_protocol" in tables
    op_cols = {r[1] for r in db.conn.execute("PRAGMA table_info(mmfdb_operation)")}
...
def test_parameter_schema_is_operation_type_schema(db):
    ...
    assert "min_photons" in schema  # reuses mmfdb_operation_parameter_def, no forked stack
...
def test_register_operation_records_protocol_ref(db, tmp_path):
    from mmfdb.provenance.result_registry import register_operation, set_global_db
    ...
    try:
        ...
        set_global_db(None)
    row = db.conn.execute(
        "SELECT protocol_id, protocol_version FROM mmfdb_operation WHERE operation_id = ?",
        (op_id,),
    ).fetchone()
    assert row[0] == pid
