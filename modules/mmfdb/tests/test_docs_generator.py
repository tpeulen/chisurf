...
from mmfdb.schema.docs_generator import (
    collect_schema,
...
def test_reference_covers_live_tables_and_operations(dictionary, live_tables):
    ...
    assert md.startswith("# MMFDB schema reference")
    # Key tables rendered with a column table.
    for table in ("flr_sample", "mmfdb_artifact", "mmfdb_operation_parameter_def"):
...
def test_default_scope_is_flr_and_mmfdb_namespaces(dictionary):
    """Without an explicit ``tables`` filter, only flr_/mmfdb_ tables are emitted."""
    schema = collect_schema(dictionary)
    assert schema, "expected some flr_/mmfdb_ tables"
    assert all(t.startswith(("flr_", "mmfdb_")) for t in schema)
    assert "flr_sample" in schema
    # A pure base-mmCIF category (not part of the MMFDB schema) is excluded.
