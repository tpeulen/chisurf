...
A metadata value of the form ``mmfdb://<node_type>/<node_id>`` materializes a
...
from mmfdb.queries.artifacts import parse_node_ref
...
def _edges(db: MFDatabase) -> list[dict]:
    return [dict(r) for r in db.conn.execute(
        "SELECT * FROM mmfdb_edge WHERE deleted_at IS NULL"
    ).fetchall()]


def test_parse_node_ref() -> None:
    assert parse_node_ref("mmfdb://artifact/art_1") == ("artifact", "art_1")
    assert parse_node_ref("  mmfdb://sample/s 1  ") == ("sample", "s 1")
...
def test_sample_metadata_materializes_edge(tmp_path: Path) -> None:
    with _db(tmp_path) as db:
        ...
        db.set_sample_key_value("s1", "related_dataset", "mmfdb://artifact/art_1")
...
def test_non_reference_and_self_reference_do_not_link(tmp_path: Path) -> None:
    with _db(tmp_path) as db:
        ...
        db.set_sample_key_value("s1", "self", "mmfdb://sample/s1")
        assert _edges(db) == []
...
def test_experiment_metadata_materializes_edge(tmp_path: Path) -> None:
    with _db(tmp_path) as db:
        ...
        db.set_experiment_key_value("e1", "on_sample", "mmfdb://sample/s1")
        edges = _edges(db)
        assert len(edges) == 1
