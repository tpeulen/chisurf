"""Tests for MMFDB authentication and authorization."""
...
def test_fresh_schema_creates_auth_tables(tmp_path: Path) -> None:
    ...
    with MFDatabase(path) as db:
        ...
        for t in ("mmfdb_group", "mmfdb_group_member", "mmfdb_object_acl",
                   "mmfdb_acl_entry", "mmfdb_session", "mmfdb_auth_attempt"):
...
def test_fresh_schema_bootstraps_groups(tmp_path: Path) -> None:
    ...
    with MFDatabase(path) as db:
        groups = db.conn.execute(
            "SELECT group_id FROM mmfdb_group WHERE deleted_at IS NULL"
        ).fetchall()
...
def test_session_stores_hash_not_token(db, normal_user):
    session = create_session(db.conn, normal_user)
    token = session["token"]
    rows = db.conn.execute("SELECT token_hash FROM mmfdb_session").fetchall()
...
def test_revoke_session(db, normal_user):
    ...
    srow = db.conn.execute(
        "SELECT session_id FROM mmfdb_session WHERE user_id = ? AND revoked_at IS NULL",
        (normal_user,),
    ).fetchone()
    session_id = srow[0]
    revoke_session(db.conn, session_id)
    srow = db.conn.execute(
        "SELECT revoked_at FROM mmfdb_session WHERE session_id = ?",
        (session_id,),
    ).fetchone()
...
def test_session_last_used_updates(db, normal_user):
    session = create_session(db.conn, normal_user)
    token = session["token"]
    db.conn.execute("UPDATE mmfdb_session SET last_used_at = NULL WHERE token_hash = ?",
                    (__import__("hashlib").sha256(token.encode()).hexdigest(),))
    db.conn.commit()
    authenticate_token(db.conn, token)
    row = db.conn.execute("SELECT last_used_at FROM mmfdb_session WHERE token_hash = ?",
...
def test_record_auth_attempt(db, normal_user):
    ...
    rows = db.conn.execute(
        "SELECT success, reason FROM mmfdb_auth_attempt WHERE user_id = ?",
        (normal_user,),
    ).fetchall()
    assert len(rows) == 1
    assert rows[0][0] == 0
...
def test_create_default_acl(db, normal_user):
    ...
    row = db.conn.execute(
        "SELECT mode, owner_user_id FROM mmfdb_object_acl WHERE object_type = ? AND object_id = ?",
        ("sample", "sample_1"),
    ).fetchone()
...
def test_group_readable_object(db, normal_user):
    other_user = _make_user(db, "other_user")
    db.conn.execute(
        "INSERT OR IGNORE INTO mmfdb_group_member (group_id, user_id, role) VALUES ('users', ?, 'member')",
        (other_user,),
    )
    create_default_acl_for_object(db.conn, "sample", "sample_1", normal_user,
...
def test_chmod_updates_mode(db, normal_user):
    ...
    row = db.conn.execute(
        "SELECT mode FROM mmfdb_object_acl WHERE object_type = 'sample' AND object_id = 'sample_1'"
    ).fetchone()
    assert row[0] == 0o704
...
def test_chown_updates_owner(db, normal_user):
    ...
    row = db.conn.execute(
        "SELECT owner_user_id FROM mmfdb_object_acl WHERE object_type = 'sample' AND object_id = 'sample_1'"
    ).fetchone()
...
def test_chgrp_updates_group(db, normal_user):
    ...
    row = db.conn.execute(
        "SELECT owner_group_id FROM mmfdb_object_acl WHERE object_type = 'sample' AND object_id = 'sample_1'"
    ).fetchone()
    assert row[0] == "users"
...
def test_inherit_acl_from_parent(db, normal_user):
    ...
    row = db.conn.execute(
        "SELECT mode, inherits_from_type, inherits_from_id FROM mmfdb_object_acl WHERE object_type = 'artifact' AND object_id = 'art_1'"
    ).fetchone()
    assert row is not None
    assert row[0] == 0o750
...
def test_revoke_acl_soft_deletes(db, normal_user):
    ...
    row = db.conn.execute(
        "SELECT entry_id FROM mmfdb_acl_entry WHERE object_type = 'sample' AND object_id = 'sample_1' AND deleted_at IS NULL"
    ).fetchone()
    entry_id = row[0]
    p = AuthenticatedPrincipal(normal_user)
    revoke_acl(db.conn, p, entry_id)
    row = db.conn.execute(
        "SELECT deleted_at FROM mmfdb_acl_entry WHERE entry_id = ?",
        (entry_id,),
    ).fetchone()
    assert row[0] is not None
...
def test_login_handler_creates_session(db, normal_user, patch_db):
    from mmfdb.admin.backend.auth_services import login_handler
...
def test_me_handler_returns_user(db, normal_user, patch_db):
    from mmfdb.admin.backend.auth_services import me_handler
    session = create_session(db.conn, normal_user)
...
def test_groups_list(db, normal_user, patch_db):
    from mmfdb.admin.backend.auth_services import groups_list_handler
    session = create_session(db.conn, normal_user)
...
def test_groups_create_admin_only(db, admin_user, patch_db):
    from mmfdb.admin.backend.auth_services import groups_create_handler
...
def test_members_list(db, normal_user, patch_db):
    from mmfdb.admin.backend.auth_services import members_list_handler
    session = create_session(db.conn, normal_user)
...
def test_members_add(db, admin_user, normal_user, patch_db):
    from mmfdb.admin.backend.auth_services import members_add_handler
    session = create_session(db.conn, admin_user)
    ...
    assert result["ok"] is True
    rows = db.conn.execute(
        "SELECT role FROM mmfdb_group_member WHERE group_id = 'admins' AND user_id = ? AND deleted_at IS NULL",
        (normal_user,),
    ).fetchall()
    assert len(rows) == 1
...
def test_members_remove(db, admin_user, normal_user, patch_db):
    from mmfdb.admin.backend.auth_services import members_add_handler, members_remove_handler
    session = create_session(db.conn, admin_user)
    ...
    assert result["ok"] is True
    rows = db.conn.execute(
        "SELECT deleted_at FROM mmfdb_group_member WHERE group_id = 'admins' AND user_id = ?",
        (normal_user,),
    ).fetchall()
...
def test_permissions_get(db, normal_user, patch_db):
    from mmfdb.admin.backend.auth_services import permissions_get_handler
    create_default_acl_for_object(db.conn, "sample", "sample_1", normal_user)
    session = create_session(db.conn, normal_user)
...
def test_logout_revokes_token(db, normal_user, patch_db):
    from mmfdb.admin.backend.auth_services import login_handler, logout_handler
...
def test_sessions_list(db, normal_user, patch_db):
    from mmfdb.admin.backend.auth_services import sessions_list_handler
    create_session(db.conn, normal_user)
    session = create_session(db.conn, normal_user)
...
def test_sessions_revoke(db, normal_user, patch_db):
    from mmfdb.admin.backend.auth_services import sessions_revoke_handler
    session = create_session(db.conn, normal_user)
    srow = db.conn.execute(
        "SELECT session_id FROM mmfdb_session WHERE user_id = ? AND revoked_at IS NULL",
        (normal_user,),
...
def test_permissions_chmod_via_rpc(db, normal_user, patch_db):
    from mmfdb.admin.backend.auth_services import permissions_chmod_handler
    create_default_acl_for_object(db.conn, "sample", "sample_1", normal_user)
    session = create_session(db.conn, normal_user)
...
def test_permissions_grant_via_rpc(db, normal_user, patch_db):
    from mmfdb.admin.backend.auth_services import permissions_grant_handler
    other_user = _make_user(db, "other_user")
    create_default_acl_for_object(db.conn, "sample", "sample_1", normal_user)
    session = create_session(db.conn, normal_user)
