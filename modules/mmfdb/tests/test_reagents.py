...
``mmfdb_reagent_usage`` (no columns added to those tables). The "what was used" query and
...
from mmfdb.samples.reagents import (
    add_reagent_lot,
    expired_lots,
    link_reagent,
    list_lots,
...
def test_usage_many_to_many_without_schema_pollution(db):
    ...
    op_cols = {r[1] for r in db.conn.execute("PRAGMA table_info(mmfdb_operation)").fetchall()}
