...
from mmfdb.lifecycle.lifecycle import (
...
def test_transition_tables_exist(db):
    ...
    assert "mmfdb_state_transition" in tables
    assert "mmfdb_state_transition_rule" in tables


def test_transition_log_has_expected_columns(db):
    cols = {r[1] for r in db.conn.execute("PRAGMA table_info(mmfdb_state_transition)")}
    assert {
...
def test_state_vocabularies_seeded(db):
    for entity_type, ld in load_lifecycle_defs().items():
        rows = {
            r[0]
            for r in db.conn.execute(
                "SELECT value FROM mmfdb_vocabulary WHERE field_name = ?",
...
def test_transition_rules_seeded(db):
    ...
    rows = {
        (r[0], r[1])
        for r in db.conn.execute(
            "SELECT from_state, to_state FROM mmfdb_state_transition_rule "
...
def test_bootstrap_is_idempotent(db):
    before = db.conn.execute(
        "SELECT COUNT(*) FROM mmfdb_state_transition_rule"
    ).fetchone()[0]
