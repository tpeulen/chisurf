...
def test_delete_setup_migrated_to_dao(db):
    ...
    db.save_setup("setup-1", name="S1")
    dao = _dao(db)
    assert dao.get("mmfdb_setup", "setup-1")["deleted_at"] is None
    db.delete_setup("setup-1")
    # Hidden from default reads; the deleted_at marker is set.
    assert dao.get("mmfdb_setup", "setup-1") is None
    assert dao.get("mmfdb_setup", "setup-1", include_deleted=True)["deleted_at"] is not None
...
def test_get_artifact_migrated_to_dao_returns_soft_deleted(db):
    ...
    dao.soft_delete("mmfdb_artifact", "a1")
...
def test_delete_parameter_migrated_to_dao_soft_deletes_by_either_key(db):
    ...
    assert dao.get("mmfdb_parameter", 101, pk_column="parameter_id") is None
    assert (
        dao.get("mmfdb_parameter", 101, pk_column="parameter_id", include_deleted=True)[
            "deleted_at"
        ]
        is not None
    )
    ...
    assert dao.get("mmfdb_parameter", 102, pk_column="parameter_id") is None
...
def test_insert_into_keyless_junction_returns_rowid(db):
    ...
    ``mmfdb_group_member`` has no primary key — only ``UNIQUE(group_id, user_id)``.
    ...
    dao = _dao(db)
    with pytest.raises(Exception):
        dao.primary_key("mmfdb_group_member")

    # Satisfy the junction's foreign keys with real parent rows.
    dao.insert("mmfdb_group", {"group_id": "g_keyless", "display_name": "Keyless"})
