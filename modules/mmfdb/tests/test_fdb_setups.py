...
def test_setup_definition_repository_crud_and_linkage(tmp_path: pathlib.Path) -> None:
    ...
    with MFDatabase(db_path) as db:
        ...
        row = db.get_setup("setup_mfd_1")
        assert row is not None
        assert row["name"] == "MFD Setup 1"
        assert row["setup_id"] == "setup_mfd_1"

        assert row["configuration_json"] == '{"laser_wavelengths":[485,640]}'
        assert row["detectors_json"] == '{"green":[0,1],"red":[2,3]}'
        ...
        # 4. Test Delete
        db.delete_setup("setup_mfd_1")
        setup = db.get_setup("setup_mfd_1")
