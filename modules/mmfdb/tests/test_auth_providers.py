...
from mmfdb.security.auth import AuthError, authenticate_token, is_throttled
from mmfdb.security.auth_providers import AuthIdentity, LocalAuthProvider
from mmfdb.security.login import (
    login,
...
def test_login_throttles_after_repeated_failures(tmp_path: Path) -> None:
    from mmfdb.security.auth import MAX_FAILED_ATTEMPTS
...
def test_jit_provision_creates_and_links_user(tmp_path: Path) -> None:
    with _db(tmp_path) as db:
        ...
        gm = db.conn.execute(
            "SELECT 1 FROM mmfdb_group_member WHERE group_id='users' AND user_id='jdoe'"
        ).fetchone()
...
def test_jit_admin_joins_admins_group(tmp_path: Path) -> None:
    with _db(tmp_path) as db:
        ...
        gm = db.conn.execute(
            "SELECT 1 FROM mmfdb_group_member WHERE group_id='admins' AND user_id='root'"
        ).fetchone()
        assert gm is not None
    ...
    with _db(tmp_path) as db:
        ...
        rows = {
            ...
            for r in db.conn.execute(
                "SELECT group_id FROM mmfdb_group_member WHERE user_id='guser'"
            )
        }
...
def test_login_via_ldap_identity_end_to_end(tmp_path: Path, monkeypatch) -> None:
    ...
    with _db(tmp_path) as db:
        ...
        assert db.conn.execute(
            "SELECT 1 FROM mmfdb_group_member WHERE group_id='public' AND user_id='mallory'"
