"""Tests for MMFDB payload msgpack codecs."""
...
from mmfdb.store.payload_codec import (
    MIGRATIONS,
    REGISTRY,
    PayloadSchemaError,
    decode_payload,
...
def test_payload_flrcif_item_ids_use_existing_parameter_translation():
    ...
    aniso_schema = get_payload_schema("anisotropy_curve")
    assert aniso_schema["fields"]["l1"]["flrcif_item_id"] == _lookup_flrcif_name("l1", resolver)
    assert aniso_schema["fields"]["l2"]["flrcif_item_id"] == _lookup_flrcif_name("l2", resolver)
    assert aniso_schema["fields"]["g_factor"]["flrcif_item_id"] == _lookup_flrcif_name("g", resolver)

    tcspc_schema = get_payload_schema("tcspc_decay")
    assert tcspc_schema["fields"]["adc_resolution_ns"]["flrcif_item_id"] == _lookup_flrcif_name("dtTAC[ns]", resolver)
    assert tcspc_schema["fields"]["micro_time_resolution_ns"]["flrcif_item_id"] == _lookup_flrcif_name("dtMT[ns]", resolver)
