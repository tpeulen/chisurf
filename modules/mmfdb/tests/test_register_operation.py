...
def test_register_operation_records_node_and_links(tmp_path):
    ...
    try:
        ...
        row = db.conn.execute(
            "SELECT operation_type FROM mmfdb_operation WHERE operation_id = ?", (op_id,)
        ).fetchone()
        assert row[0] == "microtime_shift"
        # an input port link was recorded
        n_inputs = db.conn.execute(
            "SELECT COUNT(*) FROM mmfdb_operation_artifact WHERE operation_id=? AND direction='input'",
            (op_id,),
        ).fetchone()[0]
        assert n_inputs == 1
    finally:
...
def test_register_operation_role_indexed_parameters(tmp_path):
    ...
    try:
        ...
        rows = db.conn.execute(
            "SELECT name, value, role FROM mmfdb_parameter WHERE operation_id=? AND name='shift' ORDER BY role",
            (op_id,),
        ).fetchall()
...
def test_register_operation_rejects_unknown_parameter(tmp_path):
    ...
    try:
        ...
        n = db.conn.execute(
            "SELECT COUNT(*) FROM mmfdb_operation WHERE operation_type='burst_selection'"
        ).fetchone()[0]
        assert n == 0
    finally:
        set_global_db(None)
