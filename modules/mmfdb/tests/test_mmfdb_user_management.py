...
def test_mmfdb_default_user_creation_and_fallbacks(tmp_path: Path) -> None:
...
def test_mmfdb_user_passwords_and_login(tmp_path: Path) -> None:
    db_path = tmp_path / "user_pw_test.db"

    from unittest.mock import patch

    from mmfdb.admin.backend.password_services import login_handler
    from mmfdb.admin.backend.services import (
        list_users_handler,
        save_user_handler,
    )
...
def test_mmfdb_save_user_permissions(tmp_path: Path) -> None:
...
def test_mmfdb_save_user_renames_username_and_references(tmp_path: Path) -> None:
    ...
    with patch("mmfdb.admin.backend.services.resolve_database_path", return_value=db_path):
        ...
        try:
            ...
            db.conn.execute(
                "INSERT INTO mmfdb_object_acl (object_type, object_id, owner_user_id) VALUES (?, ?, ?)",
        ...
        try:
            ...
            assert db.conn.execute(
                ...
                (db.conn.execute(
                    "SELECT session_id FROM mmfdb_session WHERE token_hash = ?",
                    (hashlib.sha256(session["token"].encode("utf-8")).hexdigest(),),
                ).fetchone()[0],),
            ).fetchone()[0] == "new_name"
            assert db.conn.execute(
                "SELECT user_id FROM mmfdb_group_member WHERE user_id = 'new_name'"
            ).fetchone()[0] == "new_name"
            assert db.conn.execute(
                "SELECT owner_user_id FROM mmfdb_object_acl WHERE object_type = 'sample' AND object_id = 'rename_sample'"
            ).fetchone()[0] == "new_name"
        finally:
            db.close()


def test_mmfdb_delete_user_admin_override(tmp_path: Path) -> None:
...
def test_admin_password_strength_enforcement(tmp_path: Path) -> None:
    ...
    from mmfdb.admin.backend.password_services import change_password_handler
    from mmfdb.admin.backend.services import (
        list_users_handler,
        save_user_handler,
    )
...
def test_migration_repairs_legacy_schema_marked_without_deleted_at(tmp_path: Path) -> None:
    ...
    try:
        ...
        conn.execute("INSERT INTO mmfdb_schema_version (version) VALUES (23)")
        conn.commit()
    finally:
        conn.close()
...
def test_v40_database_is_reconciled_by_explicit_v41_migration(tmp_path: Path) -> None:
    """A prerelease v40 DB is reconciled once before user/login queries."""
    import sqlite3

    from mmfdb.repository import MFDatabase

    db_path = tmp_path / "current_missing_auth_provider.db"
    conn = sqlite3.connect(db_path)
    try:
        conn.execute(
            "CREATE TABLE flr_sample_users ("
            "user_id TEXT PRIMARY KEY, display_name TEXT, user_uuid TEXT, "
            "is_admin INTEGER DEFAULT 0, password_hash TEXT, "
            "allow_passwordless_login INTEGER DEFAULT 0, deleted_at TEXT)"
        )
        conn.execute(
            "INSERT INTO flr_sample_users (user_id, display_name) VALUES (?, ?)",
            ("legacy", "Legacy User"),
        )
        conn.execute("CREATE TABLE mmfdb_schema_version (version INTEGER)")
        conn.execute("INSERT INTO mmfdb_schema_version (version) VALUES (40)")
        conn.commit()
    finally:
        conn.close()
    ...
    try:
        ...
        assert "mmfdb_session" in table_names
        assert "mmfdb_group" in table_names
        users = db.get_users()
