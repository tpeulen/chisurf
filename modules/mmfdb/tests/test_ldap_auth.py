...
from mmfdb.security.auth import AuthError, authenticate_token  # noqa: E402
from mmfdb.security.auth_providers import AuthIdentity, LdapAuthProvider  # noqa: E402
...
def test_ldap_missing_dependency_raises(monkeypatch) -> None:
    import mmfdb.security.auth_providers as ap
    ...
    with MFDatabase(tmp_path / "ldap.db") as db:
        ...
        groups = {
            ...
            for r in db.conn.execute(
                "SELECT group_id FROM mmfdb_group_member WHERE user_id='jdoe'"
            )
        }
...
def test_configured_auth_config_env_and_resolver(monkeypatch) -> None:
    ...
    monkeypatch.setenv("MMFDB_AUTH_PROVIDER", "ldap")
    monkeypatch.setenv("MMFDB_LDAP_HOST", "ldap.lab.org")
    monkeypatch.setenv("MMFDB_LDAP_BASE_DN", "ou=people,dc=lab")
