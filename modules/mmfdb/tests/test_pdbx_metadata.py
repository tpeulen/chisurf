...
from mmfdb.schema.pdbx_metadata import MmcifDictionary, DictItem, DictCategory
...
def test_sample_search_accepts_valid_dictionary_field():
    ...
    from mmfdb.samples.sample_requests import SampleSearchRequest
...
def test_stats_cli_runs():
    ...
    result = subprocess.run(
        [sys.executable, "-m", "mmfdb.schema.pdbx_metadata", "--stats"],
        check=False,
        capture_output=True,
        text=True,
    )
...
def test_list_categories_cli():
    ...
    result = subprocess.run(
        [sys.executable, "-m", "mmfdb.schema.pdbx_metadata", "--list-categories"],
        check=False,
        capture_output=True,
        text=True,
    )
...
def test_flr_categories_cli():
    ...
    result = subprocess.run(
        [sys.executable, "-m", "mmfdb.schema.pdbx_metadata", "--flr-categories"],
        check=False,
        capture_output=True,
        text=True,
    )
...
def test_enums_cli():
    ...
    result = subprocess.run(
        [sys.executable, "-m", "mmfdb.schema.pdbx_metadata", "--enums", "_flr_sample_probe_details.fluorophore_type"],
        check=False,
        capture_output=True,
        text=True,
    )
...
def test_search_cli():
    ...
    result = subprocess.run(
        [sys.executable, "-m", "mmfdb.schema.pdbx_metadata", "--search", "forster radius"],
        check=False,
        capture_output=True,
        text=True,
    )
...
def test_validate_cli():
    ...
    # Test valid value
    result = subprocess.run(
        [sys.executable, "-m", "mmfdb.schema.pdbx_metadata", "--validate", "_flr_sample_probe_details.fluorophore_type", "donor"],
        check=False,
        capture_output=True,
        text=True,
    )
    ...
    # Test invalid value
    result = subprocess.run(
        [sys.executable, "-m", "mmfdb.schema.pdbx_metadata", "--validate", "_flr_sample_probe_details.fluorophore_type", "invalid"],
        check=False,
        capture_output=True,
        text=True,
    )
