...
def test_record_parameter_writes_audit_log(tmp_path: pathlib.Path) -> None:
    ...
    with MFDatabase(db_path) as db:
        ...
        audit = db.conn.execute(
            "SELECT action, target_type, target_id FROM mmfdb_audit_log WHERE target_type = 'parameter'"
        ).fetchone()
...
def test_record_operation_rolls_back_on_audit_failure(tmp_path: pathlib.Path) -> None:
    ...
    with MFDatabase(db_path) as db:
        ...
        row = db.conn.execute(
            "SELECT 1 FROM mmfdb_operation WHERE operation_id = ?", ("atomic_op",)
        ).fetchone()
        assert row is None
...
def test_record_operation_link_rolls_back_on_audit_failure(tmp_path: pathlib.Path) -> None:
    ...
    with MFDatabase(db_path) as db:
        ...
        row = db.conn.execute(
            "SELECT 1 FROM mmfdb_operation_artifact WHERE operation_id = ? AND artifact_id = ?",
            ("link_op", "link_art"),
        ).fetchone()
        assert row is None
...
def test_fresh_mmfdb_edge_rejects_operation_relationships(tmp_path: pathlib.Path) -> None:
...
def test_fresh_database_uses_flr_sample_as_sample_source_of_truth(tmp_path: pathlib.Path) -> None:
    ...
    with MFDatabase(db_path) as db:
        ...
        assert "mmfdb_sample" not in tables
...
def test_manifest_missing_methods_detected(tmp_path: pathlib.Path) -> None:
    ...
    assert "mmfdb.v1.artifacts.register" in missing


# ── 4. Migration report contents ─────────────────────────────────────────


def test_fresh_mmfdb_edge_enforces_active_vocabulary(tmp_path: pathlib.Path) -> None:
    ...
    with MFDatabase(db_path) as db:
        ...
        db.conn.execute(
            "UPDATE mmfdb_vocabulary SET is_active = 0 WHERE field_name = 'relationship_type' AND value = 'custom_rel'"
        )
        with pytest.raises(sqlite3.IntegrityError):
            db.conn.execute(
                """INSERT INTO mmfdb_edge (
...
def test_add_processing_run_rolls_back_audit_failure(tmp_path: pathlib.Path) -> None:
    ...
    with MFDatabase(db_path) as db:
        ...
        assert db.conn.execute("SELECT COUNT(*) FROM mmfdb_audit_log").fetchone()[0] == 1


# Note: the former test_migrated_mmfdb_edge_* tests exercised the removed
...
# covered on a fresh database by test_fresh_mmfdb_edge_enforces_active_vocabulary and
# test_fresh_mmfdb_edge_rejects_operation_relationships above.
...
def test_migration_report_dataclass_works() -> None:
    ...
    from mmfdb.schema.schema import MigrationReport
