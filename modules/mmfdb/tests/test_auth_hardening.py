...
def test_throttle_persists_across_rpc_logins(tmp_path: Path, monkeypatch) -> None:
    ...
    db = MFDatabase(db_path)
    try:
        n = db.conn.execute("SELECT COUNT(*) FROM mmfdb_auth_attempt WHERE success=0").fetchone()[0]
...
def _groups(db, user_id):
    return {
        ...
        for r in db.conn.execute(
            "SELECT group_id FROM mmfdb_group_member WHERE user_id=? AND deleted_at IS NULL",
            (user_id,),
        )
    }
...
def test_login_reconciles_attributes_and_groups(tmp_path: Path, monkeypatch) -> None:
    ...
    with _db(tmp_path) as db:
        for g in ("g_a", "g_b"):
            db.conn.execute(
                "INSERT OR IGNORE INTO mmfdb_group (group_id, display_name) VALUES (?, ?)", (g, g)
            )
        db.conn.commit()
...
def test_reconcile_does_not_touch_unmanaged_groups(tmp_path: Path, monkeypatch) -> None:
    ...
    with _db(tmp_path) as db:
        ...
        db.conn.execute(
            "INSERT INTO mmfdb_group_member (group_id, user_id, role) VALUES ('local_only','u','member')"
        )
        db.conn.commit()
        login_mod.login(db.conn, provider="ldap", user_id="u", password="pw")
