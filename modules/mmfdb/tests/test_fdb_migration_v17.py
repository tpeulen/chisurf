...
from mmfdb.schema.schema import migrate_schema, set_schema_version, get_schema_version, SCHEMA_VERSION
from mmfdb.repository import MFDatabase
from mmfdb import api as fdb_api
...
def test_idempotent_updates(tmp_path: pathlib.Path) -> None:
    ...
    with MFDatabase(db_path) as db:
        ...
        assert len(db.conn.execute("SELECT * FROM mmfdb_operation_artifact WHERE operation_id = 'op1'").fetchall()) == 1
    ...
    with MFDatabase(db_path) as db:
        ...
        with pytest.raises(Exception):
            with db.transaction():
                ...
                db.conn.execute("INSERT INTO mmfdb_artifact (artifact_id, artifact_kind) VALUES ('art1', 'invalid')")
...
def test_cycle_safe_graph_traversal(tmp_path: pathlib.Path) -> None:
    ...
    with MFDatabase(db_path) as db:
        ...
        # input_to and produced go into mmfdb_operation_artifact (canonical source of truth)
        db.conn.execute("""
            INSERT INTO mmfdb_artifact (artifact_id, artifact_kind, storage_mode)
            VALUES ('A', 'raw_data', 'local'), ('C', 'processed_data', 'local')
        """)
        db.conn.execute("""
            INSERT INTO mmfdb_operation (operation_id, operation_type, status)
            VALUES ('B', 'burst_selection', 'succeeded')
        """)
        db.conn.execute("""
            INSERT INTO mmfdb_operation_artifact (operation_id, artifact_id, direction, role)
        ...
        """)
        # derived_from goes into mmfdb_edge (non-operation relationship)
        db.conn.execute("""
...
def test_json_rpc_versioned_services(tmp_path: pathlib.Path) -> None:
    ...
    with patch("mmfdb.api.resolve_database_path", return_value=db_path):
