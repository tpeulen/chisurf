"""Tests for dictionary-authoritative MMFDB schema mapping."""
...
from mmfdb.schema import dictionary_schema_map as mapping_module
...
@pytest.fixture()
def mmfdb_path(tmp_path):
    """Return a temporary MMFDB path with the canonical schema installed."""
...
@pytest.fixture()
def mapper(mmfdb_path):
    """Return a mapper built from dictionaries and live schema."""
    return build_dictionary_schema_map(mmfdb_path)
...
def test_unsupported_items_are_derived_from_live_schema(mmfdb_path):
    """Unsupported item reporting is computed from the live schema."""
    mapper = build_dictionary_schema_map(mmfdb_path)
    unsupported = unsupported_flr_items(mmfdb_path)
...
def test_convenience_function_uses_same_mapping(mmfdb_path):
    """Module-level lookup uses the dictionary-authoritative mapper."""
    mapped = map_dictionary_item("_flr_sample_condition.ph", mmfdb_path)
    assert mapped is not None
