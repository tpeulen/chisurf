...
chimol's in-viewport chrome — `chimol/emtk/` — is a **port**, not an
...
* **`emtk/control.py`** — the contract as a class rather than as prose:
...
`emtk/qt_host.py` is the fourth piece and the one that changes what a
