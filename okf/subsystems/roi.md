...
A second tranche came from the same reference after the removal, mined
deliberately rather than on demand (see
[scikit-image mining](/references/scikit-image-mining.md) for the whole survey
and what was left): `remove_small_objects`, `remove_small_holes`,
`relabel_sequential`, `expand_labels`, `find_boundaries`, `find_contours`,
`difference_of_gaussians`, `white_tophat`, and the multi-scale spot detectors
`blob_dog` / `blob_log`. Three of them earn their place immediately:

* **`remove_small_objects`** replaced a loop in molecule MLE that compared the
  whole frame once per label — 17–24 s against 15 ms on a field of 92,016
  components.
* **`relabel_sequential`** fixed the bug behind it: deleting labels leaves gaps,
  and every consumer that reads a label image as "1 to max" — `regionprops`
  among them — then reports regions that own no pixel.
* **`blob_log` / `blob_dog`** are a capability the tree did not have. The
  particle tracker detects spots at one fixed scale; these build a scale space
  and return each spot's *width* as well as its position, which is what an image
  of unknown focus needs.

`find_contours` is the way back from a raster to geometry — a traced
segmentation can become a `PolygonROI`, editable and resolution-independent,
where a mask is neither.
