...
- MMFDB today: `project_id TEXT` on `mmfdb_sample`/`flr_sample`/`mmfdb_experiment` —
  no project table, no membership, no custom fields.

# Design (dictionary-driven, reuse existing patterns)
- **`mmfdb_study`** (`.dic`-declared, generated, gate-covered):
  ...
  `mmfdb_artifact_owner`-style scoping where multi-owner is wanted).
- **`mmfdb_study_member`** (`.dic`-declared): `(study_id FK, member_type, member_id,
...
- **Configurable fields:** do **not** build a new EAV stack. Reuse the existing
  key-value pattern (`flr_sample_key_value` / `mmfdb_*_key_value`) with a
  `mmfdb_study_key_value(study_id, key, value, details)` for per-study metadata.
  Optionally a `mmfdb_study_field_def(study_id, name, value_type, units, required,
...
- **Browser facet:** `browse_datasets` gains a `study_id` filter (artifacts whose
  `artifact_id` is a `mmfdb_study_member`, or whose sample is a member). The
...
# Tasks
1. `.dic` + schema: declare `mmfdb_study`, `mmfdb_study_member`,
   `mmfdb_study_key_value` (and optional `mmfdb_study_field_def`); generate DDL;
   `SCHEMA_VERSION` bump; add to the total-coverage gate. Backfill `mmfdb_study`
...
5. mmfdb-admin: a Studies entity (dictionary-sourced columns) with members + fields.
...
# Definition of Done
- [x] `mmfdb_study` + `mmfdb_study_member` + `mmfdb_study_key_value` exist, dict-
...
**Increment 1 (schema + CRUD + membership + fields).** `mmfdb_study` /
`mmfdb_study_member` / `mmfdb_study_key_value` are `.dic`-declared and created by
...
manages integrity, as for `mmfdb_operation.protocol_id`). `test/fio/test_study.py`.
...
**Increment 3 (admin).** Backend handlers `mmfdb.studies.{list,get,create,
members.add,fields.set}` + `MMFDBClient` methods + a standalone
