...
# Goal
Replace opaque JSON blobs in `mmfdb_setup` with structured, queryable tables that
...
# Background
Relevant code:
- `chisurf/core/mmfdb/schema.py` — current `flr_instrument`, `flr_inst_setting`,
  `mmfdb_setup` definitions; migration helpers
- `chisurf/core/mmfdb/repository.py` — `MFDatabase` methods
- `chisurf/plugins/core/mmfdb_admin/gui/entity_registry.py` — how new entities
...
| `mmfdb_setup` | Full setup config stored as JSON blobs | Opaque to SQL queries and GUI |
...
base table `mmfdb_setup_detector_channel` (detector channels: `channels`,
`micro_time_ranges`, `g_factor`, …) and `mmfdb_setup_pie_window`. This PRD's
`mmfdb_optical_channel` is the *spectroscopic extension* of that detection channel,
**not** a parallel concept. `mmfdb_optical_channel` carries a
`detector_channel_id REFERENCES mmfdb_setup_detector_channel(...)` link (populated
...
- **Hardware level** (FilterSet equivalent): each physical component defined once
  per instrument — `mmfdb_light_source`, `mmfdb_optical_filter`, `mmfdb_dichroic`,
  `mmfdb_objective`, `mmfdb_detector`.
- **Channel level** (LightPath equivalent): `mmfdb_optical_channel` says which
  hardware components are active in a given setup and in what role.
- **Measurement level**: `mmfdb_channel_setting` captures per-measurement overrides
...
- **`mmfdb_light_source`** (PK `light_source_id`): `source_type` (laser/led/lamp/
  ...
  `tunable`, `details`.
- **`mmfdb_optical_filter`** (PK `filter_id`): `filter_type` (bandpass/longpass/
...
- **`mmfdb_detector`** (PK `detector_id`): `detector_type` (SPAD/APD/PMT/EMCCD/
  ...
  `details`.
- **`mmfdb_tcspc_board`** (PK `board_id`, critical for TCSPC/smFRET, absent from
...
- **`mmfdb_optical_channel`** (PK `channel_id`, `setup_id` NOT NULL REFERENCES
  `mmfdb_setup(setup_id)` ON DELETE CASCADE): `channel_name`, `channel_role`
...
- **`mmfdb_channel_setting`** (PK `setting_id` AUTOINCREMENT, `channel_id` NOT NULL
  REFERENCES `mmfdb_optical_channel` ON DELETE CASCADE): `measurement_id` (FK to
  raw_data/`mmfdb_artifact`), `laser_power_pct`, `laser_power_mw`, `detector_gain`,
  `detector_voltage`, `integration_time_ms`, `count_rate_hz`, `rep_rate_mhz`,
  `tac_range_ns`, `notes`.

Add indexes on `mmfdb_optical_channel(setup_id)`,
`mmfdb_channel_setting(channel_id)`, `mmfdb_channel_setting(measurement_id)`.

## Task 3: Migration helper (schema.py)
Add `_migrate_setup_to_optical_channels(conn, setup_id)` that reads existing
`mmfdb_setup.detectors_json` / `configuration_json` blobs and populates
`mmfdb_optical_channel` rows (parses a `{channel_name: {detector_type, role,
...
`chisurf/plugins/core/mmfdb_admin/gui/entity_registry.py` — add `EntitySpec`
entries so the admin GUI shows tabs for each new table under the "Instrument"
group (`mmfdb_light_source`, `mmfdb_optical_filter`, `mmfdb_dichroic`,
`mmfdb_objective`, `mmfdb_detector`, `mmfdb_tcspc_board`) and the "Setup" group
(`mmfdb_optical_channel`), each `writable=True` with the appropriate `id_field` and
display columns.

## Task 5: flrCIF export serialization
`chisurf/core/mmfdb/pdbx_metadata.py` (or a new `optical_export.py`) — add
`export_optical_channels_to_flr_inst_setting(db, setup_id, instrument_id)` that
serializes `mmfdb_optical_channel` rows into `FLR_INST_SETTING` key-value dicts
...
## Task 6: Dictionary entries (`mmfdb_flr_ext.dic`)
...
`_item_enumeration.value` loops for `mmfdb_light_source.source_type` (laser/led/
lamp/other), `mmfdb_optical_filter.filter_type` (bandpass/longpass/shortpass/notch/
multipass/dichroic), and `mmfdb_optical_channel.channel_role` (donor/acceptor/vv/
...
its light-source and detector components; a per-measurement `mmfdb_channel_setting`
...
|---|---|
| `LaserLine` (excitation source) | `mmfdb_light_source` |
| `FilterSetting` (excitation_filter / emission_filter / dichroic / splitter) | `mmfdb_optical_filter` / `mmfdb_dichroic` |
| `DetectorSetting` (detector channel) | `mmfdb_detector` + `mmfdb_optical_channel` |
| `FluorophoreSetting` | `flr_probe_list` (existing) |
| `InstrumentSetting` (composite) | `flr_instrument` + `mmfdb_channel_setting` |
...
2. **Visualize** a stored setup: load its optical-config rows from MMFDB back into
   the node graph.
3. **Validate / derive**: the simulator already computes crosstalk + R₀ overlap
   ...
   filter choices.
4. **Link, don't duplicate**: `mmfdb_optical_channel.detector_channel_id`
   references the detection channel base table (`mmfdb_setup_detector_channel`); the
...
chisurf-DB-free; MMFDB read/write lives in the plugin's backend services, mirroring
the PRD-08 repository methods.

# Definition of Done
- [ ] Tables `mmfdb_light_source`, `mmfdb_optical_filter`, `mmfdb_dichroic`,
      `mmfdb_objective`, `mmfdb_detector`, `mmfdb_tcspc_board` exist in schema
- [ ] Tables `mmfdb_optical_channel`, `mmfdb_channel_setting` exist in schema
...
- [ ] New tables appear in mmfdb-admin GUI under "Instrument" group
...
# Relationships
- Extends the detection-channel base tables introduced with the burst pipeline work; `mmfdb_optical_channel` links to them rather than duplicating channels.
