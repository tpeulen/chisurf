...
registered in MMFDB with stable, queryable provenance (raw TTTR artifacts →
...
analysis API stays the source of truth for computing burst results; MMFDB
...
definitions and the TTTR reading routine — a first-class MMFDB record so a burst
run can be traced back to what setup produced the photons.

# Status
In progress. Burst Selection is the reference implementation for
workflow-ready plugins with MMFDB archival; it must not reimplement storage,
operation/parameter recording, or payload serialization owned by the registry
and codec layers. The main body is implemented: `chisurf/core/mmfdb/pipeline.py`
is removed, `chisurf/plugins/burst/burst_selection/api/mmfdb.py` holds
`BurstMMFDBPipeline`, and `register_result()` / `register_raw_measurement()`
...
MMFDB registration is layered on top of the analysis result. PRD-04 must not
...
- **Keep analysis pure.** `analyze_request(request)` computes and writes requested
  files; it must not require MMFDB and stays directly testable without a database.
- **Register after success.** MMFDB registration runs only after analysis succeeds.
...
- **Preserve optionality.** MMFDB failure must not break burst analysis in any
  flow; produce warnings and empty artifact IDs.
- **Avoid core→plugin dependencies.** Burst orchestration lives under
  `chisurf/plugins/burst/burst_selection/`; core MMFDB modules expose only generic
  helpers.
- **Keep the external contract explicit and canonical.** MMFDB context is nested
  under `mmfdb`; legacy top-level provenance fields and RPC aliases are not part of
  the reference implementation.

# Prerequisite: setup and channel definitions in MMFDB
Full provenance tracing is only possible if the *setup* that produced the photons
is itself a first-class, queryable MMFDB record. In chisurf the setup is defined by
...
## Current gap
- Detector setups persist to `mmfdb_setup` as **opaque JSON blobs**
  (`configuration_json` / `detectors_json` / `timing_resolution_json` /
  `burst_defaults_json`): not queryable, not validated.
- `mmfdb_flr_ext.dic` has **no save block** for `mmfdb_setup` or the detector /
  channel/window definitions, so admin cannot resolve field descriptions or
  enumerate valid values.
- mmfdb-admin's "Setups" entity shows only setup-level rows; the definitions inside
  the blobs are not structured records.
- `selected_setup` is a free-text label; nothing guarantees a matching row exists
  and `mmfdb_operation.setup_id` is not populated, so the chain has no setup node.

## Required outcome
1. **Setup persisted as a resolvable record.** Each named setup is an `mmfdb_setup`
   row addressed by the deterministic id from `setup_id_for_name()`
   (`tttr_detector_setup:<slug>`). The pipeline resolves `request.mmfdb.setup_id`
...
3. **flrCIF / `.dic` alignment — the dictionary is the single source of truth.**
   `mmfdb_flr_ext.dic` is authoritative for item names, types (`_item_type.code`),
...
4. **Correct mmfdb-admin display** of the setup and its detector/window definitions
   as structured rows; selecting a `burst_table` navigates to its setup.
5. **Operation links to setup.** `mmfdb_operation.setup_id` is populated for the
...
1. Author the field vocabulary once in `mmfdb_flr_ext.dic` — one item block per
...
2. Bind every item to its column with the mandatory `_chisurf_schema.table_name`
   / `_chisurf_schema.column_name` bridge — the new tables (`mmfdb_setup`,
   `mmfdb_setup_detector_channel`, `mmfdb_setup_pie_window`) are not `flr_`-prefixed,
...
- **P1 — reading/processing setup schema.** Decision (locked): **Option A, child
  ...
  `mmfdb_setup_detector_channel` (one row per channel: `setup_id` FK ON DELETE
  ...
  only for genuinely list-valued fields, scalars typed); `mmfdb_setup_pie_window`
  ...
  as **real typed columns** on `mmfdb_setup` — the blob fallback is not acceptable.
...
- **P1a — generate table DDL from the dictionary.** Add
  `chisurf/core/mmfdb/schema_from_dictionary.py`: for each bridged category emit
...
- **P4 — mmfdb-admin display** of the setup and its channel definitions as
...
- **P5 — resolve and link the setup in the burst operation.** `MMFDBContext.setup_id`
  ...
  forwarding, look up the `mmfdb_setup` row; if absent, warn and forward an empty
  id so `mmfdb_operation.setup_id` stays NULL (never a dangling FK). Record
...
- **Sidequest (per-user setups + public sharing).** Detector setups are a
  ...
  `flr_sample_users(user_id)`) and `is_public` columns to `mmfdb_setup`
  ...
  legacy JSON is deleted (MMFDB is authoritative). Reads return own + public +
...
- **Sidequest B (FCS channel definitions — same treatment).** FCS channel-pair
  setups are the FCS analogue and get the identical pattern: stored as `mmfdb_setup`
  rows (`setup_type="fcs_channel_setup"`, user-namespaced id) with a structured
  child table `mmfdb_setup_fcs_pair` (per pair: `setup_id` FK, `name`, `channel_a`,
  `channel_b`, `kind`). **Correlator settings are per pair** (locked):
  `n_bins`/`n_casc`/`make_fine` are dictionary-declared typed columns on
  `mmfdb_setup_fcs_pair` — not a JSON blob; the setup-level columns become defaults
  seeding a new pair. mmfdb-admin shows the FCS setup and pair rows structured;
  ...
  construction smoke test. Default Save goes to MMFDB only (no user-folder JSON;
  the confirmation names MMFDB); legacy JSON is migrated-and-removed; JSON becomes
  export-only via an explicit path. No-MMFDB Save degrades **softly** — it still
...
- **Sidequest C (g-factor calibration provenance).** The reference decay (VV/VH
  ...
  `mmfdb_setup_detector_channel` linking a channel to its calibration. Fix the
  broken g-factor plugin GUI/MMFDB path and the FCS read-only-construction
  ...
  reference decay (VV/VH file)       [raw_measurement / tcspc_decay artifact]
    ...
    -> consumed by mmfdb_setup_detector_channel  [g_factor/l1/l2 + calibration link]
  ```
- **Sidequest D (time-versioned setup calibration).** A setup drifts (detectors
  ...
  (routing, PIE windows, TTTR reading — stays on `mmfdb_setup` / child tables) from
  ...
  `mmfdb_setup_calibration` (`calibration_snapshot_id` PK, `setup_id` FK,
  ...
  a snapshot; `mmfdb_setup_detector_channel` factors become a cache of the latest.
...
`chisurf/plugins/burst/burst_selection/api/mmfdb.py` (plugin-owned; may import
Burst Selection API models and result-registry primitives).
`chisurf/core/mmfdb/pipeline.py` is **removed**; core MMFDB exports no
`BurstPipeline`.

## Request context
Add a nested MMFDB context to `AnalysisRequest` (so future provenance attributes
...
accept `{"mmfdb": {"sample_id": …, "source_artifact_ids": {…}}}`; top-level
...
role map), `mmfdb_artifacts: dict` (filled by the registration layer), and
`warnings: list[str]` (non-fatal MMFDB failures).

## Pipeline API
`api/mmfdb.py` defines `BurstRegistrationResult` (`input_artifacts`,
...
for sidecars) and `BurstMMFDBPipeline(db=None).register_run(request, result)` which:
returns empty immediately when `mmfdb.enabled` is false; normalizes paths with
...
## Error handling
MMFDB registration is best-effort: missing DB, invalid `sample_id`, invalid source
...
`BurstMMFDBPipeline` tests verify failure paths report and leave no partial rows.

# Tasks (main body)
1. **Replace the legacy core pipeline boundary** — delete
   `chisurf/core/mmfdb/pipeline.py`, remove `BurstPipeline` from exports and the
   stale vocabulary (`artifact_type`, `storage_mode="local"`, `status="success"`),
   and stop executing burst analysis from core.
2. **Add MMFDB request/result contract fields** (`MMFDBContext`, `AnalysisRequest.mmfdb`,
   `output_paths_by_file`, `mmfdb_artifacts`, `warnings`, nested-`mmfdb` payload
   normalization, contract descriptor docs). Existing payloads without `mmfdb` still
   work; `_to_payload()` round-trips `mmfdb` JSON-safely.
...
4. **Implement the plugin-owned MMFDB pipeline** (`BurstRegistrationResult`,
   `BurstMMFDBPipeline`, `extract_burst_parameters`, `build_burst_metadata`) using
   the result registry.
5. **Register from the service adapter** — after `analyze_request(request)`, when
   `request.mmfdb.enabled`, call `BurstMMFDBPipeline().register_run(...)`, set
   `result.mmfdb_artifacts`, extend warnings. Do not register from pure
   `analyze_request()` unless via an explicit `analyze_and_register_request()`.
6. **Pass MMFDB context from GUI/client** — optional `mmfdb` on
   ...
   failing the run.
7. **Focused tests** (`test/fio/test_burst_pipeline_mmfdb.py`): one raw artifact per
   ...
   warnings when MMFDB unavailable; two-file per-file path handling; zero rows left
   for a failed invalid-sample registration. Plugin-local tests: nested `mmfdb`
   payload acceptance, `mmfdb_artifacts`/warnings in the result payload, service
   handler returns `mmfdb_artifacts` on a successful enabled run. Use the real
   `MFDatabase` and `db.conn` (not `db.con`).

# Definition of Done
- Prerequisite: each named setup resolves to an `mmfdb_setup` row; detector/window
  definitions are structured `mmfdb_setup_detector_channel` / `mmfdb_setup_pie_window`
  rows, not blobs.
- Prerequisite: `mmfdb_flr_ext.dic` dictates the schema — every new item declares
  ...
  in mmfdb-admin with dictionary-sourced labels/enums.
- Prerequisite: the `burst_selection` operation populates `mmfdb_operation.setup_id`.
- Core no longer owns or executes Burst Selection analysis;
  `chisurf/core/mmfdb/pipeline.py` is removed.
- Request contract includes `MMFDBContext`; result contract preserves per-file
  output paths and returns MMFDB artifact IDs/warnings.
- Plugin-owned `BurstMMFDBPipeline` registers inputs and burst-table outputs through
  ...
  provided.
- MMFDB registration failures never fail GUI/CLI/RPC analysis; mmfdb-admin lists
  registered `burst_table` artifacts and their provenance edges.
- Focused MMFDB tests and Burst Selection plugin tests pass.
...
- **Sidequest B (FCS).** Architecture sound (shared `tttr_setup_utils`, structured
  `mmfdb_setup_fcs_pair`, dict-declared correlator columns, gate extended), but the
  ...
  regressions.
- **Sidequest B addendum 2 (MMFDB-default save).** Fixed the confirmation message
  hard-coding a JSON path and the silent user-folder JSON write. Locked policy: no-
  MMFDB save is a **soft fallback with warning** (still succeeds via JSON, warns the
...
- Reference archival path over the [MMFDB (current)](/architecture/mmfdb.md) store for a [plugin](/architecture/plugin-system.md); relates to [Plugins target](/specs/plugins.md).
