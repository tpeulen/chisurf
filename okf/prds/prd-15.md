...
# Summary
This PRD adds a minimal consumable inventory so a result ties to exactly what was used — which dye lot, which filter set, which buffer. `mmfdb_reagent_lot` records lots with vendor, catalog/lot number, concentration, and expiry, and an orthogonal many-to-many `mmfdb_reagent_usage` links a lot to an operation, setup, or sample without adding columns to those tables. A fluorophore lot may optionally reference a known probe, connecting inventory to the fluorophore database. The repository API covers lot CRUD, usage links, and expiry queries (including a QC "expired lots" helper), and an admin view lists/filters lots and shows per-lot usage. It is the lowest-priority LIMS layer, intended for shared-MMFDB or strict-reproducibility use.

# Status
Done. Dictionary-declared lot and usage tables, the reagents API, and an mmfdb-admin view landed (headless + view tests). Reworked to drop the schema-version bump and hard foreign keys per the disposable-DB policy.
...
- LIMS reference: a mature LIMS tracks commercial kits, per-user lots, library
  ...
  runs.
- MMFDB today: instruments and setups are modelled, but consumables (dye lots,
  buffers, filters) are not inventoried. Lower priority for a single-lab tool;
  valuable as MMFDB is shared and for strict reproducibility.

# Design (dictionary-driven, minimal)
- **`mmfdb_reagent_lot`** (`.dic`-declared, generated, gate-covered):
...
- **Usage links (many-to-many):** `mmfdb_reagent_usage` (`.dic`-declared):
  `(id PK, lot_id FK mmfdb_reagent_lot, target_type, target_id, role, created_at)`,
...
- **Optional probe link:** where a fluorophore lot corresponds to a known probe,
  allow `mmfdb_reagent_lot` → `probes(probe_id)` via a nullable FK, connecting the
...
# Tasks
1. `.dic` + schema: declare `mmfdb_reagent_lot` + `mmfdb_reagent_usage` (+ kind /
...
4. mmfdb-admin: a Reagent Lots entity (dictionary-sourced columns) + a usage view
...
# Status — complete (headless core + mmfdb-admin view)
`mmfdb_reagent_lot` + `mmfdb_reagent_usage` are dictionary-declared in
`mmfdb_flr_ext.dic` (auto-created via `reconcile_schema`; audit columns
auto-appended). The API lives in `chisurf/core/mmfdb/reagents.py`: `add_reagent_lot`
...
PK; the repository manages integrity — the same convention as `mmfdb_study_member`).

# Definition of Done
- [x] `mmfdb_reagent_lot` + `mmfdb_reagent_usage` exist, dict-declared, generated;
...
- [x] mmfdb-admin lists lots, filters by kind, toggles expired, shows per-lot
      detail, and creates lots — `gui/reagents_view.py` (`ReagentLotsView`) over
      `mmfdb.reagents.*` handlers + `MMFDBClient` methods. Headless-tested
...
# Scope note
Lowest priority of the LIMS layers P1–P4. Implement when MMFDB is shared across
