...
type: PRD
prd: "45"
title: "PRD-45: Chemical registry-number as a first-class chemical identity in MMFDB"
...
- `mmfdb_adapter.register_component(..., cas=...)` — convenience parameter that records the registry number for a scraped component.

This PRD promotes that property-bag stopgap into a dictionary-defined, validated, indexed identity.

# Goals & Constraints
1. **Dictionary-defined.** The registry number is declared in the `.dic` schema authority (a `_mmfdb_schema`-mapped item), not hand-added DDL — consistent with the rule that the `.dic` family is the schema source of truth.
...
## 1. Dictionary + schema
- Add a registry-number item to `mmfdb_flr_ext.dic` (e.g. on the probe/chem-component category) with `_mmfdb_schema.table_name`/`.column_name` mapping it to a real column, an enumeration-free `code`/`line` type, and a description. Regenerate DDL via `schema_from_dictionary.py` (do not hand-edit generated DDL).
...
## 2. Validation & normalisation (`chisurf/core/mmfdb/`)
...
- Expose through the transport-agnostic `api.py` and the mmfdb-admin RPC (`chemistry.cas.lookup`, `chemistry.cas.set`).
...
- `chisurf/core/chem/cas_resolver.py`: registry number → external compound identifier → canonical name / InChIKey / structure, cache-first (store resolved metadata in MMFDB), fully offline-safe (no network ⇒ no-op). Gated like the AI settings: never block ingest on network.

# Files (anticipated)

| File | Change |
|------|--------|
| `chisurf/core/mmfdb/data/mmfdb_flr_ext.dic` | Declare the registry-number item + `_mmfdb_schema` mapping |
| `chisurf/core/mmfdb/schema*.py` | Regenerated DDL + index + migration of existing `cas` properties |
| `chisurf/core/mmfdb/cas.py` (NEW) | `normalize_cas`, check-digit validation |
| `chisurf/core/mmfdb/repository.py` | indexed `find_probes_by_cas`, `set_probe_cas`, `resolve_chemical_identity` |
| `chisurf/core/mmfdb/api.py` | transport-agnostic registry-number functions |
| `chisurf/plugins/core/mmfdb_admin/backend/*` | `chemistry.cas.*` RPC |
| `chisurf/plugins/core/mmfdb_admin/gui/optical_components/*.view.json` | registry-number field + filter |
| `chisurf/plugins/core/mmfdb_admin/cli/__init__.py` | `cas` lookup command |
...
- Depends on the vendor-neutral dictionary namespace from [PRD-44](prd-44.md) (the identifier is a `_mmfdb_schema`-mapped item).
- Follows the fail-loud policy of PRD-25 for malformed values.
- Extends the [MMFDB (current)](/architecture/mmfdb.md) toward the [MMFDB target](/specs/mmfdb.md); surfaced via [GUI & AutoForm](/subsystems/gui-autoform.md) and the [RPC target](/specs/rpc.md).
