...
type: PRD
prd: "09"
title: "PRD-09: Microtime Shifter — Workflow Plugin + MMFDB Provenance"
description: Convert the microtime-shifter tool into a layered api/backend/cli/gui workflow plugin with RPC and full MMFDB provenance.
status: done
phase: "0"
...
`core`/`api`/`cli`/`gui` split that communicates via RPC, with full MMFDB
...
per-channel shift values are persisted as **dictionary-declared** MMFDB data so the
...
read/write of shifted files. No split, no RPC, no MMFDB. Burst Selection is the
canonical workflow-plugin template (its `api/contract.py`, `api/models.py` +
`MMFDBContext`, `api/selection.py`, `api/mmfdb.py`, `backend/services.py`,
`cli/main.py`, `gui/client.py` + `gui/tool.py`).

# Governing rules
- **MMFDB is the store; the `.dic` dictates the schema.** Anything structured the
  plugin persists (the shift values especially) is declared in
  `chisurf/core/mmfdb/data/mmfdb_flr_ext.dic` with `_chisurf_schema` bridges and is
  covered by the total-coverage `validate_mapping()` gate. No opaque JSON blobs
  for the shift values.
- MMFDB registration is best-effort for the GUI flow — failures warn, never crash
  the shifter.
...
1. **Layer purity.** `api/shift.py` has no Qt and no DB imports — pure functions,
   ...
   `tttrlib` read/write and no MMFDB calls in the widget. `backend/services.py` is
   the only place wiring pure-API → MMFDB.
...
4. **Best-effort MMFDB, but fail loud on real bugs.** MMFDB-unavailable warns and the
   shift still works; genuine errors (bad migration, missing column, failed
   registration) must surface, not be swallowed.
5. **Tests assert behavior, not existence.** MMFDB tests assert the `derived_from`
...
1. **API layer (`api/`).** `api/models.py`: `ShiftRequest` (`files`,
   ...
   `mmfdb: MMFDBContext`) and `ShiftResult` (`output_paths_by_file`,
   `applied_shifts_by_file`, `mmfdb_artifacts`, `warnings`). `api/shift.py`: **pure**
   ...
   `api/mmfdb.py`: registration (Task 4).
2. **Backend service + RPC (`backend/services.py`).** RPC handlers on the ChiSurf
   ...
   API then `api/mmfdb.py`.
3. **CLI (`cli/main.py`).** Headless entry: input file(s), shifts, output dir; run
   the pure API; optionally register in MMFDB.
4. **MMFDB treatment — connectivity-aware save (two modes).**
   - **Connected to MMFDB → register, don't write a loose file.** Save writes the
     ...
     `derived_from` edge). The GUI confirms "shifted data registered in MMFDB".
   - **Not connected → old behaviour.** A Save dialog writes the shifted TTTR to a
     ...
     work is never lost). "Connected" means a usable MMFDB resolves
     ...
     On load/drop, compute the file content hash and look it up in `mmfdb_object`
     (by `content_md5`); reuse the existing artifact/object if present, else
     register via `register_raw_measurement`; surface "already in MMFDB" vs "newly
...
5. **Shift values need a `.dic` entry (structured, dictionary-declared).**
   Preferred: a structured child table `mmfdb_microtime_shift` — one row per
   (operation/artifact, routing_channel) with `shift` (micro-time channels), plus a
   `global_shift` at the operation/artifact level; declared in `mmfdb_flr_ext.dic`
   ...
   `mmfdb_parameter` rows named `shift_ch<idx>` plus `global_shift`, with matching
...
6. **GUI (`gui/`) — client + dockable tool.** `gui/client.py`: RPC client wrapping
   ...
   and keeps an explicit "Save to file…"; show MMFDB status without blocking.
...
8. **Tests.** Pure `api/shift.py` (known shift → expected micro times;
   read→shift→write→read round-trip); contract round-trip; MMFDB (dropped file
...
- Dropped TTTR files are identified in the MMFDB object store (deduped by content
...
- MMFDB failures warn but never crash the shifter.
- API/contract/MMFDB tests and a GUI smoke test pass; all ten Definition of Clean
  standards are met.

# Relationships
- First consumer and acceptance case for [PRD-10](prd-10.md) (dataset browser "From MMFDB…").
