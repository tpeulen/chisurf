...
type: PRD
prd: "34"
title: "PRD-34: Burst-ID Native MMFDB Save and Downstream Ingest"
description: Makes MMFDB the default save target for burst-identification selections when connected, and lets downstream burst tools ingest them directly from the dataset picker.
status: planned
phase: "cross-cutting"
...
# Summary
When ChiSurf is connected to MMFDB, saving a burst-identification (BID) selection should register it in MMFDB (sample-linked, with the single output-folder group and burst tables) rather than only writing a loose folder, with a file-only escape hatch when disconnected. Downstream burst tools (the companion exploration tool, BVA, Burst MLE, burst-wise FCS, Trace/Burst Browser, BID→Analysis) then ingest BIDs directly by picking a registered artifact through the shared dataset picker instead of browsing the filesystem. Because burst files reference photons by index in the original TTTR file, a BID is registered as an on-disk directory reference (never copied into the object store), and downstream results are parented to the BID artifact for a clean raw → BID → analysis lineage.
...
When chisurf is **connected to MMFDB**, saving a **BID** (a burst-identification /
...
**MMFDB**, not (only) to a loose file on disk. Downstream burst plugins (the
companion exploration tool, BVA, Burst MLE, burst-wise FCS, Trace/Burst Browser,
BID→Analysis, …) should then **ingest BIDs directly from MMFDB** — pick a
registered BID via the dataset picker instead of browsing for a folder.

```
Burst Selection ──(MMFDB connected)──▶ BID registered in MMFDB (sample-linked)
                                            │
        ┌───────────────────────────────────┼───────────────────────────────┐
        ▼                                   ▼                                 ▼
   exploration tool (open BID)   BVA / Burst MLE / burst-FCS          BID→Analysis
   from the dataset picker       ingest the same MMFDB BID             ingest the same MMFDB BID
```
...
- duplicates the burst output and loses the single-group/named conventions MMFDB
...
The plumbing already exists: Burst Selection can register a run into MMFDB
(`--mmfdb`, PRD-28: raw inputs sample-linked, burst tables, and a single
`external_reference` output-folder group), and `mmfdb.datasets.open` resolves that
...
only meaningful next to its co-located TTTR. So "save to MMFDB" means **register a
...
copying would break the photon-index linkage. `mmfdb.datasets.open` materializes the
reference to that path; downstream tools open the path as today.

# Scope

## A. Save behavior (Burst Selection + any BID producer)
- When an MMFDB connection/sample context is active, the default "save BID" action
  registers the BID in MMFDB (sample-linked, `operation_type="burst_selection"`,
  the single output-folder group + burst tables) instead of only writing a folder.
- Keep a **file-only** escape hatch (no MMFDB / explicit "export to folder") for
...
- Each downstream tool gains an **"Open BID from MMFDB…"** entry that uses the
  existing `MmfdbDatasetPickerDialog` filtered to BID artifacts
  (`kind="external_reference"`, `format="directory"`, burst operation), resolves
  the artifact to a local BID path via `mmfdb.datasets.open`, and loads it exactly
  as a folder open would.
- File-browse open stays available for disconnected use.
- A small shared helper (mirroring `chisurf/plugins/ndxplorer/mmfdb_launcher.py`'s
  `resolve_dataset_path`) so each tool wires "pick → resolve → open" without
  reimplementing MMFDB access.
...
- **Registration:** `burst_selection/api/mmfdb.py` + the `--mmfdb` CLI path (PRD-28):
  raw+sample registration, burst tables, single output-folder group, real names.
- **Resolve/open:** `mmfdb.datasets.open` / `MFDatabase.open_dataset` →
  co-located BID folder; `resolve_dataset_path` in the exploration-tool launcher.
- **Picker:** `chisurf/gui/widgets/mmfdb/dataset_browser.MmfdbDatasetPickerDialog`
...
2. Burst Selection GUI: make "save BID" register to MMFDB when connected (default),
   with a file-only escape hatch; reuse the PRD-28 registration path.
3. Shared `open_bid_from_mmfdb(...)` helper (pick → `resolve_dataset_path` → return
   local BID path) for downstream tools.
4. Wire "Open BID from MMFDB…" into the downstream burst plugins (the exploration
...
# Definition of Done
- [ ] When connected, saving a BID registers it in MMFDB (reference, sample-linked,
      single named group); disconnected still writes a folder.
- [ ] Downstream burst plugins can pick a registered BID from MMFDB and open it
...
# Definition of Clean
Reuse the PRD-28 registration + `mmfdb.datasets.open` + the PRD-10 picker (no new
...
# Relationships
- Generalizes [PRD-28](prd-28.md) (which did this for the companion exploration tool specifically) to all BID producers/consumers, making MMFDB the default save target when connected.
...
- Complements [PRD-33](prd-33.md) (acquisition→MMFDB) with the same connected-save principle.
