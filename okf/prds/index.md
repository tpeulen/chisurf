...
* ✏️ [PRD-74: FCS kinetics model — full mode vs fast (eigenvalue-only bunching) mode](prd-74.md) — Add a fast path that skips the spatial S1-population solve and 3D autocorrelation and computes only the eigenvalue-based photokinetic bunching factor (brightness correctly included), trading the saturation volume expansion for near-instant evaluation, with a weak-power agreement contract between the two modes.
