...
MMFDB (`chisurf/core/mmfdb/`) is today an in-process, SQLite-backed store (`SCHEMA_VERSION = 39`, ~40 tables) consumed by ChiSurf plugins via a flat function API (`api.py`) and remotely via the ZeroMQ RPC server (`chisurf/server/`, `ChisurfClient`). Its canonical tables are **generated from mmCIF dictionaries** (`schema_from_dictionary.py` → `CREATE TABLE`/`ALTER` from `data/*.dic`), which descend from the wwPDB / PDB-IHM family: `mmcif_std`, `pdbx_v50`, `ihm_ext`, `ihm_flr_ext`, plus our own `mmfdb_flr_ext`.

**MMFDB is a prototype for an eventual worldwide public fluorescence-data resource** — a central, worldwide public resource for fluorescence data. This note answers: *what is the durable core of that resource, and what role (if any) does GraphQL play?*
...
- The established structural-biology data ecosystem's standard *is* the PDBx/mmCIF dictionary. Multiple independent public implementations consume the same dictionary. Our `mmfdb_flr_ext.dic` / `ihm_flr_ext.dic` is the candidate to become "the worldwide fluorescence standard."
...
- **Our data is graph-shaped.** The provenance DAG (`mmfdb_operation → mmfdb_artifact → mmfdb_edge`; `graph_upstream`/`downstream`/`traverse_canonical_graph`) plus `struct_ref` cross-refs are exactly the nested, federatable reads GraphQL serves well.
...
2. **Keep the access layer transport-agnostic.** `api.py` stays a clean domain layer with no ZMQ/HTTP/GraphQL assumptions, so GraphQL *and* REST *and* bulk export can be added later without a rewrite. Preserve the current discipline: `mmfdb_admin` calls `api.py` in-process; `ChisurfClient` calls the same surface over ZMQ. "The API is just functions."
3. **Treat `mmfdb_flr_ext.dic` as a published spec:** version it, validate deposits against it, guarantee CIF export/import round-trips (ties to PRD-02a / PRD-39).
4. **Do not conflate the MMFDB SQLite schema with the resource's serving store.** A worldwide read tier outgrows SQLite; dissemination APIs sit above whatever store we migrate to. Keep that boundary clean now.
...
| provenance edges (`mmfdb_edge`) | `upstream`/`downstream` connection fields |
...
- Constrains the [MMFDB (current)](/architecture/mmfdb.md), [MMFDB target](/specs/mmfdb.md), and [RPC target](/specs/rpc.md).
