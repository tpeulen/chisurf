...
output in MMFDB with full provenance — object stored, artifact and operation rows
created, input/output and derived-from / measured-sample edges wired, and
parameters recorded — without the plugin needing to understand MMFDB internals.
Most plugins produce data but do not write to MMFDB; this gives them a
...
Plugins should not need to understand MMFDB internals — just call
...
87 plugins produce data but do not write to MMFDB; this PRD gives them a
...
fixed, bounds, units}`) as an `mmfdb_parameter` row with a fresh `parameter_uuid`,
...
`mmfdb_parameter` columns `name`/`value`. Cases: dict data, file data
...
metadata=…)` in a `try/except pass` — MMFDB is optional and must never break the
...
- Adds a uniform write path over the [MMFDB (current)](/architecture/mmfdb.md) store for [plugins](/architecture/plugin-system.md); relates to [Plugins target](/specs/plugins.md).
