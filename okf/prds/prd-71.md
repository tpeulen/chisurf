...
* **1b — known-rate kinetics recovered — ⏳ outstanding on real data, passing as a
  code test.** A system whose exchange rate is known independently must come back
  correct, which is what exercises the occupation-time propagator and the
  span/duration lookup together. The tree contains no such *measurement*, so the
  physics gate stays open.

  What now exists is `simulate.py`: smFRET bursts with exchange on a chosen
  timescale, written as a real burst folder and read back through the ordinary
  reader. It is a **code test, never a physics test** — it shares its physics with
  the model — but it is generated by a genuinely different route (an explicit
  sampled Markov path per burst, and micro times drawn as a response sample plus an
  exponential delay, against the model's closed-form occupation law and wrapped
  moments), so a wiring error shows up rather than cancelling.

  Regimes are named in **transitions per burst**, because a rate means nothing
  without an observation window. Fitting the same molecule in each (3000 bursts):
  ...
  magnitude away from it**, symmetrically: far below, almost no burst switches, so
  the histogram cannot say how rarely; far above, every burst reports the same
  average, so it cannot say how often. That identifiability structure is asserted
  in the tests rather than smoothed over, because a tolerance wide enough to pass
  at both extremes would eventually be met by making the model agree with it.

  The sharp result is the negative: **static data returns no exchange.**
...
3b. **A generator with exchange on a chosen timescale** — ✅ *landed*.
   `simulate.py` samples an explicit Markov path per burst, assigns each photon to
   whichever state was occupied when it arrived, and draws its micro time as a
   response sample plus an exponential delay; `write_folder` emits a real
   burst-analysis folder with its manifest, so a simulated measurement goes through
   the same reader, channel verification and response estimation as a measured one.
   `true_responses()` hands back the declared response, which *measures* the
   contamination of the estimated one (its mean sits ~2.5 ns late) instead of
   leaving it a suspicion — the same contamination seen on the real measurement.

   Cost work this forced, both real: the nested background sum is now evaluated for
   every occupation node of a burst **at once** (they differ only in the acceptor
   probability), and the occupation grid is coarsened to ~16 nodes, since the
   propagator's own resolution runs to hundreds under fast exchange for a resolution
