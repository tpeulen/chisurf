...
# Status
In-progress (re-verified against code 2026-07-05). Landed: the hermetic test harness (`test/conftest.py`) and real in-process `MMFDBClient.call` contract test. **Open:** handler/session injection is not complete; `chisurf/core/mmfdb/api.py` and `chisurf/plugins/core/mmfdb_admin/backend/*.py` still contain many direct `MFDatabase(resolve_database_path())` calls, so the "one injected session/db boundary" DoD remains unmet.
