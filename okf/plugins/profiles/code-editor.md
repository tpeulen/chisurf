...
| Notebooks | `notebook_editor.py` — `.ipynb` tabs: a one-row toolbar over a stack of `NotebookCell` widgets (code/markdown/raw) run in a single in-process `Shell`, with a `Chinsole` terminal attached below sharing that same shell. Every surface is sized to its content; the `[n]` prompt lives in a left gutter, not a header row. |
...
- **How to re-measure:** `QT_QPA_PLATFORM=offscreen python -m build_tools.dev_utils.grab_notebook_editor` writes `_host`, `_widget`, `_cells`, `_states` (traceback, clamped output, markdown in edit mode, terminal hidden) and `_narrow` PNGs. Read the images; a construction test cannot see a clipped output panel.
- **Open front:**
