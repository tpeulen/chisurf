...
Beyond the string-equation catalogue, the FCS model list also carries **non-parse** (Python-computed) `ModelCurve` models — **`DyeShapeFCSModel`** ("FCS dye shape"), **`MaxEntFCSModel`** / **`MaxEntRHModel`** ("FCS MaxEnt" / "FCS MaxEnt rH"), **`MdfFCSModel`** ("FCS MDF (Gauss--Lorentz)"), **`GeneralFCSModel`** ("FCS (general: diffusion + bunching/anticorr)"), and **`FCSKineticsModel`** ("FCS (kinetics)") — registered in `experiment_configs.yaml` alongside `ParseFCSModel`. Every one of them is now a Qt-free pure model rendered from a `*.view.json` (PRD-38): `dye_shape.py` and `maxent_models.py` were extracted out of `gui/widgets/models/fcs/`, which is left as four deprecation shims. The two MaxEnt models expose their regularization sweep as the shared `LCurveData` and declare the AutoForm `lcurve` section, which carries the sweep window, the compute button, the detected corner and click-to-adopt — their bespoke plot class and controller are gone. Note that the sweep's misfit norm was NaN for any fit whose range was still unset (`Fit` starts at `xmax == 0`, an *empty* window), so the L-curve drew nothing until a range was chosen; a degenerate window now reads as the whole curve. `MdfFCSModel`/`GeneralFCSModel` are Qt-free pure models (PRD-62): `chisurf/core/models/fcs/mdf.py` moved the MDF model out of `chisurf/gui/widgets/models/fcs/` (deleting the hand-built `MdfFCSWidget`), regrouped its parameters into `FittingParameterGroup` tables (`physical`, `optics`, `outputs`) rendered by the generic `AutoForm`/`parameter_group_table` machinery (`mdf.view.json`), and renamed its emission-waist parameter from `R0` to `wem` — the old name collided with FRET's Förster-radius `R0` in the shared parameter-name registry and silently inherited its "Förster radius" description (see the registry-scoping fix below). The model closes the non-Gaussian molecule-detection-function gap: `chisurf/core/fluorescence/fcs/enderlein.py` ports Fretica's `FEnderleinMDF`/`FVeffEnderlein`/`FGdiffEnderlein` — the Gauss--Lorentz MDF `U(ρ,z)=κ(z)/w(z)²·exp(-2ρ²/w²)/norm` (with axially-varying beam width `w(z)` and pinhole collection efficiency `κ(z)`), the effective volume `Veff = π(∫κ)²/∫(κ²/w²)` (so absolute concentrations, not just relative amplitudes), and the diffusion autocorrelation obtained by convolving the MDF with the 3-D propagator. The lateral integral is analytic; the axial convolution uses **Gauss--Hermite quadrature in the difference direction** (substituting `z' = z + √(4Dτ)ξ`) so the propagator is resolved at any lag — a fixed z-grid fails once `√(4Dτ)` drops below the grid spacing. The model exposes N, D (µm²/s), `w0`/`wem` (nm), a background count rate `bg` (kHz) and fixed optics (λex/λem, n, pinhole, magnification), reports `Veff`, concentration, `τ_D` and a background-corrected molecular brightness (`compute_brightness`, `(CR_total - bg)/N` in kHz, `CR_total` from the data file's `mean_count_rate` metadata, e.g. the Kristine format's header) as outputs, and takes zero or more **bunching** relaxation terms (`chisurf/core/models/fcs/relaxation.py::BunchingTerms`, an add/remove dynamic table). It is the accurate-`Veff` basis two-focus/dual-focus calibration wants (a known inter-focus separation `diam` > 0).

`GeneralFCSModel` (`chisurf/core/models/fcs/general.py`) is the general composable FCS model: pick a `diffusion_mode` — `"gauss"` (the default; a classic single-focus 3-D-Gaussian `GaussDiffusion` group), `"mdf"` (reuses `MdfPhysical`/`MdfOptics`), or `"two_focus"` (the classic Dertinger dual-focus technique, the same `GaussDiffusion` class as `"gauss"` but a separate preset instance with its `diam` unfixed and non-zero by default — a directly selectable, discoverable option rather than a parameter buried in the Gaussian table) — and add any number of **bunching** and **anticorrelation** (photon-antibunching) terms (`relaxation.py::AnticorrTerms`, a pure sub-Poissonian dip `1 - a·exp(-τ/t)`, no plateau shift; time constants ns-scale, vs. bunching's ms-scale). Only the active mode's panel is even visible: the `diffusion_mode` `choice` section carries `rebuild_on_change` (a declarative, opt-in `AutoForm.rebuild()` trigger — see [gui-autoform](/subsystems/gui-autoform.md)) and each diffusion panel fully hides via `hidden_when: {attr: diffusion_mode, not_equals: "<own mode>"}` (header included, not just folded), so switching modes live-hides the siblings instead of only at editor-build time. `GaussDiffusion` reports `s = w_z/w_r` (the classic structure/aspect-ratio parameter) and brightness as derived outputs even though it still fits absolute `w_r`/`w_z` (not the dimensionless legacy parametrization); its baseline offset `b` defaults to 1 (the typical normalized-ACF convention) rather than 0. Both `MdfFCSModel` and `GeneralFCSModel` expose the compound formula currently being fit as a live "Equation" panel (`equation_html()`, an `info`/`source` section) — the diffusion term times each active bunching/anticorrelation factor, the latter rendered via `BunchingTerms.equation_html()`/`AnticorrTerms.equation_html()` on the term groups so it can't drift from what `apply()` actually computes. This supersedes the diffusion-x-bunching/anticorrelation combinatorial subset of the `models.yaml` catalogue (`"3D Gauss, N bunching"`, `"... + antibunching"`) for new work; the catalogue itself is unchanged (and keeps its own, older "antibunching" terminology) and remains the path for cases `GeneralFCSModel` does not cover (flow, scanning FCS, FRET-FCCS, afterpulsing/bleaching correction terms). Headless coverage: `test/fluorescence/test_enderlein_fcs.py` (MDF shape, `Veff` scaling, `g(0)=1` normalisation and `g(0)=1/Veff` consistency, D-scaling of the decay), `test/gui/test_fcs_models_resolve.py` (both models resolve + appear in the FCS menu as `ModelCurve`s), and `test/gui/test_fcs_model_editor.py` (AutoForm table rendering, bunching/anticorrelation add/remove with decade-spaced defaults, two-focus suppression, default-mode/offset/output checks, live re-fold on mode change, the `wem`-not-Förster-radius registry regression guard).

`FCSKineticsModel` (`chisurf/core/models/fcs/kinetics.py`) is the *photokinetic* FCS model — the home of the kinetic saturation scheme that used to sit inside `GeneralFCSModel`. The general model is deliberately kept purely operational (analytical diffusion + relaxation terms); an arbitrary N-state photochemical scheme cannot fit there because its parameters are meaningful only when the model actually evaluates the non-analytical path. `kinetics.py` therefore owns the rate-scheme groups — `RateMatrixRates`/`DarkRates`/`ExcRates` (off-diagonal transition matrices as fit parameters, dark in `1/µs` and excitation as dimensionless cross-sections, rebuilt on `n_states` change), `StateBrightness` (relative per-state `Q_i`), and `KineticSaturationTerms` (power/extinction/`w_r`/`w_z`/`D`/`N` plus the dark/excitation matrices and brightness) — composed with a `GaussDiffusion` term (`"gauss"` single-focus or `"two_focus"` Dertinger preset, the same `diffusion_mode` live-hide machinery as `GeneralFCSModel`) and optional bunching/anticorrelation relaxation terms. When `Power (P) > 0` the model switches onto `saturated_curve_shape` (`chisurf/core/fluorescence/fcs/saturation.py`, shared with the `fcs_saturation_calc` calculator): the steady-state population profile of the rate scheme is solved over a `(r, z)` grid spanning 5 beam waists, spatially autocorrelated via a 3-D FFT (`fcs_numerical_g_diff`, with the DC mode removed so it belongs to the baseline), and the amplitude carries the volume-expansion ratio `V_0/V_eff` — so `G(0)` drops as power rises (G(0) ≈ 1.98 analytical → 1.40 at 2 mW for the default 3-state scheme). `P = 0` reproduces the analytical Gaussian shape exactly, so the model also serves as an advanced FCS fit model without saturation. The numerical shape is cached (keyed on the lag grid, power, extinction, both matrices, brightness, `w_r`/`w_z`/`D`), so fit iterations that only vary `N`/`b`/relaxation parameters do not re-run the 3-D FFT (~0.4 ms cached vs ~150 ms cold). The `fcs_saturation_calc` plugin tool reuses the same `KineticSaturationTerms` group directly (`chisurf/plugins/calculator/fcs_saturation_calc/gui/tool.py`). Headless coverage: `test/gui/test_fcs_models_resolve.py` (resolves + appears in the FCS menu), `test/gui/test_fcs_model_editor.py` (editor renders, P=0 analytical equivalence, SatNumInt equation, `G(0)` saturation drop), and the `fcs_saturation_calc` plugin suite.

The parameter-name registry (`chisurf/core/settings/constants/parameter_registry.json`, generated by `build_tools/dev_utils/export_fitting_parameters.py`) used to key every `FittingParameter(name=...)` call site by its bare name only, so two unrelated classes reusing the same short name (FRET's `R0` vs. the old FCS MDF model's own `R0`) silently merged into one shared description. PRD-62 made this scoped: the generator now also emits a `by_qualified_id` index (`"<ClassName>.<name>"`) and flags a bare-name entry `"ambiguous"` when more than one class contributes to it; `chisurf.core.parameter.Parameter.__init__`'s registry lookup prefers an explicit `registry_id=` (e.g. `"fcs_mdf.wem"`, the convention already used by RICS's `registry_id="rics.D"`-style groups), then a class-scoped match derived by inspecting the constructing frame, and only falls back to the legacy bare-name index when it is not ambiguous — see [parameters](/subsystems/parameters.md).
