...
| `fcs_correlator` | Correlator workflow (hidden building block) | Two-pane navigation workflow that selects/filters TTTR photons, computes correlation functions and merges curves; exposes correlator, filter and merger panels (each an AutoForm `*.view.json`). `menu_hidden` — hosted as the first section of `FCS`. |
