---
type: Specification
title: PTO.MFDB — the MMFDB profile of the photon container
description: How one measurement is written as one .pto file — the instrument data kept verbatim and restorable at the front, every result an artifact beside it, and every name taken from the mmCIF dictionaries rather than invented.
tags: [specs, pto, mfdb, container, provenance, flrcif, data-io]
...
from the [MMFDB](/architecture/mmfdb.md) dictionaries, so a `.pto` and a row in
...
| | name | what it claims |
|---|---|---|
| the container | `<name>.pto` | an EBML document with `DocType "pto"`. Says nothing about what is inside. |
| this profile | `<name>.mmfdb.pto` | a `.pto` that **also** conforms to PTO.MFDB: the container-level profile tags are present and every kind, encoding, grain and relation in it is a term from the MMFDB dictionaries. |
...
| PTO | PTO.MFDB | Term source |
|---|---|---|
| `AttachedFile` | one artifact | — |
| `PtoKind` | `artifact_kind` | `_mmfdb_artifact.artifact_kind` |
| `PtoEncoding` | `data_format` | `_mmfdb_artifact.data_format` |
| `PtoRowCount` | `row_count` | `_mmfdb_artifact.row_count` |
| `FileMediaType` | `mime_type` | `_mmfdb_artifact.mime_type` |
| `FileName` | a label, not an identity | — |
| `FileUID` | container-local handle only | — |
...
[assessment](assessment.md) tracks the specifics. The direction of travel is:
