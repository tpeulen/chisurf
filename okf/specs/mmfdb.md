---
type: Specification
title: MMFDB — Metadata & Provenance Store — Target
...
tags: [target, mmfdb, metadata, provenance, schema]
timestamp: '2026-07-06T00:00:00Z'
---

> The clean-architecture target for metadata and provenance. Current shape: [MMFDB architecture](/architecture/mmfdb.md). Current-state gaps: [assessment](assessment.md).

## Purpose

MMFDB is the vendored package for ChiSurf's memory of *what was done*: which samples and measurements
...
[Core](core.md) produces the science, and MMFDB remembers its lineage.
...
- **One access layer.** Application code reaches MMFDB through a single
...
scientific objects themselves (that is [Core](core.md)). MMFDB is imported as
`mmfdb` and surfaced to
...
3. Application code reaches MMFDB only through the single function API — not raw
...
Today MMFDB is a historic mess: the core tables are hand-written (and duplicated
