...
- 📋 **Still open** — `categories`-vs-directory-group drift (`batch_analysis` lives in `core/` but is `"Main:Tools:…"`), the undocumented hub/child `menu_hidden` pattern, dead `deprecated` fields, and gating the demo games.
