...
| [INC-07](#inc-07) | S3 | INC | Plugins | `categories` drifts from directory group & `display_name`; demo games mixed in | 🚧 PARTIAL (`categories` + menu-path vocabulary deduplicated & case-enforced; demo games gated on a `demo` manifest flag; directory-group drift and the hub/child pattern still open) |
