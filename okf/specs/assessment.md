...
| [DATA-05](#data-05) | S2 | DATA | MMFDB | External-tool runs not first-class in provenance (no `command_line`/`exit_code` cols, no `external_tool` op type, inconsistent op_type validators) | 🚧 PARTIAL (validators reconciled; `command_line`/`exit_code` columns + `external_tool` type landed; actor flag + container schema open) |
| [DATA-06](#data-06) | S3 | DATA | MMFDB | Deposition is one-way: `archive.zip.export` exists but no importer; bundler reads `file_path` not object store; mmCIF export is FLR-only | 🚧 PARTIAL (CIF export exposed on `api.py` + `cli.py`; bundler now object-store-aware and collision-free; importer + general CIF path open) |
...
| [INC-07](#inc-07) | S3 | INC | Plugins | `categories` drifts from directory group & `display_name`; demo games mixed in | 🚧 PARTIAL (`categories` + menu-path + ribbon-tab vocabulary deduplicated, case-enforced & closed; demo games gated on a `demo` manifest flag; directory-group drift and the hub/child pattern still open) |
| [INC-08](#inc-08) | S3 | INC | Server | Generic `JobManager` bypassed by the only real long-running jobs | ~~REPORTED~~ ✅ FIXED |
| [INC-09](#inc-09) | S3 | INC | MMFDB | MMFDB is packaged standalone but a chisurf-free client is missing; the only RPC client + example facade live in chisurf | 🚧 PARTIAL (HTTP transport ships as `mmfdb.client`; ergonomic facade + in-process principal open) |
...
- 🚧 **Validators reconciled** (2026-07-26): `operation_type` now has one authority — the extensible `mmfdb_vocabulary` table checked inside `record_operation`. `record_operation_with_artifacts` and the chinet adapter's `store_chinet_session` dropped their static `.dic`-enum pre-checks, which had made the extension mechanism only half-work: a type added with `register_vocabulary_value()` was accepted by the plain record path and rejected by the two richer ones. Nothing is loosened — `mmfdb_operation.operation_type` carries no CHECK constraint and the vocabulary table is *seeded from that same enum* at schema creation, so the extensible check is a strict superset of the static one. The rejection now surfaces from inside the transaction instead of before it; a test pins that an unknown type still leaves no operation, artifact or link row behind. Tests: `modules/mmfdb/tests/test_fdb_vocab_and_migration.py` (+2), `test/fio/test_mmfdb_chinet_adapter.py` (+2).
- 🚧 **Command line, exit code and `external_tool` landed** (2026-07-27, mmfdb `a853295`): the dictionary is the schema authority, so all three go into `mmfdb_flr_ext.dic` — an `external_tool` value on the `_mmfdb_operation.operation_type` enumeration, plus two new items `command_line` (verbatim invocation) and `exit_code` (0 / non-zero / negative on a POSIX signal, deliberately distinct from `status`, which is the provenance lifecycle state). Schema **v45** reconciles an existing database: the two columns are added and the vocabulary is re-seeded from the dictionary. Both are threaded through `record_operation`, `record_operation_with_artifacts` and the `api.py` boundary that the versioned `mmfdb.v1.operations.*` RPC methods dispatch to, so `SELECT operation_id, command_line FROM mmfdb_operation WHERE exit_code != 0` is now a query rather than a `settings_json` scan. The **workflow runner** was the in-tree offender and is now the demonstration: it wrote both into `settings` and claimed exit code `0` for an in-process `python:` step that never spawned a process — it now stamps the columns and leaves `exit_code` NULL where there is no process. The shipped external-tool example stopped masquerading as `burst_selection`. Tests: `modules/mmfdb/tests/test_fdb_vocab_and_migration.py` (+4), `test_api_auth.py` (+1), `test_workflow_runner.py` (updated). Still open: the software-vs-human actor flag (`operator_user_id` is a human-user FK) and a container/environment schema (image + digest).
...
- ✅ **The bundler reads the object store, and stopped losing same-named files** (2026-07-28, mmfdb `9e2f6fa`): the finding's "copies native files from `node.file_path`" half is closed, and a second defect surfaced while closing it. Every bundled payload was named after its **basename alone** (`external_data/<basename>`), so two artifacts called `data.ptu` in different directories wrote to one destination — `shutil.copy2` overwrote the first, `zipfile` emitted two members under one name (it warns `Duplicate name`, into a log nobody reads), and the manifest pointed **both** nodes at it. Restoring by manifest entry then returned the wrong bytes for one of them with nothing in the archive saying so: the deposition analogue of the [burst-companion](../subsystems/burst-companions.md) column-wise misalignment, where a wrong layout does not fail, it silently mismatches. Members are now allocated through `_archive_member_name()` (suffix a taken name), and two nodes that genuinely share one blob or one file are bundled **once** with both entries pointing at that member. On the original half, `_bundle_payload_source()` prefers the content-addressed store whenever the node carries an `object_uuid` — the store is the durable location, `file_path` may be stale, on another machine or absent — and falls back to the recorded path; a node reachable *only* through the store used to be skipped before it got a manifest entry at all, and is now bundled. Each entry records the `source` it came from, and an unreachable payload records `unavailable_reason` instead of a bare `copied: false`, so a gap in a deposition is stated rather than silent. Tests: `modules/mmfdb/tests/test_zip_archive_export.py` (6) — the handler had **none**; 5 of the 6 fail against the previous code. The 4 chisurf-side tests in `test/fio/test_fdb_archive.py` (which cover `base_path_map` remapping) pass unchanged. Still open: the `archive.zip.import` handler and the general dictionary-driven CIF path.
...
- 🔍 **A second hand-kept method contract survives inside the server** (found 2026-07-26 while reconciling the docs with the registry). `chisurf/server/protocol.py::METHOD_CATALOGUE` is a hand-maintained dict that `dispatcher._handler_from_spec` returns verbatim for `meta.protocol`, while the methods actually registered come from `server_methods.json`. Diffed: no phantom entries (everything in the catalogue *is* registered), but **40 registered methods are missing from it**, including three whole namespaces — `plot`, `detector_setups`, `flr` — plus `fit.sample.*`, `fit.parameter_scan.*`, `fit.group.*`, `fit.diagnostics`, `fit.posterior`, `fit.reweight_prior`, `fit.select`, `fit.reorder`, `fit.mask.set`, `fit.range.auto`, `fit.parameter_snapshot`, `fit.restore_parameters`, `fit.add`, `model.component.*` and `model.state.*`. A client discovering methods through `meta.protocol` therefore cannot see the sampling, scan, plot, detector-setup or FLR surface; `meta.methods` / `list_methods`, which read the dispatcher's real registration table, do show them. → Generate the catalogue from `server_methods.json`, or add a guardrail test that the two agree.
  - ✅ **FIXED** (2026-07-26): `METHOD_CATALOGUE` is now **generated** — `protocol.build_method_catalogue()` groups `load_method_specs()` by namespace, so `meta.protocol` advertises all 93 registered methods across 14 namespaces instead of 50 across 10; the four namespaces the hand-kept dict never had (`detector_setups`, `flr`, `plot`, `pda`) are visible. Only the namespace prose (`NAMESPACE_DESCRIPTIONS`) stays hand-written, and a namespace without prose gets a placeholder that a guardrail test fails on. The same defect was hiding in `METHOD_SCHEMAS`, which hand-repeated each method's `events`: it advertised **four topics no handler ever publishes** — `parameter.changed` (7 methods), `editor.document.changed` (3), `fit.added` and `session.cleared` — and missed the one `fit.create` really emits (`fit.created`), so a client subscribing off `meta.protocol` waited on dead topics. The hand-written half is now `METHOD_PARAM_SCHEMAS` (params + result only) and `build_method_schemas()` merges the registry's `events` in, which makes the SV-05 event contract structurally unforgeable rather than test-enforced. `server_methods.json` is read through one loader shared with `ServiceDispatcher._build_default_registry`. Guardrails: `test/server/test_protocol.py` (+4, 2 rewritten) pins catalogue == registry, prose for every namespace, no phantom schema methods, and schema events == registry events. Remaining under INC-03: the MMFDB `mmfdb.*` / `mmfdb.v1.*` split, `list_methods`, and the 43 registered methods that still have no parameter/result schema.
...
- ✅ **The flags reach the menu a tool is actually opened from** (2026-07-28, RF-517): the discovery record `iter_plugins()` yields now carries `experimental` / `deprecated` and their messages, so a host that embeds no panel can still mark the tool — and the ribbon does, appending the marker to the button label and the warning line above the description in its tooltip. Both hosts read one table (`navigation.MATURITY_FLAGS`) through `maturity_markers` / `maturity_message` / `maturity_warnings`, which `_panel_label` and `_maturity_banner` were rewritten onto, so the two entries for one tool cannot drift. Of the 8 flagged plugins in the tree, 6 are `menu_hidden` (hub panels); the two menu-visible ones — `lifetime_analysis` and `spectra_downloader` — now render `⚠️` in the ribbon, screenshotted offscreen and inspected. Tests: `test/gui/test_navigation_maturity.py` (+7), `test/plugins/test_plugin_maturity_metadata.py` (+3, tree-wide: a manifest flag the record drops fails the suite). Docs: `docs/development/plugin_architecture.md`. Still open in RF-517: the warning **banner atop the launched window** — a ribbon entry `exec`s the plugin's `__init__.py` and gets no widget handle back, so there is nothing to wrap; that is a launch-path change of its own.
...
- ✅ **A plugin that declares no menu path lands in a bucket its author never chose** (2026-07-28): `display_name` is the *only* live identity field — `_read_manifest_metadata()` prefers it over the legacy module-level `name`, and its `:`-separated segments are what both the ribbon and the generated catalogue group by. Nothing requires it to carry a path, and one manifest did not: `spectra_downloader` declared `"Spectra Downloader"`, so `_parse_hierarchical_plugin_name()` returned the `Main` fallback category and `_display_parts()` printed `Uncategorized` — while the plugin's own `__init__.py` had declared `name = "Spectroscopy:Spectra Downloader"` all along and its `categories` said `["Spectra"]`. Three sources, three answers, and the one that won was the accidental one; the published catalogue carried a whole `## Uncategorized` section holding this single plugin, whose page contradicted itself (`Menu path | Uncategorized` beside `Categories | Spectra`). This is the same failure family as [BUG-13](#bug-13) — a plugin misdescribing its own location in silence — but it survived the two INC-07 guardrails added earlier because both waive a manifest with no parent segments (`if not parents … continue`). The manifest now declares the path its module always did (`"Spectroscopy:Spectra Downloader"`, `categories: ["Spectroscopy", "Spectra"]`), which also brings it *under* `test_builtin_categories_agree_with_menu_path` instead of being skipped by it. Guardrail: `test_builtin_manifests_declare_a_menu_path` fails tree-wide on any `display_name` without a `:`, plus a pin of the fallback on the live ribbon parser so the rule cannot be read as arbitrary. Tests: `test/plugins/test_plugin_menu_metadata.py` (new, 6). Docs: the catalogue's `Uncategorized` section is gone.
- ✅ **A plugin opened a ribbon tab of its own** (2026-07-29): the ribbon builds its top-level tabs *from the manifests* — the first `display_name` segment becomes a category, created on demand (`ribbon_plugins.py:157`) — so a first segment nobody else uses opens a whole tab holding one entry. `img_tracking` declared `Microscopy:Imaging:Particle Tracking` while the other fourteen tools in `plugins/microscopy/` declared `Imaging:…`, so the ribbon grew a `Microscopy` tab whose single `Imaging` panel held that one tool, beside the `Imaging` tab holding the rest. The OKF concept [imaging](../plugins/imaging.md) had recorded the intended path as `Imaging:Particle Tracking` all along and the guide said `Microscopy → Imaging → Particle Tracking`: three sources, two answers, and the accidental one was what the user saw — the same silent-misdescription family as the `Uncategorized` manifest above, but surviving both guards added there because a *deeper* path passes a check that only asks for a path at all. The manifest now declares `Imaging:Particle Tracking` / `["Imaging", "Tracking"]`. Verified by building the real ribbon offscreen before and after: tabs went `Analysis, Imaging, Main, Microscopy, Spectroscopy, Structure, Tools` → `Analysis, Imaging, Main, Spectroscopy, Structure, Tools`, with Particle Tracking now rendered beside Colocalization, Drift Correction and FRC Resolution. Guardrail: `test/plugins/test_plugin_menu_tabs.py` (4) pins the closed set of top-level segments a menu-visible manifest may declare — and pins that every entry in that set is a tab some host or plugin really builds, so the vocabulary cannot drift into a wish list. Checked in the same pass and **not** a defect: `database_connector` declares `Core:Database Connector` but exposes no `gui` entrypoint, `iter_plugins()` derives `cli_only` from exactly that, and the ribbon hides CLI-only plugins outside experimental mode — no `Core` tab is built, and the guard skips those manifests on the same grounds. Docs: guide 50, the plugin catalogue and the [use case](../usecases/particle-tracking-diffusion.md).
- ✅ **The demo games are gated on a flag they declare themselves** (2026-07-29): five games and their hub shipped in the menus beside the analysis tools with nothing marking them as demonstrations — the tree had no way to *say* "this is a toy", so the only alternatives were deleting them or leaving them in. A manifest now declares `"demo": true`, and the gate is applied at the **one** seam every menu reads: `_iter_plugins_uncached()` turns the flag into `menu_hidden` unless `plugins.show_demo_plugins` is set, so the ribbon, the plugin menu and the two ribbon-category builders are all covered without a filter being written four times (and without a fifth host being able to forget it — the same reasoning that put the maturity flags into the discovery record for RF-517). The Plugin Manager and the help browser do not filter on `menu_hidden`, so a demo stays inspectable and installable there; the record carries `demo` verbatim, so a host that wants to present them differently can. `demo` is deliberately *not* a maturity flag: it carries no banner and says nothing about how far the tool can be trusted, so it is not a row in `MATURITY_FLAGS`. Guardrails: `test/plugins/test_plugin_demo_gating.py` (9) — the flag is a declared key of the closed manifest schema and round-trips through `to_dict`, every manifest under `plugins/misc/games/` declares it, no demo is menu-visible by default, opting in brings the hub back, and the gate changes **no** non-demo plugin's visibility. Docs: `docs/development/plugin_architecture.md` §3.1.2.
- 📋 **Still open** — `categories`-vs-directory-group drift (`batch_analysis` lives in `core/` but is `"Main:Tools:…"`) and the undocumented hub/child `menu_hidden` pattern.
...
- 🚧 **PARTIAL** (2026-07-28): the **HTTP transport** moved into the package as `mmfdb.client` — `validate_base_url()` plus `HttpJsonRpcClient` (JSON-RPC over `POST /rpc` with a bearer token, and streamed, bounded object upload/download). It is standard-library-only and imports neither the repository nor the admin host, so an external tool now gets the hardened transport by importing `mmfdb` instead of copying `examples/external_tools/mmfdb_min_client.py`, which had no URL policy and no size bounds at all. ChiSurf's `MMFDBClient` imports it rather than carrying its own copy, so the wire contract has **one** implementation; what stays chisurf-side is the ergonomic layer on top (the ~90 named method wrappers, YAML-settings/credential-store integration, and the in-process and message-queue transports). Tests: `modules/mmfdb/tests/test_http_client.py` (21, against a throwaway `http.server` — URL policy, bearer header, error-body propagation, object round trip, oversize/short-stream refusal); the 20 existing chisurf tests in `test/plugins/test_mmfdb_http_client.py` pass unchanged against the moved code. Remaining: the ergonomic facade (`BurstWorkflow`) and an in-process default principal so `mmfdb.api` is usable without bootstrapping a user (INC-04).
...
Locations (42 files): densest under `chisurf/gui/widgets/models/tcspc/` (3),
...
= 42). Migrate opportunistically as each form is touched; keep every un-migrated
form on the `retranslate_from_ui` path so language switching keeps working meanwhile.
The 43rd was `gui/plots/surfaceplot/chi2Hist.ui`, deleted with its orphaned
