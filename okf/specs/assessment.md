...
- 📋 **Still open** — `categories`-vs-directory-group drift (`batch_analysis` lives in `core/` but is `"Main:Tools:…"`), the undocumented hub/child `menu_hidden` pattern, surfacing the declared-but-unrendered `deprecated` flag the way `experimental` is surfaced, and gating the demo games.
