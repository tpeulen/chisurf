...
**ChiSurf today** has `ChiSurfProgress` and the AutoForm `progress` section for
*display*, but the execution side is ad-hoc — `ThreadPoolExecutor`/`QThreadPool`
appear in three unrelated places with no shared cancel or partial-result
contract.

**Do:** add a `ChiSurfTaskMixin` next to the progress seam with exactly this
surface, and make the AutoForm `progress` section its default renderer. Fits
[PRD-23](/prds/prd-23.md) (thin widgets): long work moves off the widget behind
a uniform contract.
