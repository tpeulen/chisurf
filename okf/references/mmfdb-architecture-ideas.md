---
type: Reference
title: MMFDB Architecture Ideas
description: The durable design rationale behind the MMFDB overhaul PRDs — each decision grounded in an observed bug class.
tags: [reference, mmfdb, architecture]
timestamp: '2026-07-06T00:00:00Z'
---

# Reference

This concept captures the design rationale behind the MMFDB overhaul PRDs. Each
...
**Observed problem.** Lineage was ad-hoc `mmfdb_edge` SQL re-written per call site
...
widgets with no construction coverage; an FCS dialog wrote to MMFDB on
construction.
...
## Extract MMFDB into a standalone package

MMFDB (schema, repository/API, server, admin) should become an independent module
...
- **Uniform error policy:** "MMFDB unavailable → soft warn; real error → raise"
...
- **Sample tables:** make the `mmfdb_sample` (index) ↔ `flr_sample` (canonical)
...
## Bold / breaking ideas (possible because MMFDB is unreleased)
...
family, the flrCIF `flr_*` family, and the `mmfdb_*` family — with reads and writes
split across them. That split caused a sample-name bug
(`mmfdb_sample.display_name` shadowed the authoritative `flr_sample.description`)
...
categories). Delete the legacy `fdb_*` family; remove the `mmfdb_*` tables that
...
Because MMFDB is unreleased, the historical migration versions carry no value.
...
MMFDB is already provenance-first and content-addressed, and an `mmfdb_branch` hint
