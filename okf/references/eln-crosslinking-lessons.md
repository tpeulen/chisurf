...
description: What eLabFTW's auth, entity-linking, tagging and metadata model teaches MMFDB — what was adopted and what is deferred.
tags: [mmfdb, provenance, crosslinking, metadata, eln, lessons]
...
inform MMFDB's information-management and crosslinking model. External software is
...
The ELN validates MMFDB's design: search-then-bind LDAP, group/team reconciliation
...
which confirms MMFDB was right to **skip timing-equalization**. Patterns worth a
...
## Crosslinking — MMFDB is architecturally ahead
The ELN uses a **per-pair junction matrix** (~25 `X2YLinks` tables). MMFDB's single
**polymorphic `mmfdb_edge`** (`source/target node_type+id`, `relationship_type`,
...
(`graph_upstream`/`downstream`). Keep MMFDB's single-edge model; do **not** copy
the junction matrix.
...
MMFDB uses **`.dic`-dictionary-driven typed vocabulary + a lifecycle state
machine**. Keep MMFDB's dictionary approach (better for a standardized databank).
...
- **Metadata → edge materialization** (the ELN's "a metadata field that
  ...
  `mmfdb://<node_type>/<node_id>` on a sample/experiment key-value now materializes
  an idempotent `mmfdb_edge` (`linked_to`, tagged with the originating key). See
...
- **Tags** — a lightweight polymorphic tag layer (`mmfdb_tag` / `mmfdb_tag_member`)
  ...
  reference shape. Optional — only if ELN-style browsing is wanted; MMFDB is
  provenance-first, not a general ELN.
- **Copy-edges-on-clone** — a generic helper to copy a node's edges when it is
  duplicated (the ELN's `duplicate()` on links); MMFDB currently handles bulk copy
