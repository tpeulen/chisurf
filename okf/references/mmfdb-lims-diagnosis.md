---
type: Reference
title: MMFDB ↔ LIMS design diagnosis
description: Gap analysis of MMFDB against laboratory information-management-system design principles, mapping each gap to PRD-12 through PRD-15.
tags: [reference, mmfdb, lims]
timestamp: '2026-07-06T00:00:00Z'
---

# Purpose

This concept preserves the durable gap analysis behind the MMFDB LIMS-workflow
PRDs. It asks: what does a mature **laboratory information-management system
(LIMS)** provide that MMFDB — ChiSurf's SQLite-backed metadata and provenance
store (`chisurf/core/mmfdb/`) — does not yet, and how does each gap map onto a
...
The takeaway: MMFDB already implements the **data** half of a LIMS more strongly
...
# Where MMFDB already stands (and exceeds a classic LIMS)

MMFDB is a **provenance-first scientific data system**. On several axes it is
*stronger* than the reference LIMS, and those strengths should be preserved:

- **Dictionary-dictates-schema (flrCIF/pdbx).** MMFDB's `.dic` dictionaries plus
...
- **Content-addressed object store with dedup** (`mmfdb_object.content_md5`,
  refcounting) — stronger than file-path/mass-storage tracking.
- **Fine-grained provenance graph** — `mmfdb_operation`, `mmfdb_edge`
...
- **Branching** (`mmfdb_branch`), **multi-owner + ACL** (`mmfdb_artifact_owner`,
  `mmfdb_acl_entry`, `mmfdb_group`), **time-versioned calibration**
  (`mmfdb_setup_calibration`), **extensible vocabulary** (`mmfdb_vocabulary`).

# Gap analysis — LIMS principles MMFDB lacks

Verified against the MMFDB schema: MMFDB has no project/study entity (only a loose
...
| LIMS principle | MMFDB today | Gap / opportunity | PRD |
|---|---|---|---|
| Lifecycle state machine + history | `mmfdb_operation.status` enum; `mmfdb_artifact.validation_status`; `flr_experiment.status` — flat flags, no transition history | **No tracked lifecycle** for sample / experiment / dataset (registered → measured → processed → validated → archived) with who/when transitions | [PRD-12](/prds/prd-12.md) |
...
| Request → resolution → delivery | none | No way to request an analysis / deliver results between users in a shared MMFDB; in scope only if MMFDB becomes a shared lab service | — |
...
Each adoption is implemented **the MMFDB way** — dictionary-declared, generated,
...
The single biggest LIMS principle MMFDB lacks, and the highest value. Rather than
per-entity status columns, add a generic, dictionary-declared **state-transition
log**: an `mmfdb_state_transition(id, entity_type, entity_id, from_state,
...
(`mmfdb_vocabulary`). Lifecycles: **sample** (registered → measured → processed →
...
building on `mmfdb_audit_log` but queryable per entity.

## Study/project entity with configurable fields → [PRD-13](/prds/prd-13.md)

Promote `project_id` from a loose string to a real grouping. Add
`mmfdb_study(study_id, name, description, owner, is_public, created_at, …)` and
`mmfdb_study_member(study_id, sample_id|artifact_id)`. Reuse the existing
dynamic-field pattern (`flr_sample_key_value`, `mmfdb_*_key_value`) for per-study
...
For reproducibility, a minimal `mmfdb_reagent_lot(lot_id, kind, name, lot_number,
expiry, …)` referenced from operations/setups (e.g. which fluorophore lot, which
filter set). Lower priority for a single-lab tool; valuable once MMFDB is shared.
...
likely out of scope and should be revisited if MMFDB is deployed as a shared lab
service. Neither has a dedicated PRD in the PRD-12…PRD-15 wave.

# Summary

MMFDB already implements the **data** half of a LIMS better than the reference
